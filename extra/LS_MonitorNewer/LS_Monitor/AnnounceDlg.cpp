...
BEGIN_MESSAGE_MAP(CAnnounceDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_PRACTICE, &CAnnounceDlg::OnBnClickedButtonPractice)
END_MESSAGE_MAP()
...
void CAnnounceDlg::OnBnClickedButtonPractice()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CString userId;
	...
	for(int i = 0 ; i < count ; i++)
	{
		...
		if(!userId.IsEmpty())
		{
			m_parent->RequestPractice(userId);
		}
	}

	MessageBox(_T("Completed"), _T("PRACTICE OUT"), MB_OK);
}
