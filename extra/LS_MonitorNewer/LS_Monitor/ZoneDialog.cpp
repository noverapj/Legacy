...
void CZoneDialog::OnClose(const ServerTypes serverType, const int errCode, const int billingKeyIndex)
 {
	if(serverType == SERVER_TYPE_MAIN)
	{
		...
		SetTimer(MONITOR_TIMER_MAINSERVER, 2000, NULL);
		CLS_MonitorDlg* dlg = (CLS_MonitorDlg*)AfxGetApp()->m_pMainWnd;
		dlg->OnMainServerDisConnect(GetOtherServerInfo(SERVER_TYPE_MAIN), SERVERSTATE_OFF);		//타입이랑, 패킷을 PostMessage로 보내도록 수정해야함
	}
	else if(serverType == SERVER_TYPE_LOGIN)
	{
		...
		SetTimer(MONITOR_TIMER_LOGINSERVER, 2000, NULL);
		CLS_MonitorDlg* dlg = (CLS_MonitorDlg*)AfxGetApp()->m_pMainWnd;
		dlg->OnLoginServerDisConnect(GetOtherServerInfo(SERVER_TYPE_LOGIN), SERVERSTATE_OFF);		//타입이랑, 패킷을 PostMessage로 보내도록 수정해야함
	}
	else if(serverType == SERVER_TYPE_BILLING)
	{
		m_level = ADMIN_LEVEL_NONE;
...
void CZoneDialog::OnReceive(const ServerTypes serverType, SP2Packet &rkPacket)
{
	...
	case STPKM_UPDATE_CLIENT_VERSION :
		{
...
void CZoneDialog::ReadCurrentUserCCU()
{
	// 오늘 내역을 출력한다.
	...
	int iLastMaxCount = 0;
	int imaxValue = 0;
   	m_countGraph.SetDay(sysTime.wDay);
...
//			strTime = strTemp.Right(strTemp.GetLength() - (1));
		...
		if(iLastMaxCount != imaxValue)
		{
			...
			m_countGraph.SetRange(true,	0, imaxValue, 1);
			m_graphDlg->SetRange(true,	0, imaxValue, 1);
		}
...
//			strTime = strTemp.Right(strTemp.GetLength() - (1));
			...
			CString xunits;
			xunits.Format(_T("%04d-%02d-%02d User(%06d/%06d)"), iYear, iMonth, iDay, iCount, imaxValue);
			m_countGraph.SetXUnits(xunits);

			m_countGraph.AppendDateTimePoint(iYear, iMonth, iDay, iHour, iMinute, iSecond, m_bCountryCode, iCount);
			m_graphDlg->SetXUnits(xunits);
			m_graphDlg->AppendDateTimePoint(iYear, iMonth, iDay, iHour, iMinute, iSecond, iCount);
        }
    }

	m_graphDlg->ComboBoxSetting(sysTime.wYear, sysTime.wMonth, sysTime.wDay);
...
void CZoneDialog::UpdateGraph(const int userCount)
{
	...
	if(false == WriteLog(m_currentUser))
	{
		return ;
	}
...
bool CZoneDialog::WriteLog(int currentUser)
{
	if(m_file)
	{
		SYSTEMTIME sysTime;
		...
		ULONGLONG iDiff = DiffTime(m_BeforeSysTime, sysTime);
		if(iDiff > 60000)
		{
...
CIOSocket* CZoneDialog::GetSockMain()
{
	return &m_sockMain;
}
...
void CZoneDialog::RequestPractice(CString publicID)
{
	...
	int type = ADMINCOMMAND_PRACTICE;
	kPacket << type;
	m_sockMain.SendPacket(kPacket);
}
	...
	if(GetGameServerCount() == 0)
	{
		...
		for(int i = 0 ; i < (serverCount-1) ; i++)
		{
			...
			serverList.push_back(serverInfo);
			  ...
			  BYTE byIP[4];
			if(serverInfo->serviceName.IsEmpty() && serverInfo->serviceIP.IsEmpty())
			{
				  byIP[0] = (BYTE)FIRST_IPADDRESS(serverPort);
				  byIP[1] = (BYTE)SECOND_IPADDRESS(serverPort);
				  byIP[2] = (BYTE)THIRD_IPADDRESS(serverPort);
				  byIP[3] = (BYTE)FOURTH_IPADDRESS(serverPort);
//				continue;
			}
	...
	serverInfo->serverState		= SERVERSTATE_ON;
...
void CZoneDialog::OnServerDisconnect(SP2Packet &rkPacket)
{
	...
	m_services[1].SetItemText(serverInfo->serverIndex, 1, userCount);
	if(serverInfo->servicePort != CCU_PORT)
	{
		m_services[1].SetItemText(serverInfo->serverIndex, 2, roomCount);
		m_services[1].SetItemText(serverInfo->serverIndex, 3, plazaCount);
		m_services[1].SetItemText(serverInfo->serverIndex, 4, battleRoomCount);
		m_services[1].SetItemText(serverInfo->serverIndex, 5, pingMS);
		m_services[1].SetItemText(serverInfo->serverIndex, 6, DBMS);
	}

	UpdateServer(serverInfo, SERVERSTATE_OFF);
	CreateServerDownLog(serverInfo);
 	CLS_MonitorDlg* dlg = (CLS_MonitorDlg*)AfxGetApp()->m_pMainWnd;
	dlg->OnGameServerDisconnect(serverIP, serverPort);
}
...
void CZoneDialog::OnUpdateClientVersion(SP2Packet &rkPacket)
{
	...
	SP2Packet kPacket(TOOL_SVR::MCTSC_S_UPDATE_CLIENT_VERSION);
	kPacket << serverIP << serverPort << version;

	CLS_MonitorDlg* dlg = (CLS_MonitorDlg*)AfxGetApp()->m_pMainWnd;
	dlg->OnMainServerAdmin(iClientNumber, kPacket);
}
	...
	case ADMINCOMMAND_MTOOL_AUTH :
		{
			...
			rkPacket >> iClientNumber >> result >> level;
			SP2Packet kPacket(TOOL_SVR::MSTMC_S_ADMIN_RESPONSE);
			kPacket << type << result << level;
	...
	case ADMINCOMMAND_MTOOL_USERBLOCK:
		{
			...
			ioHashString ioUserId;
			LONG success;
			rkPacket >> iClientNumber >> ioUserId >> success;
			SP2Packet kPacket(TOOL_SVR::MSTMC_S_ADMIN_RESPONSE);
			kPacket << type << ioUserId << success;
