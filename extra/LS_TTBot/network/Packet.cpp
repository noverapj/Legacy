...
CPacket::CPacket(const uint8 *buffer, uint32 size) 
{
	Clear();

	if( size >= sizeof(PACKETHEADER) )
	{
		memcpy( m_pBuffer , buffer, sizeof(PACKETHEADER) );
		m_currentPos = sizeof(PACKETHEADER);
	
		if( size >= GetBufferSize() )
		{
			if( GetBufferSize() < m_currentPos )
			{
				MessageBox(NULL, "CPacket error", "error", MB_OK );
				exit(1);
				return;
			}
...
const uint8* CPacket::GetBuffer() const
{
	return &m_pBuffer[0];
}

const uint8* CPacket::GetData() const
{
	return &m_pBuffer[0] + sizeof(PACKETHEADER);
}

uint32 CPacket::GetBufferSize() const
{
	return *m_packet_header.m_Size;
}

uint32 CPacket::GetDataSize() const
{
...
{	
	...
	{
		//LOG.PrintTimeAndLog( 0,"[ID:%d - SIZE:%d] << Packat Size Overflow!", GetPacketID(), m_currentPos + iAddSize );
		return false;
	}
