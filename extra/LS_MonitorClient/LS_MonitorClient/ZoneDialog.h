...
protected:
	...
	void CreatePanel();
	void CreatePopup();
	void CreateServerDownLog(SERVERINFO *serverInfo);
...
public:
	...
	void RequestPractice(CString publicID);
	...
	void RequestTodayCCU(WORD wYear, WORD wMonth, WORD wDay);
	void RequestSearchCCU(WORD wYear, WORD wMonth, WORD wDay);
...
protected:
	...
	void OnAnswerTodayCCU(SP2Packet &rkPacket);
	void OnAnswerSearchCCU(SP2Packet &rkPacket);
	...
	void OnConnectState(SP2Packet &rkPacket);
...
protected:
	...
	CString		m_MNTClientIP;
	
	int			m_MNTClientPort;
	
	CIOSocket	m_sockMNTClient;
