...
void CGraphDlg::Startup(CString title, CZoneDialog* pkZonedlg)
{
	m_title = title;
	m_pkZonedlg = pkZonedlg;
...
void CGraphDlg::SetXSearchUnits(CString string)
{
	m_graphCCU.SetXUnits(string);
}
...
void CGraphDlg::ProgressTime(MSG* pMsg)
{
	if(m_graphCCU.GetSafeHwnd() == pMsg->hwnd)
	{
		...
		int iNum = iHour*60 + iMinute;
		int iCount = 0;
		if(iNum < m_points.size())
		{
			iCount = m_points[iNum];
		}
		szTime.Format("Time:%02d:%02d\nUser:%05d", iHour, iMinute, iCount);

		m_graphCCU.SetToolTip(x, y, szTime );;
	}
}
...
void CGraphDlg::OnBnClickedToday()
{
	...
	m_PCCU = 0;
	m_bChangeGraph = true;
	m_pkZonedlg->RequestTodayCCU(sysTime.wYear, sysTime.wMonth, sysTime.wDay);
...
void CGraphDlg::OnBnClickedYesterday()
{
	...
	m_pkZonedlg->RequestSearchCCU(sysTime.wYear, sysTime.wMonth, sysTime.wDay);
	ComboBoxSetting(sysTime.wYear, sysTime.wMonth, sysTime.wDay);
}
...
void CGraphDlg::OnBnClickedSerach()
{
	...
	m_PCCU = 0;
	if(sysTime.wYear == iYear && sysTime.wMonth == iMonth && sysTime.wDay == iDay)
	{
		m_bChangeGraph = true;
		m_pkZonedlg->RequestTodayCCU(sysTime.wYear, sysTime.wMonth, sysTime.wDay);
	}
	...
	{
		m_bChangeGraph = false;
		m_pkZonedlg->RequestSearchCCU((WORD)iYear, (WORD)iMonth, (WORD)iDay);
	}
	ComboBoxSetting((WORD)iYear, (WORD)iMonth, (WORD)iDay);
}
