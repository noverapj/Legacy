...
void COScopeCtrl::SetToolTip(CString text)
{
...
/////////////////////////////////////////////////////////////////////////////
void COScopeCtrl::SetRange( bool b, double dLower, double dUpper, int nDecimalPlaces)
{
...
void COScopeCtrl::InvalidateCtrl()
{
	...
	strTemp.Format (_T("%d"), (int)m_dUpperLimit);
	m_dcGrid.TextOut (m_rectPlot.left, m_rectPlot.top, strTemp);
	...
	strTemp.Format (_T("%d"), (int)m_dUpperYLimit);
	m_dcGrid.TextOut (m_rectPlot.right, m_rectPlot.bottom+10, strTemp);

	// x max
	
	m_dcGrid.SetTextAlign (TA_RIGHT|TA_BOTTOM);
	strTemp.Format ("%d", (int)m_dLowerXLimit); 
	m_dcGrid.TextOut (m_rectPlot.left-4, m_rectPlot.bottom+10, strTemp);
...
void COScopeCtrl::InvalidateXUnits()
{
	// There is a lot of drawing going on here - particularly in terms of 
	// drawing the grid.  Don't panic, this is all being drawn (only once)
	// to a bitmap.  The result is then BitBlt'd to the control whenever needed.
...
/////////////////////////////////////////////////////////////////////////////
double COScopeCtrl::AppendPoint(bool bCCU, double dNewPoint)
{
...
void COScopeCtrl::AppendCCUPoint(double dNewGB,double dNewDE,double dNewFR,double dNewIT,double dNewPL,double dNewTR, double dNewETC )
{
...
int COScopeCtrl::GetRightPixel()
{
	return m_rectPlot.right;
}

int COScopeCtrl::GetLeftPixel()
{
