...
BOOL CNMSimpleStream::SetBuffer( BYTE* pBuf, BOOL bReference )
{
	...
	UINT32	uStartMarker	= *( reinterpret_cast<UINT32*>(pBuf) );

	CStreamCryptor*	pStreamCryptor;

	pStreamCryptor	= CStreamCryptorFactory::GetInstance().FindStreamCryptor( uStartMarker );

	if ( pStreamCryptor != NULL )
	{
		this->m_uBufLen			= *( reinterpret_cast<UINT32*>(pBuf + kStartMarkerSize) );
		UINT32	uEndMarker		= *( reinterpret_cast<UINT32*>(pBuf + kStartMarkerSize + kBufLenSize + this->m_uBufLen) );

		if ( uEndMarker != pStreamCryptor->GetEndMarker() )
			goto fail;
		...
		else
		{
			...
			::CopyMemory( this->m_pBuf, pBuf, this->m_uBufLen + kExtraSize );

			this->m_uBufMaxLen	= this->m_uBufLen + kExtraSize;
		}

		this->m_uBufPtr			= kStartMarkerSize + kBufLenSize;
		this->m_uStreamVersion	= pStreamCryptor->GetVersion();
...
void CNMSimpleStream::ResetPtr( BOOL bWrite )
{
	this->m_uBufPtr		= kStartMarkerSize + kBufLenSize;
...
BYTE* CNMSimpleStream::GetBufferPtr( void )
{
	...
	pStreamCryptor	= CStreamCryptorFactory::GetInstance().FindStreamCryptor( this->m_uStreamVersion );

	if ( pStreamCryptor != NULL )
	{
		*( reinterpret_cast<_UInt32_*>(this->m_pBuf) )						= pStreamCryptor->GetStartMarker();
		*( reinterpret_cast<_UInt32_*>(this->m_pBuf + kStartMarkerSize) )	= this->m_uBufLen;
		*( reinterpret_cast<_UInt32_*>(this->m_pBuf + kStartMarkerSize + kBufLenSize + this->m_uBufLen) )
																			= pStreamCryptor->GetEndMarker();
	}
...
_UInt8_ CNMSimpleStream::GetLatestCryptorVersion( void )
{
	return CStreamCryptorFactory::GetInstance().GetLatestVersion();
}

BOOL CNMSimpleStream::Encrypt( _UInt8_ uStreamVersion )
{
	this->m_uStreamVersion	= uStreamVersion;
	...
	if ( pStreamCryptor != NULL )
	{
		if ( this->RequestBuffer( pStreamCryptor->GetEncSize( this->m_uBufLen ) - this->m_uBufLen ) )
		{
			if ( CStreamCryptor::kError_None == pStreamCryptor->Encrypt( this->m_pBuf + kStartMarkerSize + kBufLenSize, this->m_uBufMaxLen, &this->m_uBufLen ) )
			{
				this->m_uBufPtr	= kStartMarkerSize + kBufLenSize + this->m_uBufLen;
...
BOOL CNMSimpleStream::Decrypt( void )
{
	if ( this->m_uStreamVersion == kCryptor_NotUsed )
		return TRUE;
	...
	if ( pStreamCryptor != NULL )
	{
		if ( this->m_bReference )
		{
			BYTE*		pNewBuf;
			_UInt32_	uNewBufLen;

			uNewBufLen	= GET_BLOCK_SIZE( this->GetBufferSize() );
			pNewBuf		= (BYTE*) _NMHeapAlloc( uNewBufLen );

			if ( pNewBuf == NULL )
				return FALSE;

			::CopyMemory( pNewBuf, this->m_pBuf, this->GetBufferSize() );
		...
		if ( CStreamCryptor::kError_None == pStreamCryptor->Decrypt( this->m_pBuf + kStartMarkerSize + kBufLenSize, this->m_uBufMaxLen, &this->m_uBufLen ) )
		{
			return TRUE;
		}
	}
...
void CNMSimpleStreamEncoder::EncodeString( LPCWSTR wszValue )
{
	if ( this->m_pStream )
	{
		UINT32 uLen = static_cast< UINT32 >( wszValue ? wcslen( wszValue ) : 0 );

		this->EncodeLen( uLen );

		if ( 0 < uLen && this->m_pStream->RequestBuffer( uLen << 1 ) )
		{
			::CopyMemory( this->m_pStream->m_pBuf + this->m_pStream->m_uBufPtr, wszValue, uLen << 1 );
...
void CNMSimpleStreamEncoder::EncodeString( LPCSTR aszValue )
{
	UINT32	uLen	= static_cast< UINT32 >( aszValue ? strlen( aszValue ) : 0 );
	WCHAR*	wszTemp	= new WCHAR[ uLen + 1 ];

	::MultiByteToWideChar( m_pStream->GetCodePage(), 0, aszValue, -1, wszTemp, uLen + 1 );
	wszTemp[ uLen ] = NULL;

	this->EncodeString( wszTemp );
...
_LPCSTR_ CNMSimpleStreamDecoder::DecodeStringA( void )
{
	...
	if ( szTemp )
	{
		...
		::WideCharToMultiByte( m_pStream->GetCodePage(), 0, szTemp, -1, this->m_pTempAscStr, ( uLen + 1 ) << 1, NULL, NULL );
...
_LPCSTR_ CNMSimpleStreamDecoder::DecodeStringA( _LPSTR_ pOutBuf, _UInt32_ uMaxStrLen )
{
	...
	if ( szTemp )
	{
		...
		::WideCharToMultiByte( m_pStream->GetCodePage(), 0, szTemp, -1, pOutBuf, ( uMaxStrLen == 0 ? ( uLen << 1 ) + 1 : uMaxStrLen ), NULL, NULL );
...
CNMSerializableDecoder::CNMSerializableDecoder( CNMSimpleStream& ssStream , CNMSerializable* pSerializable) : CNMSimpleStreamDecoder( ssStream )
{
	...
	if ( ( uObjectHeader >> 16 ) == pSerializable->m_uObjectMainCode )
	{
		...
		pSerializable->m_uSerializerVersion	= this->m_pStream->GetStreamVersion();
	}
	else
	{
