...
protected:
	...
	_UInt32_			m_uBufPtr;
	BOOL				m_bIsValid;
	_UInt8_				m_uStreamVersion;
	_UInt32_			m_uCodePage;
...
public:
	...
	void				ResetPtr( BOOL bWrite = FALSE );
	BYTE*				GetBufferPtr( void );
	_UInt32_			GetBufferSize( void );

	_UInt8_				GetStreamVersion( void ) { return this->m_uStreamVersion; }
	void				SetStreamVersion( _UInt8_ uStreamVersion ) { this->m_uStreamVersion = uStreamVersion; }

	_UInt32_			GetCodePage( void ) { return this->m_uCodePage; }
	void				SetCodePage( _UInt32_ uCodePage ) { this->m_uCodePage = uCodePage; }

	_UInt8_				GetLatestCryptorVersion( void );

	BOOL				Encrypt( _UInt8_ uStreamVersion );
	BOOL				Decrypt( void );
