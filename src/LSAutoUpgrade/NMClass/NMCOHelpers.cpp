...
,	LPCTSTR		szPosition			// = NULL, 128
)
{
	CNMExecuteNGMPatcherFunc	func( uGameCode, szUrl, szPatchDir, szExecCmdLine, bWaitParentExit, bSmallDialog, bUseLocalNGM, szSessionID, uPatchType, szPosition );
	return CNMManager::GetInstance().CallNMFunc( &func );
}
...
{
	CNMExecuteNGMLauncherFunc	func( uGameCode, uLaunchType, szArgument, bWaitParent, szPassport, bUseLocalNGM, szStickPath, szPosition );
	return		CNMManager::GetInstance().CallNMFunc( &func );
}
...
{
	CNMExecuteNGMInstallerFunc	func( uGameCode, bWaitParent, bLaunchAfterInstall, szArgument, szPassport, szSessionID, szPosition );
	return		CNMManager::GetInstance().CallNMFunc( &func );
}
