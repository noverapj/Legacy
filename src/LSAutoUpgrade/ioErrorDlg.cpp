...
void ioErrorDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_X_BTN, m_XBtn);
...
BEGIN_MESSAGE_MAP(ioErrorDlg, CDialogSK)
	...
	ON_BN_CLICKED(IDC_X_BTN, OnBnClickedXBtn)
...
BOOL ioErrorDlg::OnInitDialog()
{
	...
	int iIDErrorBack = IDB_ERROR_BACK;
	int iIDSolution  = IDB_SOLUTIONBTN;
	int iIDExit      = IDB_EXITBTN;
	ioLocalParent *pLocal = g_LocalMgr.GetLocal( ioLocalManager::GetLocalType() );
	if( pLocal )
	{
		iIDErrorBack = pLocal->GetResourceIDErrorBackBtnBmp();
		iIDSolution  = pLocal->GetResourceIDErrorSolutionBtnBmp();
		iIDExit      = pLocal->GetResourceIDErrorExitBtnBmp();
	}

	// skin
	EnableEasyMove();                    // enable moving of the dialog by clicking anywhere in the dialog
	SetBitmap(iIDErrorBack);           // set background bitmap
	...
	m_XBtn.LoadBitmap(IDB_ERROR_XBTN);
	...
	{
		m_SolutionBtn.LoadBitmap(iIDSolution);
		m_SolutionBtn.MoveWindow(17, 252, 286, 31);
	}


	m_ExitBtn.LoadBitmap(iIDExit);
...
void ioErrorDlg::ShowErrorDlg( CString szErrorText, int iErrorNumber, bool bExitProgram )
{
	if( IsWindowVisible() )
		return;

	m_bExitProgram = bExitProgram;
	m_iErrorNumber = iErrorNumber;

    CSystemInfo kSystemInfo;
	kSystemInfo.SetupSystemInfo();

	static char szEditText[MAX_PATH*8]="";
	ZeroMemory( szEditText, sizeof( szEditText ) );
	SafeSprintf( szEditText, sizeof(szEditText), STR(1), STRFILEVER,(LPCTSTR)kSystemInfo.GetSystemInfo(),(LPCTSTR)szErrorText );
