...
: ioUDPSocket()
{
	m_pNS = new ioUDPClientSecurity; // 무조건 암호처리함.
}
...
bool ioClientUDPSocket::CheckNS( CPacket &rkPacket )
{
	...
	{
		LOG.PrintTimeAndLog( 0, "ioClientUDPSocket::CheckNS Type:%d State Not Same Client:%d, Server:%d", 
							     rkPacket.GetPacketID(), rkPacket.GetState(), m_pNS->GetRcvState() );
		return false;
	}
...
{
	StringCbPrintf( m_recent_rcv_ip, sizeof(m_recent_rcv_ip), iip );
...
bool ioUDPClientSecurity::CheckState( CPacket &rkPacket )
{
	if( rkPacket.GetState() != MAGIC_TOKEN_FSM )
	{
		...
		StringCbCopy(m_pre_recent_rcv_ip, sizeof(m_pre_recent_rcv_ip), m_recent_rcv_ip);
