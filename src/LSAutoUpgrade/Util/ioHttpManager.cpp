#include "../stdafx.h"
#include ".\ioHttpManager.h"
#include <wininet.h>

template<> ioHttpManager* Singleton< ioHttpManager >::ms_Singleton = 0;

ioHttpManager::ioHttpManager(void)
{
}
...
ioHttpManager & ioHttpManager::GetSingleton()
{
	return Singleton< ioHttpManager >::GetSingleton();
}

//주의. 싱말용 인자값이 추가되어 있습니다.
bool ioHttpManager::SendGetData( IN const char *szURL, IN const char *szGetData, IN bool bHttps /*= true */ )
{
	if( szURL == NULL )
	{
		return false;
	...
	CString szSendData = kUrlcomponent.lpszUrlPath;
	//싱말용 인자값 추가
	szSendData += "/ls_city.php?cip=";
	szSendData += szGetData;
	hRequest = HttpOpenRequest( hConnect, NULL, szSendData, NULL, NULL, NULL, iSecureFlags , NULL ); //park test
	if( hRequest == NULL )
	{
		InternetCloseHandle( hInternet );
	...
	if( HttpSendRequest( hRequest, szHeader, strlen( szHeader ), (LPVOID)szGetData, strlen( szGetData ) ) == FALSE )
	{
		InternetCloseHandle( hInternet );
		InternetCloseHandle( hConnect );
		InternetCloseHandle( hRequest );
		TRACE("%s Error HttpSendRequest. (%s:%s) (%x)" , __FUNCTION__ , szURL, szGetData, GetLastError() );
		return false;
	}
...
bool ioHttpManager::GetResultGetData( IN const char *szURL, OUT char *szReturnData, IN int iReturnDataSize )
{
...
bool ioHttpManager::GetResultPostData( IN const char *szURL, IN const char *szPostData, OUT char *szReturnData, IN int iReturnSize, IN bool bHttps /*= true */ )
{
	if( szURL == NULL )
	{
