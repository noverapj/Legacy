...
const char * ioLocalBrazil::GetMemTextList()
{
...
DWORD ioLocalBrazil::GetResourceIDBackBmp()
{
	return IDB_BACK_US;
}

DWORD ioLocalBrazil::GetResourceIDOptimizeBtnBmp()
{
	return IDB_OPTIMIZE_BTN_US;
}

DWORD ioLocalBrazil::GetResourceIDStartBtnBmp()
{
	return IDB_START_US;
}

DWORD ioLocalBrazil::GetResourceIDErrorBackBtnBmp()
{
	return IDB_ERROR_BACK_US;
}

DWORD ioLocalBrazil::GetResourceIDErrorSolutionBtnBmp()
{
	return IDB_SOLUTIONBTN_US;  
}

DWORD ioLocalBrazil::GetResourceIDErrorExitBtnBmp()
{
	return IDB_EXITBTN_US;
}

const char * ioLocalBrazil::GetRegKey()
{
	return "Software\\SP2ClientBrazil\\";
}

const char * ioLocalBrazil::GetFontName()
{
	return "Tahoma";
}

void ioLocalBrazil::CreateShortcuts( const CString &rszRootDir, const CString &rszShortcutName, const CString &rszLoginURL )
{
...
const char * ioLocalBrazil::GetErrorSolutionURL()
{
	return "http://support.z8games.com/lostsaga/";
}

bool ioLocalBrazil::GetNewCmdLine( IN const char *szCmd, OUT char *szNewCmd, IN int iNewCmdSize )
{
...
bool ioLocalBrazil::GetLoginResultPostData( OUT char *szError, IN int iErrorSize, const char *szURL, const char *szID, const char *szPW )
{
	return true;
}

int ioLocalBrazil::GetLoginKeyLiveTime()
{
	return ( LOGIN_KEY_LIVE_TIME*100 ); // 서버에서 로그인을 하므로 로그인키 생존 시간이 필요 없기때문에 아주길게 설정
}

void ioLocalBrazil::SetLoginData( const char *szLoginID, const char *szLoginEncodePW )
{
	if( szLoginID == NULL )
		return;
...
void ioLocalBrazil::SetServerID( const char *szServerID )
{
	if( szServerID == NULL )
		return;
...
bool ioLocalBrazil::IsRightID( const char *szID )
{
	enum { MIN_LENGTH = 3,  };
...
bool ioLocalBrazil::IsRightPW( const char *szPW )
{
	enum { MIN_LENGTH = 3,  };
...
const char * ioLocalBrazil::GetConnectFailMent()
{
...
const char * ioLocalBrazil::GetLoginWrongIDMent()
{
	return "Wrong ID";
}

const char * ioLocalBrazil::GetLoginWrongPWMent()
{
	return "Wrong Password";
}

const char * ioLocalBrazil::GetServerFullMent()
{
	return "Game Server is full.\r\n(Error Code:228)";
}

const char * ioLocalBrazil::GetEntryURL()
{
	return "http://lostsaga.z8games.com/landing/";
}

const char * ioLocalBrazil::GetFindIDURL()
{
	return "http://www.z8games.com/lost_id_password.aspx";
}

const char *ioLocalBrazil::GetLoginDlgURL()
{
	return "http://images.z8games.com/lsna/launcher/img/launcher_main.jpg";
}

const char * ioLocalBrazil::GetFindPWURL()
{
	return ""; // facebook
}

void ioLocalBrazil::SetUserType( const char *szUserType )
{
	StringCbCopy( m_szUserType, sizeof( m_szUserType ), szUserType );
}

void ioLocalBrazil::FillLoginData( OUT SP2Packet &rkPacket )
{
	rkPacket << m_szUserType;
}


void ioLocalBrazil::OnFacebookData( const string &rsEncryptData )
{
...
#ifdef STEAM_ATTACH
int ioLocalBrazil::UpdateNRunSteamLauncher( HTTPManager* HttpMgr)
{
	...
	char szFileLink[MAX_PATH]="";
	kIniLoder.LoadString("launcher", "", szFileLink, sizeof(szFileLink));

	CString strSteamFileURL = szFileLink; //ini
...
bool ioLocalBrazil::CheckFileInFolder( CString* const strPath, CString* const strFile )
{
	CFileFind pFind;
...
bool ioLocalBrazil::DownloadLauncher( const CString & strLauncherURL, HTTPManager* HttpMgr )
{
...
bool ioLocalBrazil::IsLoginDataValidate()
{
...
void ioLocalBrazil::GetWorkingFolderPath( OUT CString& strPath )
{
