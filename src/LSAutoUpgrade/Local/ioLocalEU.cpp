...
ioLocalEU::ioLocalEU(void)
{
	...
	ZeroMemory( m_szNexonPassport, sizeof( m_szNexonPassport ) );
}
...
void ioLocalEU::RegisterCallBack(HWND hWnd)
{
#ifdef SRC_EU
	LoadIt();
	_nmman.Init();
	...
	bool bResult = false;
	bResult = _nmco.SetLocaleAndRegion( kLocaleID_EU, kRegionCode_EU );
	
	if(!bResult)
		LOG.PrintTimeAndLog(0, "Nexon Set Locale fail");
		
	if ( !_nmco.Initialize( kGameCode_lostsaga_eu ) )
	{
		LOG.PrintTimeAndLog(0, "Nexon Initializing fail");
	}

	if ( !_nmco.RegisterCallbackMessage( hWnd, WM_MESSENGER_NOTIFY ) )
	{
		LOG.PrintTimeAndLog(0, "Nexon Register fail");
	}	
#endif
}
...
const char * ioLocalEU::GetMemTextList()
{
	return "|EXE_CAutoUpgradeApp::IsDuplication_1|\r\n\r\n      [Lost Saga] is running.\r\n\r\n      Close [Lost Saga] and try again.\r\n\r\n\r\n      (error code:201)|"
		...
		"|EXE_ioLocalEU::GetLoginResultPostData_1|%s|"
		"|EXE_ioLocalEU::GetLoginResultPostData_2|Unknown Country Code: %d %d.|"
		"|EXE_ioLocalEU::GetLoginResultPostData_3|Unknown Error Code: %d %d.|"
		"|EXE_ioLocalEU::GetLoginResultPostData_4|XML Load Fail. Error Code : %d %d.|";
}
...
DWORD ioLocalEU::GetResourceIDBackBmp()
{
	return IDB_BACK_US;
}

DWORD ioLocalEU::GetResourceIDOptimizeBtnBmp()
{
	return IDB_OPTIMIZE_BTN_US;
}

DWORD ioLocalEU::GetResourceIDStartBtnBmp()
{
	return IDB_START_US;
}

DWORD ioLocalEU::GetResourceIDErrorBackBtnBmp()
{
	return IDB_ERROR_BACK_US;
}

DWORD ioLocalEU::GetResourceIDErrorSolutionBtnBmp()
{
	return IDB_SOLUTIONBTN_US;  
}

DWORD ioLocalEU::GetResourceIDErrorExitBtnBmp()
{
...
void ioLocalEU::CreateShortcuts( const CString &rszRootDir, const CString &rszShortcutName, const CString &rszLoginURL )
{
...
{
	return "http://support.lostsaga.nexoneu.com";
}
...
bool ioLocalEU::GetNewCmdLine( IN const char *szCmd, OUT char *szNewCmd, IN int iNewCmdSize )
{
	...
	for (int i = 0; i < MAX_LOOP ; i++)
	{
		...
		if( i == TYPE_EXPAND_KEY )
		{
			...
			StringCbCat( szNewCmd, iNewCmdSize, m_szNexonPassport );
...
bool ioLocalEU::GetLoginResultPostData( OUT char *szError, IN int iErrorSize, const char *szURL, const char *szID, const char *szPW )
{
	...
	CAtlString strNexonID;
	strNexonID.SetString(szID);
	CAtlString strPassword;
	strPassword.SetString(szPW);

	NMLoginAuthReplyCode resultAuth = _nmco.LoginAuth( strNexonID, strPassword, 0, NULL, 0, &nSubErrorCode );

	if ( resultAuth == kLoginAuth_OK )
	{
		_nmco.GetNexonPassport( m_szNexonPassport );
		return true;
	}

	// 리소스에서 res\\NXEUErrorMessage.xml 를 읽어들임
	HMODULE hModule = ::GetModuleHandle( NULL );
	HRSRC hSrc		= ::FindResource( hModule, MAKEINTRESOURCE( IDR_EU_ERRMSG_XML ), _T( "XML" ) );
	HGLOBAL hGlobal	= ::LoadResource( hModule, hSrc );

	char *chXML = static_cast<char*>( ::LockResource( hGlobal ) );
	::FreeResource( hGlobal );
	
	// 넥슨 로그인 인증 에러코드 파싱	JCLEE 130326
	ioXMLDocument xmlDoc;
	if( xmlDoc.LoadFromMemory( chXML ) == false )
	{
		// xml load error
		return false;	
	}
	else
	{
		...
		ioXMLElement xChildElement1 = xRootElement.FirstChild( "Language" );
		int iLocalType				= ioLocalManager::GetLocalType() - ioLocalManager::LCT_EU;

		enum{ MAX_EU_COUNTRY = 6, MAX_SIZE = 3, }; 		
		char chLocal[MAX_EU_COUNTRY][MAX_SIZE] = { "EN", "DE", "FR", "IT", "PL", "TR" };

		for( int i = 0; i < xRootElement.GetChildCount(); ++i )
		{			
			CString szValueXML = xChildElement1.GetStringAttribute( "value" );			

			// 국가 타입 체크
			if( szValueXML == chLocal[iLocalType] )
			{
				ioXMLElement xChildElement2 = xChildElement1.FirstChild( "Error" );

				for( int j = 0; j < xChildElement1.GetChildCount(); ++j )
				{
					CString szValue2	= xChildElement2.GetStringAttribute( "code" );
					int iErrXML			= atoi( szValue2 );

					// 에러 코드 체크
					if( nSubErrorCode == iErrXML )
					{
						char chTemp[MAX_PATH*4] = {0,};
						CString szValue3 = xChildElement2.GetText();
						sprintf_s( chTemp, "%s", szValue3 );						
						
						// 앞뒤 공간 날리기
						szValue3 = Help::Trim(chTemp);					

						sprintf( szError, szValue3 );					

						break;
					}
					else
					{
						xChildElement2 = xChildElement2.NextSibling();
					}

					// 에러 코드에 없는 내용
					if( j == xChildElement1.GetChildCount() - 1 )
						sprintf( szError, STR(3), resultAuth, nSubErrorCode );
				}
				break;				
			}
			else
			{
				xChildElement1 = xChildElement1.NextSibling();
			}

			// 국가가 없음
			if( i == xRootElement.GetChildCount() - 1 )
				sprintf( szError, STR(2), resultAuth, nSubErrorCode );
		}
	}
...
int ioLocalEU::GetLoginKeyLiveTime()
{
	return ( LOGIN_KEY_LIVE_TIME*100 ); // 서버에서 로그인을 하므로 로그인키 생존 시간이 필요 없기때문에 아주길게 설정
}

void ioLocalEU::SetLoginData( const char *szLoginID, const char *szLoginEncodePW )
{
	if( szLoginID == NULL )
		return;
...
void ioLocalEU::SetServerID( const char *szServerID )
{
	if( szServerID == NULL )
		return;
...
bool ioLocalEU::IsRightID( const char *szID )
{
	enum { MIN_LENGTH = 3,  };
...
bool ioLocalEU::IsRightPW( const char *szPW )
{
	enum { MIN_LENGTH = 3,  };
...
const char * ioLocalEU::GetConnectFailMent()
{
...
const char * ioLocalEU::GetLoginWrongIDMent()
{
	return "Wrong ID";
}

const char * ioLocalEU::GetLoginWrongPWMent()
{
	return "Wrong Password";
}

const char * ioLocalEU::GetServerFullMent()
{
	return "Game Server is full.\r\n(Error Code:228)";
}

const char * ioLocalEU::GetEntryURL()
{
	switch(m_nSubType)
	{
	case 0:
		return "http://en.lostsaga.nexoneu.com";
	case 1:
		return "http://de.lostsaga.nexoneu.com";
	case 2:
		return "http://fr.lostsaga.nexoneu.com";
	case 3:
		return "http://it.lostsaga.nexoneu.com";
	case 4:
		return "http://pl.lostsaga.nexoneu.com";
	case 5:
		return "http://tr.lostsaga.nexoneu.com";
	default:
		return "http://en.lostsaga.nexoneu.com";
	}
}

const char * ioLocalEU::GetFindIDURL()
{
	return "https://passport.nexoneu.com/User/FindID.aspx?nexoneuTheme=lostsaga";
}

const char *ioLocalEU::GetLoginDlgURL()
{
	return "http://images.z8games.com/lsna/launcher/img/launcher_main.jpg";
}

const char * ioLocalEU::GetFindPWURL()
{
	return "https://passport.nexoneu.com/User/FindPassword.aspx?nexoneuTheme=lostsaga"; 
}

void ioLocalEU::SetUserType( const char *szUserType )
{
	StringCbCopy( m_szUserType, sizeof( m_szUserType ), m_szNexonPassport );
}

void ioLocalEU::FillLoginData( OUT SP2Packet &rkPacket )
{
	rkPacket << m_szNexonPassport;
}


void ioLocalEU::OnFacebookData( const string &rsEncryptData )
{
