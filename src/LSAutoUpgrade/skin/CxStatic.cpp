...
void CxStatic::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CCustomMemDC MemDC(pDC, &rect, m_bTransparentBk);
...
#define NMAP_STYLE(src, dest) if(!(dwStyle & (src))) dwFlags |= (dest)
	...
	for (int j = 0; j < nLine; j++){
		newRC.top = pRect->top + ( tag.tmHeight * j ) + ( m_iLineOffset * j) + m_iTopOffset ;
...
// 		SRCCOPY);
 	...
 	pDCMem->TransparentBlt(rect.left, rect.top, rect.Width(), rect.Height(), &dcMem, 
 			0, 0, m_bmInfo.bmWidth, m_bmInfo.bmHeight, RGB(0, 255, 0));
