...
void ioProgressBar::OnPaint()
{
	...
	{
		DrawBitmap( &dc, &m_bmpBar, m_iWidthBack, m_iHeightBack, 1.0f ); // 1.0f : 바탕은 항상 풀이미지를 그린다.
		DrawBitmap( &dc, &m_bmpBarOptimize, m_iWidth, m_iHeight, m_fProgressRate );
	}
	
}


void ioProgressBar::SetBitmap( UINT nIDResource, UINT nIDResourceBack, UINT nIDResourceOptimize )
{
	// bar
	m_bmpBar.DeleteObject();
	...
	// optimize img
	m_bmpBarOptimize.DeleteObject();
	m_bmpBarOptimize.LoadBitmap(nIDResourceOptimize);
	m_bmpBarOptimize.GetBitmap(&kBitMap);
...
void ioProgressBar::ChangeProgressBmp() //최적화 이미지를 위하여 이미지 변경함다.
{
	m_bOptimize = true;
}
