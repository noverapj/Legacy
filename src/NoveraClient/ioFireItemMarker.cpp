...
void ioFireItemMarker::UpdateSpecialAimState( ioPlayStage *pStage )
{
	if( !pStage ) return;
	...
	if( m_SpecailID == -1 ) return;

	ioMapEffect *pEff = pStage->FindMapEffect( m_SpecailID );
	if( pEff )
	{
		pEff->SetWorldPosition( vPos );
