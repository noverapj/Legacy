...
ioJoroExpansionItem::ioJoroExpansionItem()
{
...
ioJoroExpansionItem::ioJoroExpansionItem( const ioJoroExpansionItem &rhs )
	: ioWeaponItem( rhs ),
	...
	m_stGaugeDecEffect( rhs.m_stGaugeDecEffect )
{
	m_ChargeState = CS_NONE;
	m_dwAttackStartTime = 0;
	m_iCurCombo = 0;
...
void ioJoroExpansionItem::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_fNeedExtraGauge = rkLoader.LoadFloat( "extra_need_gauge", FLOAT10 );

	rkLoader.LoadString_e( "gauge_dec_effect", "", szBuf, MAX_PATH );
	m_stGaugeDecEffect = szBuf;
}

ioItem* ioJoroExpansionItem::Clone()
{
	return new ioJoroExpansionItem( *this );
}

ioWeaponItem::WeaponSubType ioJoroExpansionItem::GetSubType() const
{
	return WST_JORO_EXPANSION_ITEM;
}

void ioJoroExpansionItem::OnReleased( ioBaseChar *pOwner )
{
...
void ioJoroExpansionItem::SetNormalAttack( ioBaseChar *pOwner, int iCurCombo )
{
	ClearCancelInfo();
...
void ioJoroExpansionItem::CheckNormalAttack( ioBaseChar *pOwner )
{
...
void ioJoroExpansionItem::OnCharging( ioBaseChar *pOwner )
{
...
void ioJoroExpansionItem::ChangeToAttackFire( ioBaseChar *pOwner, bool bCharged )
{
	if( !pOwner ) return;
...
void ioJoroExpansionItem::OnItemGaugeOK( ioBaseChar *pOwner, SP2Packet &rkPacket )
{
	if( !pOwner ) return;
...
void ioJoroExpansionItem::OnItemGaugeCancel( ioBaseChar *pOwner, SP2Packet &rkPacket )
{
	if( !pOwner ) 
		return;
...
void ioJoroExpansionItem::SetExtraInfoToWeapon( ioWeapon *pWeapon, bool bJumpAttack )
{
...
bool ioJoroExpansionItem::IsChargeAttackState( ioBaseChar *pOwner )
{
...
bool ioJoroExpansionItem::IsEnableAttackDashCheck( ioBaseChar *pOwner, bool bCheck )
{
...
bool ioJoroExpansionItem::SetDodgeDefenseState( ioBaseChar *pOwner )
{
...
void ioJoroExpansionItem::ApplyDodgeDefenseState( ioBaseChar *pOwner, SP2Packet &rkPacket )
{
	if( !pOwner ) return;
...
void ioJoroExpansionItem::CheckDodgeDefenseState( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioJoroExpansionItem::CheckReserve( ioBaseChar *pOwner )
{
...
bool ioJoroExpansionItem::ProcessCancel( ioBaseChar *pOwner )
{
	...
	while( iter != vTimeList.end() )
	{
		...
		if( m_dwCancelCheckTime > 0 && COMPARE( dwTime, m_dwCancelCheckTime, dwCurTime ) )
		{
			...
			{
				//SetCurBullet( GetCurBullet() - GetNeedBullet() );
				DecreaseGauge( pOwner );
...
bool ioJoroExpansionItem::SetJumpAttack( ioBaseChar *pOwner, int iAniID, float fTimeRate, AttackType eType )
{
	if( !pOwner ) return false;
	...
	if( !pAttr )
	{
		LOG.PrintTimeAndLog( 0, "ioJoroExpansionItem::SetJumpAttack - AttackAttribute Not Exist" );
		return false;
	}
	...
	if( iAniID == -1 )
	{
		LOG.PrintTimeAndLog( 0, "ioJoroExpansionItem::SetJumpAttack - JumpAttack Animation Not Exist" );
		return false;
	}
...
void ioJoroExpansionItem::ProcessJumpAttack( ioBaseChar *pOwner, float fHeightGap )
{
	if( m_pExtendJump )
	{
...
void ioJoroExpansionItem::SetUseExtraGauge( float fAmt )
{
	m_fCurExtraGauge -= fAmt;
	m_fCurExtraGauge = max( 0.0f, m_fCurExtraGauge );
}

int ioJoroExpansionItem::GetMaxBullet()
{
	return (int)m_fMaxExtraGauge;
}

int ioJoroExpansionItem::GetCurBullet()
{
...
void ioJoroExpansionItem::SetCurBullet( int iCurBullet )
{
	if( iCurBullet < 0 )
		return;
...
int ioJoroExpansionItem::GetNeedBullet()
{
	return (int)m_fNeedExtraGauge;
}

void ioJoroExpansionItem::UpdateExtraData( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
bool ioJoroExpansionItem::IsEnableGauge()
{
	if( GetCurBullet() >= GetNeedBullet() )
		return true;
...
void ioJoroExpansionItem::DecreaseGauge( ioBaseChar *pOwner )
{
	m_fCurExtraGauge = max( 0.f, m_fCurExtraGauge - m_fNeedExtraGauge );
	if( pOwner->IsNeedProcess() )
		SetGaugeDecEffect( pOwner );
}


void ioJoroExpansionItem::SetGaugeDecEffect( ioBaseChar *pOwner )
{
	pOwner->AttachEffect( m_stGaugeDecEffect );
	if( pOwner->IsNeedProcess() )
	{
