...
ActionStopType ioMineWeapon6::m_ActionStopType = AST_NONE;
ActionStopType ioMineWeapon6::m_DieActionStopType = AST_NONE;

ioMineWeapon6::ioMineWeapon6( ioBaseChar *pOwner, const WeaponAttribute *pAttr )
...
ioMineWeapon6::~ioMineWeapon6()
{
	if( m_pMine )
	{
		LOG.PrintTimeAndLog( 0, "ioMineWeapon6::~ioMineWeapon6 - UnReleased Resource" );
	}
	...
	if( pOwner )
	{
		...
		if( pStage )
			pStage->DestroyMapEffect( m_MineEffectID );
	}
}

void ioMineWeapon6::LoadProperty( ioINILoader &rkLoader )
{
	...
	//////////////////////////////////////////////////////////////////////////
	m_nNeedItemGauge = rkLoader.LoadInt( "need_item_gauge",  1 );
	m_nExplosionWeaponIndex = rkLoader.LoadInt( "explosion_weapon_index", -1);
}

void ioMineWeapon6::SetTeam( ioPlayStage *pStage, TeamType eType )
{
	if( !pStage ) return;
...
void ioMineWeapon6::SetStartPosition( const D3DXVECTOR3 &vPos, ioPlayStage *pStage )
{
	if( !pStage )
	{
		SetPosition( vPos );
	...
	*/
	...
	SetNetSyncTime( FRAMEGETTIME() );
}

void ioMineWeapon6::Process( float fTimePerSec, ioPlayStage *pStage )
{
	switch( m_State )
	{
	case MS_CHARGE:
...
void ioMineWeapon6::OnCharge( ioPlayStage *pStage )
{
	...
	else
	{
		...
		ioHashString szMineEffect;
		if( m_CurTeamColor == TEAM_BLUE )
			szMineEffect = m_BlueMineEffect;
		else
			szMineEffect = m_RedMineEffect;

		if( !szMineEffect.IsEmpty() )
		{
			ioMapEffect* pEffect = pStage->CreateMapEffect( szMineEffect, GetPosition(), GetScale() );
			if( pEffect )
				m_MineEffectID = pEffect->GetUniqueID();
		}
	}
}

void ioMineWeapon6::OnSensing( ioPlayStage *pStage, float fTimePerSec )
{
	VisibleCheck( pStage );
	...
	// Mine Check
	/*if( pStage->CheckSensingMine6( this, m_fMineSensingRange ) )
	{
	SetWaitExplosionMine( pStage, true );
	}
	else*/
...
void ioMineWeapon6::VisibleCheck( ioPlayStage *pStage )
{
	if( !m_pMine )	return;

	ioMapEffect* pMapEffect = pStage->FindMapEffect( m_MineEffectID );
	if( !pMapEffect )
		return;
	...
	if( pTarget )
	{
		...
		else
		{
			...
			pMapEffect->SetVisible( m_bMineVisible );
		}
	}
	else
	{
...
bool ioMineWeapon6::DropZoneCheck( ioPlayStage *pStage )
{	
	D3DXVECTOR3 vCurPos = GetPosition();
...
void ioMineWeapon6::UpdateWorldCollisionBox()
{
...
void ioMineWeapon6::OnWaitExplosion( ioPlayStage *pStage )
{		
...
void ioMineWeapon6::SetWaitExplosionMine( ioPlayStage *pStage, bool bSendNetwork )
{
	...
	if( !m_WaitEffect.IsEmpty() )
	{
		pStage->CreateMapEffect( m_WaitEffect,
			GetPosition(),
			GetScale() );
...
void ioMineWeapon6::SetNormalDeadMine( ioPlayStage *pStage, bool bSendNetwork )
{
	if( m_State == MS_WAIT_EXPLOSION ||
		m_State == MS_EXPLOSION ||
		m_State == MS_NORMAL_DEAD ||
		m_State == MS_SILENCE_DEAD )
		return;
...
void ioMineWeapon6::SetSilenceDeadMine( ioPlayStage *pStage, bool bSendNetwork )
{
	if( m_State == MS_WAIT_EXPLOSION ||
		m_State == MS_EXPLOSION ||
		m_State == MS_NORMAL_DEAD ||
...
void ioMineWeapon6::ExplosionMine( ioPlayStage *pStage, bool bWait )
{
	...
	kFireTime.iAttributeIdx = m_nExplosionWeaponIndex;
...
void ioMineWeapon6::NormalDeadMine( ioPlayStage *pStage, bool bWait )
{
	if( !IsCanExplosion() )	return;
...
void ioMineWeapon6::TranslateByConveyer( const D3DXVECTOR3 &vMoveAmt )
{
...
bool ioMineWeapon6::IsCanMineExplosion()
{
...
bool ioMineWeapon6::IsCanExplosion() const
{
...
void ioMineWeapon6::OnExplosion( ioPlayStage *pStage )
{
	if( m_bWait )
	{
		m_bWait = false;
...
void ioMineWeapon6::OnNormalDead( ioPlayStage *pStage )
{
	if( m_bWait )
	{
		m_bWait = false;
...
int ioMineWeapon6::GetNeedGauge()
{
	if( m_State == MS_NORMAL_DEAD || m_State == MS_SILENCE_DEAD )
		return 0;

	return m_nNeedItemGauge;
}

void ioMineWeapon6::DestroyResource( ioPlayStage *pStage )
{
	if( m_pMine )
	{
...
const ioOrientBox& ioMineWeapon6::GetLocalAttackBox() const
{
	return m_LocalAttackBox;
}

ioWeapon::WeaponType ioMineWeapon6::GetType() const
{
	return WT_MINE6;
}

ActionStopType ioMineWeapon6::GetActionStopType() const
{
	return m_ActionStopType;
}

ActionStopType ioMineWeapon6::GetDieActionStopType() const
{
	return m_DieActionStopType;
}

bool ioMineWeapon6::IsCanVisible( ioWeapon *pWeapon ) const
{
...
bool ioMineWeapon6::IsCollision( ioPlayEntity *pEntity, bool bMapCollision )
{
	if( m_State != MS_EXPLOSION )
		return false;
...
DWORD ioMineWeapon6::GetSensingStartGapTime()
{
...
DWORD ioMineWeapon6::GetCollisionEnableTime() const
{
	if( m_State == MS_EXPLOSION )
...
const BuffInfoList& ioMineWeapon6::GetTargetAirBuffList() const
{
...
const BuffInfoList& ioMineWeapon6::GetTargetBuffList() const
{
	if( !m_bUseTeamTargetBuff )
...
const BuffInfoList& ioMineWeapon6::GetTargetDownBuffList() const
{
	if( !m_bUseTeamTargetBuff )
...
bool ioMineWeapon6::CheckEnableUpgradeState()
{
	if ( m_State == MS_CHARGE || m_State == MS_SENSING )
		return true;
	else
		return false;
}
