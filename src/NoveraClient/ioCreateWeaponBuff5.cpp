...
ioCreateWeaponBuff5::ioCreateWeaponBuff5()
{
	m_dwDelayEffectID = -1;
	m_dwCheckFireTime = 0;
...
ioCreateWeaponBuff5::ioCreateWeaponBuff5( const ioCreateWeaponBuff5 &rhs )
: ioBuff( rhs ),
...
m_szDelayEffectOther( rhs.m_szDelayEffectOther ),
m_szFireEffectOther( rhs.m_szFireEffectOther ),
...
void ioCreateWeaponBuff5::LoadProperty( ioINILoader &rkLoader )
{
	...
	rkLoader.LoadString_e( "delay_effect_other", "", szBuf, MAX_PATH );
	m_szDelayEffectOther = szBuf;
	if( m_szDelayEffectOther.IsEmpty() )
		m_szDelayEffectOther = m_szDelayEffect;
	rkLoader.LoadString_e( "attack_effect_other", "", szBuf, MAX_PATH );
	m_szFireEffectOther = szBuf;
	if( m_szFireEffectOther.IsEmpty() )
		m_szFireEffectOther = m_szFireEffect;
...
ioBuff* ioCreateWeaponBuff5::Clone()
{
	return new ioCreateWeaponBuff5( *this );
}

void ioCreateWeaponBuff5::StartBuff( ioBaseChar *pOwner )
{
...
bool ioCreateWeaponBuff5::ApplyStartBuff(ioBaseChar *pOwner, DWORD dwGlobalTime, DWORD dwGapTime )
{
...
void ioCreateWeaponBuff5::ProcessBuff( float fTimePerSec )
{
...
bool ioCreateWeaponBuff5::CheckOwnerStateCheck()
{
...
bool ioCreateWeaponBuff5::CheckRestoreStateCheck()
{
	if( !m_pOwner ) return false;
...
void ioCreateWeaponBuff5::EndBuff()
{
...
void ioCreateWeaponBuff5::GetBuffInfo( SP2Packet &rkPacket )
{
	ioBuff::GetBuffInfo( rkPacket );
	rkPacket << (int)m_BuffState;
}

void ioCreateWeaponBuff5::ApplyBuffInfo( SP2Packet &rkPacket )
{
...
void ioCreateWeaponBuff5::ProcessKeyInput()
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
...
void ioCreateWeaponBuff5::ProcessState()
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
	...
	case BS_DELAY:
		{
			if( m_dwDelayEffectID == -1 )
			{
				...
				if( pOwner->IsOwnerChar() )
					pEffect = pOwner->AttachEffect( m_szDelayEffect, NULL, !m_bVisibleOnlyEffect );			
				else
					pEffect = pOwner->AttachEffect( m_szDelayEffectOther, NULL, !m_bVisibleOnlyEffect );			
...
void ioCreateWeaponBuff5::CreateWeapon()
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
	...
	if( pOwner->IsOwnerChar() )
		...
		pOwner->AttachEffect( m_szFireEffectOther, NULL, !m_bVisibleOnlyEffect );
...
void ioCreateWeaponBuff5::ApplyExtraBuffInfo( SP2Packet &rkPacket )
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
