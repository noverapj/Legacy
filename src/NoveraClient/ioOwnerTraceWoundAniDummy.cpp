...
ioOwnerTraceWoundAniDummy::ioOwnerTraceWoundAniDummy( ioEntityGroup *pGrp, ioPlayMode *pMode ):
...
ioOwnerTraceWoundAniDummy::~ioOwnerTraceWoundAniDummy(void)
{
}

void ioOwnerTraceWoundAniDummy::LoadProperty( ioINILoader &rkLoader )
{
	char szBuf[MAX_PATH] = "";
	char szKey[MAX_PATH] = "";
	...
	m_szWoundAni	= szBuf;
	m_fWoundAniRate	= rkLoader.LoadFloat_e( "wound_ani_rate", 0.0f );
	...
	m_bSpecialAttack = rkLoader.LoadBool_e( "use_special_attack", false );

	wsprintf_e( szBuf, "dummy_special_attack_sub_type", i+1 );
	m_DummySpecialAttack.m_iSubUseType = rkLoader.LoadInt( szBuf, DSUT_NONE );
	LoadAttackAttribute( "dummy_special_attack", m_DummySpecialAttack.m_AttackAttribute, rkLoader );

	rkLoader.LoadString_e( "owner_effect", "", szBuf, MAX_PATH );
	m_OwnerEffect	= szBuf;

	m_bDropZoneFlyMove	= rkLoader.LoadBool_e( "drop_zone_fly_move", false );
}

void ioOwnerTraceWoundAniDummy::InitDummyCharInfo( int iIndex,
	...
	float fStartAngle, bool bNetWork )
{
	...
	{	
		...
		if( pStage )
		{
			...
			if( !m_OwnerEffect.IsEmpty() && pOwner->IsOwnerChar() )
			{
				szCurTeamEffect = m_OwnerEffect;
			}
			else if( pOwner->GetTeam() == TEAM_RED && !m_RedTeamEffect.IsEmpty() )
			{
...
void ioOwnerTraceWoundAniDummy::Update( float fTimePerSec, DWORD dwPreTime, DWORD dwCurTime )
{
...
bool ioOwnerTraceWoundAniDummy::CheckOwnerDestroyState()
{
	ioBaseChar *pOwner = GetOwner();
...
void ioOwnerTraceWoundAniDummy::ProcessState( float fTimerPerSec, DWORD dwPreTime )
{
	ioBaseChar *pOwner = GetOwner();
	if ( !pOwner )
	{
...
void ioOwnerTraceWoundAniDummy::ProcessPos( float fTimePerSec )
{
	...
	do 
	{
		...
		if( !m_bDropZoneFlyMove || fMapHeight != 0.0f )
		{
...
void ioOwnerTraceWoundAniDummy::SetDelayState( bool bNet )
{
...
void ioOwnerTraceWoundAniDummy::SetLoopAni( ioHashString szAniName, float fRate /*= 1.0f */ )
{
...
void ioOwnerTraceWoundAniDummy::ProcessStartState()
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioOwnerTraceWoundAniDummy::ProcessDelayState( float fTimePerSec )
{
	ioBaseChar *pOwner = GetOwner();
	if ( !pOwner || !pOwner->IsNeedProcess() )
		return;
...
bool ioOwnerTraceWoundAniDummy::CheckOwnerTrace( ioBaseChar *pOwner )
{
	if ( !pOwner || !pOwner->IsNeedProcess() )
		return false;
...
void ioOwnerTraceWoundAniDummy::SetOwnerTraceState( ioBaseChar *pOwner )
{
	if ( !pOwner )
		return;
...
void ioOwnerTraceWoundAniDummy::ProcessOwnerTraceState( ioBaseChar *pOwner, float fTimerPerSec )
{
	if ( !pOwner )
		return;
...
void ioOwnerTraceWoundAniDummy::CheckTracingDir( float fTimerPerSec, D3DXVECTOR3& vTracingMoveDir, D3DXVECTOR3 vDiff )
{
	vDiff.y=0;
...
void ioOwnerTraceWoundAniDummy::ProcessTraceMoving( ioBaseChar *pOwner, D3DXVECTOR3& vDir, float fMoveAmt, bool bNoDropZoneMove )
{
	if ( !pOwner )
		return;
	ioPlayStage *pStage = pOwner->GetCreator();
	...
	{
		if( !m_bDropZoneFlyMove && fGapHeight <= 0.0f )
		{
			D3DXQUATERNION qtRot;
			ioMath::CalcDirectionQuaternion( qtRot, -vDir );
			SetWorldOrientation( qtRot );
		...
		else
		{
			...
			if( !m_bDropZoneFlyMove && m_fEnableTraceMoveHeight < fabs(vDiff.y - fNewMapHeight) )
				return;
...
void ioOwnerTraceWoundAniDummy::SetSensingState( ioBaseChar *pOwner, bool bNet )
{
...
void ioOwnerTraceWoundAniDummy::ProcessSensingState( ioBaseChar *pOwner, float fTimerPerSec )
{
	if ( !pOwner || !pOwner->IsNeedProcess() )
		return;
...
void ioOwnerTraceWoundAniDummy::SetTargetTraceState( ioBaseChar *pOwner )
{
...
void ioOwnerTraceWoundAniDummy::ProcessTargetTraceState( ioBaseChar *pOwner, float fTimerPerSec )
{
	if ( !pOwner )
		return;
...
bool ioOwnerTraceWoundAniDummy::IsTraceTarget( ioHashString szTargetName, ioPlayStage *pStage )
{
	if( !pStage )
		return false;
...
void ioOwnerTraceWoundAniDummy::CheckTraceAttack( ioBaseChar *pOwner )
{
	if ( !pOwner || !pOwner->IsNeedProcess() )
		return;
...
void ioOwnerTraceWoundAniDummy::SetTracingAttack( ioBaseChar *pOwner, bool bNet )
{
	if ( !pOwner )
		return;
	...
	if ( !m_bSpecialAttack && m_iCurAttackCount > (int)m_vAttackCode.size() )
		return;
	...
	D3DXVECTOR3 vTargetPos = GetWorldPosition();
	D3DXVECTOR3 vDiff = pTarget->GetWorldPosition() - vTargetPos;
	...
	AttackAttribute rkCurAttribute;
	...
	bool bInitAttackCount = false;
	if( m_bSpecialAttack && m_iCurAttackCount >= (int)m_vAttackCode.size() )
	{
		m_iCurAttackCount = 0;
		bInitAttackCount = true;

		rkCurAttribute = m_DummySpecialAttack.m_AttackAttribute;
	}
	else
	{
		...
		rkCurAttribute = iter->second.m_AttackAttribute;
	}
...
void ioOwnerTraceWoundAniDummy::ProcessTracingAttack( ioBaseChar *pOwner, float fTimePerSec, DWORD dwPreTime )
{
	if ( !pOwner )
		return;
...
ApplyWeaponExp ioOwnerTraceWoundAniDummy::ApplyWeapon( ioWeapon *pWeapon )
{
	...
	if( m_DummyState == DS_DIE || m_DummyState == DS_DYING )
		return AWE_NO_EXPLOSION;
	...
	vAttackDir	= pWeapon->GetAttackDir(this);
...
bool ioOwnerTraceWoundAniDummy::IsCollisionAvailableWeapon( ioWeapon *pWeapon )
{
	if( !pWeapon ) 
		return	false;
...
void ioOwnerTraceWoundAniDummy::SetDyingState( ioBaseChar *pOwner, bool bNet )
{
	if ( !pOwner )
		return;
	ioPlayStage *pStage = pOwner->GetCreator();
...
void ioOwnerTraceWoundAniDummy::ProcessDyingState( ioBaseChar *pOwner )
{
...
void ioOwnerTraceWoundAniDummy::SetDieState( bool bEffect, bool bWoundedDie /*= false */ )
{
	if( m_State == DCS_DIE )
		return;
...
void ioOwnerTraceWoundAniDummy::ProcessAreaWeapon()
{
...
bool ioOwnerTraceWoundAniDummy::IsEnableTelepoteState()
{
	switch( m_DummyState )
	{
	case DS_DYING:
...
void ioOwnerTraceWoundAniDummy::CheckEntityCollision( ioPlayEntity *pEntity, bool bDefense )
{
...
void ioOwnerTraceWoundAniDummy::TraceOwnerAttackTarget( ioBaseChar *pOwner, ioHashString szTargetName )
{
	if ( !pOwner )
		return;
	ioPlayStage *pStage = pOwner->GetCreator();
...
void ioOwnerTraceWoundAniDummy::ApplyDummyCharSync( ioPlayStage *pStage, SP2Packet &rkPacket )
{
	if( !pStage )
		return;
...
void ioOwnerTraceWoundAniDummy::CheckDyingByHp()
{
	if( m_State == DCS_DIE )
		return;
...
AttackSide	ioOwnerTraceWoundAniDummy::GetAttackedSide( const D3DXVECTOR3 &vAttackDir ) const
{
...
void	ioOwnerTraceWoundAniDummy::SetLastApplyWeapon( int iIndex, DWORD dwTime )
{
	if( dwTime == 0 )
		return;
...
void	ioOwnerTraceWoundAniDummy::SetWoundedState( const ioHashString &szWoundedAni, float fTimeRate )
{
	if( !m_pGroup )
		return;
...
void	ioOwnerTraceWoundAniDummy::ProcessWoundedState()
{
	ioBaseChar*	pOwner	= GetOwner();
	if ( !pOwner )
		return;
	...
	DWORD	dwCurTime	= FRAMEGETTIME();
	if( m_DummyState != DS_DYING && m_DummyState != DS_DIE && m_AnimationTime.dwFullTime != 0 && m_AnimationTime.dwStartTime + m_AnimationTime.dwFullTime < dwCurTime )
	{
		SetDelayState( false );
	}
}

void	ioOwnerTraceWoundAniDummy::ApplyWeaponWounded( ioWeapon *pWeapon, AttackSide eSide, const D3DXVECTOR3 &vAttackDir )
{
	if( !pWeapon )
		return;
	...
	SetWoundedState( m_szWoundAni, m_fWoundAniRate );
	...
	if( vPushDir != ioMath::UNIT_Y )
	{
		...
		SetForcePower( vPushDir, fPushPower, FLOAT1, false );
	}

	bool	bEnableHitCount	= true;
	if( pWeapon->HasFeature(WF_DISABLE_HIT_COUNT) )
...
void	ioOwnerTraceWoundAniDummy::ApplyWeaponWoundedInNetwork( ioPlayStage *pStage, SP2Packet &rkPacket )
{
	D3DXVECTOR3	vPos;
	D3DXQUATERNION	qtRot;
	rkPacket >> vPos;
