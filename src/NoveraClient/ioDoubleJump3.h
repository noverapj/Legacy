...
class ioDoubleJump3 : public ioExtendJump
{
public:
...
protected:
	...
	ioHashStringVec m_vDoubleJumpAniList;//ioHashString m_DoubleJumpAni;
...
public:
	...
	bool IsUseJumpAttackGravity();
	float GetJumpAttackGravityAmt();
...
public:
	ioDoubleJump3();
	ioDoubleJump3( const ioDoubleJump3 &rhs );
	virtual ~ioDoubleJump3();
};

inline ioDoubleJump3* ToDoubleJump3( ioExtendJump *pJump )
{
	if( !pJump || pJump->GetType() != JT_DOUBLE_JUMP3 )
		return NULL;

	return dynamic_cast< ioDoubleJump3* >( pJump );
}
