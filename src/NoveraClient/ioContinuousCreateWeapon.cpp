...
ActionStopType ioContinuousCreateWeapon::m_ActionStopType = AST_NONE;
ActionStopType ioContinuousCreateWeapon::m_DieActionStopType = AST_NONE;

ioContinuousCreateWeapon::ioContinuousCreateWeapon( ioBaseChar *pOwner, const WeaponAttribute *pAttr )
...
ioContinuousCreateWeapon::~ioContinuousCreateWeapon()
{
	if( m_pIceCoreMesh )
	{
		LOG.PrintTimeAndLog( 0, "ioContinuousCreateWeapon::~ioContinuousCreateWeapon - UnReleased Resouce" );
	}
}

bool ioContinuousCreateWeapon::InitResource()
{
...
void ioContinuousCreateWeapon::LoadProperty( ioINILoader &rkLoader )
{
...
void ioContinuousCreateWeapon::LoadCallAttribute( ioINILoader &rkLoader )
{
	char szBuf[MAX_PATH];
...
void ioContinuousCreateWeapon::LoadCallAttributeFireDirectionList( ioINILoader &rkLoader )
{
	char szBuf[MAX_PATH] = "";
	char szKey[MAX_PATH] = "";
	...
	for( int i= 0; i < iCnt; ++i )
	{
		...
		wsprintf_e( szBuf, "call_attribute%d_time", i+1 );
		kInfo.m_dwCallTime = rkLoader.LoadInt( szBuf, 0 );
...
void ioContinuousCreateWeapon::LoadChangeDirInfoList( ioINILoader &rkLoader )
{
	char szBuf[MAX_PATH] = "";
...
bool ioContinuousCreateWeapon::SetIceCoreMesh( const ioHashString &szFileName )
{
...
void ioContinuousCreateWeapon::DestroyResource( ioPlayStage *pStage )
{
	if( m_pIceCoreMesh )
	{
...
const ioOrientBox& ioContinuousCreateWeapon::GetLocalAttackBox() const
{
	return m_LocalAttackBox;
}

ioWeapon::WeaponType ioContinuousCreateWeapon::GetType() const
{
	return WT_CONTINIOUS_CREATE;
}

ActionStopType ioContinuousCreateWeapon::GetActionStopType() const
{
	return m_ActionStopType;
}

ActionStopType ioContinuousCreateWeapon::GetDieActionStopType() const
{
	return m_DieActionStopType;
}

void ioContinuousCreateWeapon::SetMoveDir( const D3DXVECTOR3 &vDir )
{
	m_vMoveDir = vDir;
...
void ioContinuousCreateWeapon::SetStartPosition( const D3DXVECTOR3 &vPos, ioPlayStage *pStage )
{
...
void ioContinuousCreateWeapon::PlayWoundedSound( ioPlayEntity *pEntity )
{
...
void ioContinuousCreateWeapon::Process( float fTimePerSec, ioPlayStage *pStage )
{
	if( !CheckPiercing() )
		return;
...
bool ioContinuousCreateWeapon::CheckKeyInput( ioBaseChar* pOwner, ioPlayStage *pStage )
{
...
bool ioContinuousCreateWeapon::CheckMoveRange( ioBaseChar* pOwner, ioPlayStage *pStage, float fTimePerSec )
{	
...
bool ioContinuousCreateWeapon::TestMapCollision( ioOpcodeShape *pShape, ioPlayStage *pStage, const D3DXMATRIX* pShapeWorldMat )
{
...
void ioContinuousCreateWeapon::OnFloating( float fTimePerSec, ioPlayStage *pStage )
{
	if( !m_bFirst )
	{
...
void ioContinuousCreateWeapon::UpdateWorldAttackBox( float fTimePerSec, bool bSetNull )
{
	if( !IsLive() || bSetNull )
	{
...
bool ioContinuousCreateWeapon::IsCanMineExplosion()
{
...
void ioContinuousCreateWeapon::Explosion( ioPlayStage *pStage )
{	
...
bool ioContinuousCreateWeapon::IsFloating() const
{
	if( m_IceState == DIS_FLOATING )
		return true;
...
bool ioContinuousCreateWeapon::IsExplosion() const
{
	if( m_IceState == DIS_EXPLOSION || m_IceState == DIS_EXPLOSION_END )
		return true;
...
void ioContinuousCreateWeapon::CheckPreCollision( ioPlayEntity *pTarget, ioPlayStage *pStage )
{
	if( m_IceState != DIS_FLOATING )
		return;
...
bool ioContinuousCreateWeapon::IsCollision( ioPlayEntity *pEntity, bool bMapCollision )
{
	if( m_IceState != DIS_EXPLOSION && m_IceState != DIS_EXPLOSION_END )
		return false;
...
DWORD ioContinuousCreateWeapon::GetCollisionEnableTime() const
{
	if( m_IceState == DIS_EXPLOSION || m_IceState == DIS_EXPLOSION_END )
...
float ioContinuousCreateWeapon::GetMoveSpeed()
{
...
float ioContinuousCreateWeapon::GetMaxRange()
{
	return GetAttribute()->m_fMoveRange * m_fOwnerChargeRangeRate;
}

void ioContinuousCreateWeapon::CallAttribute()
{	
	...
	if(m_iCallindex >= (int)m_CallAttFireList.size())
	{
		m_bArleadyCallWeapon = true;
		return;
	}
	if(dwCurTime >= m_dwCreatedTime + m_CallAttFireList[m_iCallindex].m_dwCallTime)
	{
		D3DXVECTOR3 vDir = m_vMoveDir;
		D3DXVec3Normalize(&vDir, &vDir);
		float fAngleH = ioMath::ArrangeHead( m_CallAttFireList[m_iCallindex].m_fChangeAngleH );
		float fAngleV = ioMath::ArrangeHead( m_CallAttFireList[m_iCallindex].m_fChangeAngleV );
...
void ioContinuousCreateWeapon::ApplyWeaponSync( SP2Packet &rkPacket, ioPlayStage *pStage )
{
	int iSyncType;
	rkPacket >> iSyncType;
...
void ioContinuousCreateWeapon::SetLiveTime( DWORD dwTime )
{
	if( m_dwLiveTime == 0 )
		m_dwLiveTime = dwTime;
}

void ioContinuousCreateWeapon::CheckChangeDir()
{
...
void ioContinuousCreateWeapon::RotateMoveDir( float fAngleH, float fAngleV )
{
