...
m_fCancelJumpAmt( rhs.m_fCancelJumpAmt )
{
	m_ChargeComboState = CCS_NONE;
...
void ioGatotkacaJump::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_fJumpDashAttackGravity = rkLoader.LoadFloat_e( "jump_dash_attack_gravity_amt", FLOAT1 );
	m_bEnableCancelJump = rkLoader.LoadBool_e( "enable_cancel_jump", false );
	m_fCancelJumpAmt = rkLoader.LoadFloat_e( "cancel_jump_amt", 0.0f );
}
...
void ioGatotkacaJump::ProcessJumpping( ioBaseChar *pOwner, float fHeightGap, float fNeedGauge )
{
	...
	if( IsCanJumpDash( pOwner, fHeightGap ) )
	{
		ioAttackableItem *pItem = pOwner->GetPriorityItem( SPT_JUMP );
		if( pItem )
			SetJumpDashAttack( pOwner, m_fJumpDashAttackGravity, true );
	}
...
void ioGatotkacaJump::ProcessJumpDash( ioBaseChar *pOwner )
{
	...
	ioAttackableItem *pItem = pOwner->GetPriorityItem( SPT_JUMP );
	ioGatotkacaItem *pGatotkaca = ToGatotkacaItem( pItem );
	if( pGatotkaca && m_bEnableCancelJump && m_dwCancelJumpTime > 0 && 
		m_dwCancelJumpTime + pOwner->GetActionStopTotalDelay() < dwCurTime && 
		pGatotkaca->IsEnableGauge() && pOwner->IsJumpKey() )
	{
		m_bUsedChargeFly = false;

		pGatotkaca->CheckDecreaseGauge( pOwner );
		pOwner->ClearAttackFireTimeAndSkill();
		pOwner->SetExtendAttackEndJump( m_fCancelJumpAmt, FLOAT1, true, true, true );
		return;
	}
	...
	case CCS_DASH:
		{
			...
			SetJumpDashAttack( pOwner, fGravityAmt, bRefresh );
		}
		break;
	case CCS_CHARGE:
		{
...
void ioGatotkacaJump::SetNewComboJumpAttack( ioBaseChar *pOwner, float fGravityAmt, bool bRefresh )
{
	...
	if( fInputJumpTime > 0.0f )
		m_dwCancelJumpTime = dwStartTime + fInputJumpTime;
...
void ioGatotkacaJump::SetJumpDashAttack( ioBaseChar *pOwner, float fGravityAmt, bool bRefresh )
{
	DWORD dwCurTime = FRAMEGETTIME();
	...
	float fTimeRate = m_JumpDashAttack.m_fAttackAniRate;
	DWORD dwPreDelay = m_JumpDashAttack.m_dwPreDelay;
	DWORD dwEndDelay = m_JumpDashAttack.m_dwEndDelay;
	...
	pGrp->SetActionAni( iAniID, FLOAT100, FLOAT100, FLOAT1, FLOAT1/fTimeRate, 0.0f, dwPreDelay, true );		
	pOwner->RefreshFireTimeList( iAniID, m_JumpDashAttack.m_vWeaponInfoList,
								 FTT_JUMP_ATTACK,
								 fTimeRate,
								 dwPreDelay );
	...
	pOwner->SetReservedSliding( m_JumpDashAttack.m_vForceInfoList, iAniID, fTimeRate, dwPreDelay, FLOAT1, false );
...
void ioGatotkacaJump::ProcessNewComboJumpAttack( ioBaseChar *pOwner )
{
	...
	if( pGatotkaca && m_bEnableCancelJump && m_dwCancelJumpTime > 0 && 
		m_dwCancelJumpTime + pOwner->GetActionStopTotalDelay() < dwCurTime && 
		m_iCurComboCnt == 1 && pGatotkaca->IsEnableGauge() && pOwner->IsJumpKey() )
	{
		m_bUsedChargeFly = false;
...
bool ioGatotkacaJump::IsEnableJumpDash( ioBaseChar *pOwner )
{
