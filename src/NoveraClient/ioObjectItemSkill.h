...
class ioObjectItemSkill : public ioNormalSkill
{
public:
	enum CreateState
	{
...
public:
	ioObjectItemSkill();
	ioObjectItemSkill( const ioObjectItemSkill &rhs );
	virtual ~ioObjectItemSkill();
};

inline ioObjectItemSkill* ToObjectItemSkill( ioSkill *pSkill )
{
	ioNormalSkill *pNormal = ToNormalSkill( pSkill );
	if( !pNormal || pNormal->GetSubSkillType() != NST_OBJECT )
		return NULL;

	return dynamic_cast< ioObjectItemSkill* >( pSkill );
}
