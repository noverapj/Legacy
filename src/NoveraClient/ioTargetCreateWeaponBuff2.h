...
class ioTargetCreateWeaponBuff2 : public ioBuff
{
protected:
	enum BuffState
	{
...
protected:
	...
	bool CheckPointKey( ioBaseChar *pOwner );

protected:
	void SetAttackState( ioBaseChar *pOwner, ioHashString szName, float fRate );
	void SetPointTarget( ioBaseChar *pOwner );
...
public:
	ioTargetCreateWeaponBuff2();
	ioTargetCreateWeaponBuff2( const ioTargetCreateWeaponBuff2 &rhs );
	virtual ~ioTargetCreateWeaponBuff2();
};

inline ioTargetCreateWeaponBuff2* ToTargetCreateWeaponBuff2( ioBuff *pBuff )
{
	if( !pBuff || pBuff->GetTypeID() != BT_TARGET_WEAPON2 )
		return NULL;

	return dynamic_cast< ioTargetCreateWeaponBuff2* >( pBuff );
}