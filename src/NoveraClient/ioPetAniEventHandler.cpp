...
ioPetAniEventHandler::ioPetAniEventHandler()
{
	m_pChar = NULL;
}
...
void ioPetAniEventHandler::SetCreatePet( ioPet *pChar )
{
	m_pChar = pChar;
}

bool ioPetAniEventHandler::EventHandling( const AniEventTime *pEvent, const ioAnimation *pNotifier )
{
...
void ioPetAniEventHandler::OnAutoEffect( const AniEventTime *pEvent, const ioAnimation *pNotifier )
{
	m_pChar->AttachEffect( pEvent->szSubInfo, NULL );
}

void ioPetAniEventHandler::OnShopEffect( const AniEventTime *pEvent, const ioAnimation *pNotifier )
{
	if( !m_pChar ) return;
...
void ioPetAniEventHandler::OnAnimationClear( const AniEventTime *pEvent, const ioAnimation *pNotifier )
{
	// Hide ¸Þ½¬ º¹±Í
