...
class ioMachineCannon2 : public ioMachineStruct
{
public:
	enum CannonState
	{
...
protected:
	...
	ioHashString m_ChargingAnimation;
...
protected:
	...
	void ProcessCharging( float fTimePerSec );
	void ProcessFire( float fTimePerSec );

	void ChangeToFireState();
	void ChangeToChargingState();

	void ApplyMachineState( SP2Packet &rkPacket );

public:
	ioMachineCannon2( ioEntityGroup *pGrp, ioPlayMode *pMode );
	virtual ~ioMachineCannon2();
};
