...
void ioTargetDebuffSkill5::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_fFindEnemyAnimationRate = rkLoader.LoadFloat_e( "wait_attack_key_motion_rate", FLOAT1 );

	m_fLookDist = rkLoader.LoadFloat_e( "target_look_offset", FLOAT100 );
	rkLoader.LoadString_e( "target_find_buff", "", szBuf, MAX_PATH );
	m_stEnemyFindBuff = szBuf;

	m_fFindEnemyMaxLength = rkLoader.LoadFloat_e( "target_find_max_length", FLOAT100 );
...
void ioTargetDebuffSkill5::FindEnemyByBuff( ioBaseChar *pChar, ioPlayStage *pStage )
{
	...
	if( pFindTargetChar )
	{
		D3DXVECTOR3 vTargetDir = pFindTargetChar->GetTargetRot()  * D3DXVECTOR3( 0.0f, 0.0f, -FLOAT1 );
