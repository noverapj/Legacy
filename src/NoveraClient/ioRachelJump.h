...
class ioRachelJump : public ioExtendJump
{
public:
...
public:
	ioRachelJump();
	ioRachelJump( const ioRachelJump &rhs );
	virtual ~ioRachelJump();
};

inline ioRachelJump* ToRachelJump( ioExtendJump *pJump )
{
	if( !pJump || pJump->GetType() != JT_RACHEL_JUMP )
		return NULL;

	return dynamic_cast< ioRachelJump* >( pJump );
}
