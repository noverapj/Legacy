...
ioTeamTargetBuffSkill::ioTeamTargetBuffSkill()
{
	ClearData();
}

ioTeamTargetBuffSkill::ioTeamTargetBuffSkill( const ioTeamTargetBuffSkill &rhs )
	: ioBuffSkill( rhs ),
...
void ioTeamTargetBuffSkill::ClearData()
{
...
void ioTeamTargetBuffSkill::LoadProperty( ioINILoader &rkLoader )
{
...
ioSkill* ioTeamTargetBuffSkill::Clone()
{
	return new ioTeamTargetBuffSkill( *this );
}

bool ioTeamTargetBuffSkill::OnSkillStart( ioBaseChar *pChar )
{
...
void ioTeamTargetBuffSkill::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
...
void ioTeamTargetBuffSkill::OnSkillEnd( ioBaseChar *pChar )
{
...
bool ioTeamTargetBuffSkill::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
...
bool ioTeamTargetBuffSkill::IsEnableReserve() const
{
	if( m_dwPreDelayStartTime != 0 )
		return false;
...
void ioTeamTargetBuffSkill::ApplyExtraSkillInfo( ioBaseChar *pChar, ioPlayStage *pStage, SP2Packet &rkPacket )
{
	rkPacket >> m_szTargetName;
	SetFireState( pChar );
}

void ioTeamTargetBuffSkill::SetLoopState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pChar ) return;
...
void ioTeamTargetBuffSkill::SetFireState( ioBaseChar *pChar )
{
	// 공격키 누른후 오는 곳
	if( !pChar ) return;
...
bool ioTeamTargetBuffSkill::CheckUseItemMesh()
{
	return m_bUseItemMesh;
}

bool ioTeamTargetBuffSkill::IsProtected( int iDefenseBreakType ) const
{
...
bool ioTeamTargetBuffSkill::CheckExceptionCamera()
{
...
D3DXVECTOR3 ioTeamTargetBuffSkill::GetExceptionCameraPos( ioBaseChar *pChar )
{
	if( !pChar )
...
void ioTeamTargetBuffSkill::CheckKeyInput( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioTeamTargetBuffSkill::CheckTarget( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pStage )	return;
	if( !pChar )	return;
...
void ioTeamTargetBuffSkill::UpdateAimMark( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pChar )
		return;
...
void ioTeamTargetBuffSkill::DestroyAimMark( ioBaseChar *pChar )
{
	if( !pChar )	return;
...
void ioTeamTargetBuffSkill::AddTargetBuff( ioBaseChar *pChar )
{
	if( !pChar ) return;
