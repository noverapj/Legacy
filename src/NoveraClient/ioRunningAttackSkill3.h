...
class ioRunningAttackSkill3 : public ioAttackSkill
{
public:
	enum RunningState
	{
...
public:
	ioRunningAttackSkill3();
	ioRunningAttackSkill3( const ioRunningAttackSkill3 &rhs );
	virtual ~ioRunningAttackSkill3();
};

inline ioRunningAttackSkill3* ToRunningAttackSkill3( ioSkill *pSkill )
{
	ioAttackSkill *pAttack = ToAttackSkill( pSkill );
	if( !pAttack || pAttack->GetSubSkillType() != AST_RUNNING_ATTACK3 )
		return NULL;

	return dynamic_cast< ioRunningAttackSkill3* >( pAttack );
}
