...
class ioBowItem : public ioWeaponItem
{
protected:
	enum FireState
	{
...
private:
	ioHashString m_szDestoyrDummyCharOnReleased;
...
protected:
	int m_iNormalExtendAddAttackIndex;
	AttributeList m_vZeroExtendAddAtrributeList;
	AttributeList m_vExtendExtraAttributeList;
...
public:
	ioBowItem();
	ioBowItem( const ioBowItem &rhs );
	virtual ~ioBowItem();
};

inline ioBowItem* ToBowItem( ioItem *pItem )
{
	ioWeaponItem *pWeapon = ToWeaponItem( pItem );
	if( !pWeapon || pWeapon->GetSubType() != ioWeaponItem::WST_BOW )
		return NULL;

	return dynamic_cast< ioBowItem* >( pWeapon );
}
