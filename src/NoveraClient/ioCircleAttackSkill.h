...
class ioCircleAttackSkill : public ioNormalSkill
{
public:
	enum SkillState
	{
...
protected:
	...
	ioHashString m_stLastAttackMotion;
	float		 m_fLastAttackMotionRate;
	ioHashString m_stLastAttackFindBuff;
	ioHashString m_stLastAttackAddBuff;
...
protected:
	...
	void SetLastAction( ioBaseChar *pChar, ioPlayStage *pStage );
	...
	bool FindLastAttackTarget( ioBaseChar *pChar, ioPlayStage *pStage );
...
public:
	ioCircleAttackSkill();
	ioCircleAttackSkill( const ioCircleAttackSkill &rhs );
	virtual ~ioCircleAttackSkill();
};

inline ioCircleAttackSkill* ToCircleAttckSkill( ioSkill *pSkill )
{
	ioNormalSkill *pNormal = ToNormalSkill( pSkill );
	if( !pNormal || pNormal->GetSubSkillType() != NST_CIRCLE_ATTACK )
		return NULL;

	return dynamic_cast< ioCircleAttackSkill* >( pSkill );
}
