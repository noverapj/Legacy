...
ioUroborusSkill3::ioUroborusSkill3()
{
	m_bSetHandMesh = false;
	m_SkillState = USS_NONE;
	m_UroborusTargets.clear();
	m_iCurLinearCombo = 0;
...
ioUroborusSkill3::ioUroborusSkill3( const ioUroborusSkill3 &rhs )
: ioAttackSkill( rhs ),
 m_UroborusMotion( rhs.m_UroborusMotion ),
 m_dwUroborusFireTime( rhs.m_dwUroborusFireTime ),
 m_fTargetAngle( rhs.m_fTargetAngle ),
 m_fTargetRange( rhs.m_fTargetRange ),
 m_iTargetCount( rhs.m_iTargetCount ),
...
void ioUroborusSkill3::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_dwUroborusFireTime = rkLoader.LoadInt_e( "uroborus_fire_time", 200 );
	...
	m_iTargetCount = rkLoader.LoadInt_e( "uroborus_target_count", 1 );
...
ioSkill* ioUroborusSkill3::Clone()
{
	return new ioUroborusSkill3( *this );
}

bool ioUroborusSkill3::OnSkillStart( ioBaseChar *pChar )
{
	if( !HasSkillMotion() )
		return false;
...
void ioUroborusSkill3::OnSkillEnd( ioBaseChar *pChar )
{
...
bool ioUroborusSkill3::CheckUseItemMesh()
{
	return m_bUseItemMesh;
}

void ioUroborusSkill3::ApplyAttributeToWeapon( ioWeapon *pWeapon, ioPlayStage *pStage )
{
	...
	{
		D3DXVECTOR3 vDir;
		if( (int)m_UroborusTargets.size() > m_iWeaponIndex )
			vDir = m_UroborusTargets[m_iWeaponIndex].m_vUroborusDir;

		pUroborus2->SetMoveDir( vDir );
...
void ioUroborusSkill3::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	DWORD dwCurTime = FRAMEGETTIME();
	...
	case USS_FIRE:
		{
			if( m_dwMotionStartTime + m_dwUroborusFireTime > dwCurTime || m_bWeaponFire )
				return;

			m_bWeaponFire = true;
			int iCount = min( (int)m_UroborusTargets.size(), m_iTargetCount );
			for( int i = 0; i < iCount; i++ )
			{
				pChar->SkillFireExplicit( GetAttribute().m_vWeaponInfoList.front(), GetName(), m_dwWeaponBaseIndex, m_vTargetPos, m_UroborusTargets[i].m_vUroborusDir );
...
void ioUroborusSkill3::SetSkillEndState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioUroborusSkill3::CheckComboState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
bool ioUroborusSkill3::IsProtected( int iDefenseBreakType ) const
{
...
void ioUroborusSkill3::ApplyExtraSkillInfo( ioBaseChar *pChar, ioPlayStage *pStage, SP2Packet &rkPacket )
{
	int iSyncType;
	rkPacket >> iSyncType;
	...
	case US_PRE_INFO:
		{
			...
			{
				UroborusTargetInfo cInfo;
				rkPacket >> cInfo.m_szName;
				rkPacket >> cInfo.m_fRange;
				rkPacket >> cInfo.m_vUroborusDir;
				m_UroborusTargets.push_back( cInfo );
			}
		}
		break;
	case US_COMBO:
...
bool ioUroborusSkill3::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
	if( m_dwPreDelayStartTime > 0 )
		return false;
...
bool ioUroborusSkill3::IsEnableReserve() const
{
	if( m_dwPreDelayStartTime != 0 )
		return false;
...
void ioUroborusSkill3::AttackSkillFire( ioBaseChar *pChar )
{
	...
	if( pChar && pChar->IsNeedSendNetwork() )
	{
		...
		int iTargetCount = m_UroborusTargets.size();
		kPacket << iTargetCount;
		for( int i = 0; i < iTargetCount; i++ )
		{
			kPacket << m_UroborusTargets[i].m_szName;
			kPacket << m_UroborusTargets[i].m_fRange;
			kPacket << m_UroborusTargets[i].m_vUroborusDir;
		}
		P2PNetwork::SendToAllPlayingUser( kPacket );
	}
}

void ioUroborusSkill3::CheckPreUroborusInfo( ioBaseChar *pChar )
{
	if( !pChar ) return;
	...
	for( int i = 0; i < iTargetCount; i++ )
	{
		pTarget = pChar->GetBaseChar( m_UroborusTargets[i].m_szName );
		if( pTarget )
		{
			if( CheckDownAimState( pTarget ) )
				m_UroborusTargets[i].m_vUroborusDir = pTarget->GetMidPositionByRate(m_fTargetDownMidRate) - m_vTargetPos;
			else
				m_UroborusTargets[i].m_vUroborusDir = pTarget->GetMidPositionByRate(m_fTargetMidRate) - m_vTargetPos;
		}
		else
		{
			m_UroborusTargets[i].m_vUroborusDir = vSightDir;
		}
		D3DXVec3Normalize( &m_UroborusTargets[i].m_vUroborusDir, &m_UroborusTargets[i].m_vUroborusDir );
	}
}

void ioUroborusSkill3::SetUroborusMotionState( ioBaseChar *pChar, ioWeapon *pWeapon )
{
	if( !pChar ) return;
	ioEntityGroup *pGrp = pChar->GetGroup();
...
void ioUroborusSkill3::CheckUroborusAutoTarget( ioBaseChar *pChar )
{
	m_iWeaponIndex = 0;
	m_bWeaponFire = false;
	m_UroborusTargets.clear();
	...
	for( iter = vTargetList.begin(); iter != vTargetList.end(); ++iter )
	{
		...
		cInfo.m_szName = pTarget->GetCharName();
		cInfo.m_fRange = D3DXVec3Length( &vDiff );

		m_UroborusTargets.push_back( cInfo );
	}
	if( m_UroborusTargets.size() == 0 )
	{
		UroborusTargetInfo cInfo;
		m_UroborusTargets.push_back( cInfo );
	}
	std::sort( m_UroborusTargets.begin(), m_UroborusTargets.end(), UroborusTargetSort() );
}

void ioUroborusSkill3::SetNextLinearCombo( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioUroborusSkill3::CheckLinearCombo( ioBaseChar *pChar, bool bFirst )
{
	if( !pChar )
		return;
...
bool ioUroborusSkill3::CheckDownAimState( ioBaseChar *pTarget )
{
