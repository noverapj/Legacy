...
ioCatchMode::ioCatchMode( ioPlayStage *pCreator )
: ioPlayMode( pCreator )
{
	m_iBlueTeamCnt = 0;
	m_iRedTeamCnt = 0;
...
ioCatchMode::~ioCatchMode()
{
	m_bZeroHP = false;
	ClearMode();
}

void ioCatchMode::InitMode( int iMapIndex )
{
...
void ioCatchMode::LoadModeResource( ioINILoader &rkLoader )
{	
...
void ioCatchMode::UpdateMiniMapSpotPoint()
{
...
void ioCatchMode::UpdateTimeScoreWnd()
{
...
void ioCatchMode::UpdateRoundStateWnd()
{
...
void ioCatchMode::ProcessReadyState()
{
...
void ioCatchMode::ProcessPlayState()
{
...
void ioCatchMode::CheckRoundEnd()
{
...
int ioCatchMode::GetTeamPlayerCount( TeamType eTeam )
{
	int iUserCnt = 0;
...
int ioCatchMode::GetTeamPurePlayerCount( TeamType eTeam )
{
	int iUserCnt = 0;
...
float ioCatchMode::GetBlueCatchRate()
{
...
float ioCatchMode::GetRedCatchRate()
{
...
void ioCatchMode::ProcessResultState()
{
...
void ioCatchMode::RenderAfterGUI()
{
	/*
	if( m_ModeState == MS_PLAY )
	{
...
void ioCatchMode::AddNewRecord( ioBaseChar *pChar )
{
	if( !pChar )
		return;
...
void ioCatchMode::RemoveRecord( const ioHashString &rkName )
{
	int iCharCnt = m_vCharRecordList.size();
	for( int i=0 ; i<iCharCnt ; i++ )
	{
...
void ioCatchMode::RemoveRecord( int nIndex )
{
	int iCharCnt = m_vCharRecordList.size();
	if( iCharCnt > nIndex )
	{
...
ioBaseChar* ioCatchMode::GetRecordChar( int i )
{
...
ioBaseChar* ioCatchMode::GetRecordChar( const ioHashString &rkName )
{
...
int ioCatchMode::GetRecordCharCnt() const
{
	return m_vCharRecordList.size();
}

ModeRecord* ioCatchMode::FindModeRecord( int iIdx )
{
...
ModeRecord* ioCatchMode::FindModeRecordCharName( const ioHashString &rkName )
{
	return (ModeRecord*)FindRecordCharName( rkName );
}

CatchRecord* ioCatchMode::FindRecordCharName( const ioHashString &rkName )
{
	int iCharCnt = m_vCharRecordList.size();
	for( int i=0 ; i<iCharCnt ; i++ )
	{
...
void ioCatchMode::SetRoundEndInfo( WinTeamType eTeam, bool bRoundSetEnd )
{
	CheckLastKiller();
	ioPlayMode::SetRoundEndInfo( eTeam, bRoundSetEnd );
}

bool ioCatchMode::GetRoundInfo( int iIndex, RoundInfo &rkInfo )
{
...
bool ioCatchMode::GetResultInfo( int iIndex, ResultInfo &rkInfo )
{
...
bool ioCatchMode::GetFinalInfo( int iIndex, FinalInfo &rkInfo )
{
...
void ioCatchMode::ApplyModeInfo( SP2Packet &rkPacket )
{
...
void ioCatchMode::ApplyCharModeInfo( const ioHashString &rkName, SP2Packet &rkPacket, bool bDieCheck )
{
	bool bRecord;
	rkPacket >> bRecord;
...
bool ioCatchMode::ProcessTCPPacket( SP2Packet &rkPacket )
{
	ErrorReport::SetPosition( 1030, 9 );
...
bool ioCatchMode::ProcessUDPPacket( SP2Packet &rkPacket )
{
...
void ioCatchMode::NotifyEntityDie( ioPlayEntity *pEntity, ioPlayEntity *pKiller, ioBaseChar *pBestAttacker,
...
void ioCatchMode::NotifyCharDropDie( ioBaseChar *pDieChar, ioBaseChar *pKillChar, ioBaseChar *pBestAttacker,
									 float fLastRate, float fBestRate )
{
	OnNotifyCharDie( pDieChar, pKillChar, pBestAttacker, fLastRate, fBestRate );
...
void ioCatchMode::OnNotifyCharDie( ioBaseChar *pDieChar, ioBaseChar *pKillChar, ioBaseChar *pBestAttacker,
								  float fLastRate, float fBestRate )
{
	if( !pDieChar )
	{
		LOG.PrintTimeAndLog( 0, "ioCatchMode::OnNotifyCharDie - Entity is Not Char" );
		return;
	}
	...
	if( !pDieRecord )
	{
		LOG.PrintTimeAndLog( 0, "ioCatchMode::OnNotifyCharDie - %s Not Exist Char",
	...
	if( pKillChar )
	{
		...
		if( !pKillRecord )
		{
			LOG.PrintTimeAndLog( 0, "ioCatchMode::OnNotifyCharDie - %s Not Exist Kill Char",
...
void ioCatchMode::UpdateDropDieRecord( ioBaseChar *pDieChar, ioBaseChar *pKillChar, ioBaseChar *pBestAttacker )
{
	bool bDropKill = true;
...
D3DXVECTOR3 ioCatchMode::GetRevivalPos( ioBaseChar *pChar )
{
...
void ioCatchMode::ContributeLOG()
{
	char szLog[MAX_PATH];
...
void ioCatchMode::ContributeConvertTeam( TeamType eTeam )
{
...
void ioCatchMode::ContributeConvert()
{
	ContributeConvertTeam( TEAM_NONE );        //TEAM_NONE는 전체 유저에 대한 기여도 평가	
}

int ioCatchMode::ContributeMyRank( int &rRankValue1, int &rRankValue2, int &rRankValue3, int &rRankValue4 )
{
...
void ioCatchMode::ContributeRoundPercent( DWORD dwCurTime, DWORD dwTotalTime )
{
	//기여도의 100분율
...
void ioCatchMode::ContributeRoundAllPercent( DWORD dwCurTime, DWORD dwTotalTime )
{
	//기여도의 100분율
...
void ioCatchMode::DamageContribute( const ioHashString szAttacker, const ioHashString szTarget, float fDamage, DWORD dwItemCode, int iWeaponType )
{
...
void ioCatchMode::KillContribute( const ioHashString szKiller, const ioHashString szKilled, DWORD dwItemCode )
{
...
void ioCatchMode::AttackCntContribute( const ioHashString szAttacker, const ioHashString szTarget, DWORD dwItemCode, int iAttackValue/* = 100 */ )
{
...
void ioCatchMode::PrisonerContribute( const ioHashString szAttacker, const ioHashString szPrisoner, bool bFirstPrisoner )
{
...
void ioCatchMode::PrisonerEscapeContribute( const ioHashString szAttacker, const ioHashString szPrisonerEscape )
{
...
void ioCatchMode::PrisonerDropContribute( const ioHashString szAttacker, const ioHashString szPrisonerDrop )
{
...
void ioCatchMode::CheckLastKiller()
{
...
void ioCatchMode::FillLastPlayRecordInfo( SP2Packet &rkPacket )
{
	int iCharCnt = m_vCharRecordList.size();
	rkPacket << iCharCnt;
	for(int i = 0;i < iCharCnt;i++)
...
void ioCatchMode::FillPlayRecordInfo( SP2Packet &rkPacket )
{	
	int iCharCnt = m_vCharRecordList.size();
	rkPacket << iCharCnt;
	for(int i = 0;i < iCharCnt;i++)
...
void ioCatchMode::ApplyPlayRecordInfo( SP2Packet &rkPacket )
{
	int iCharCnt;
	rkPacket >> iCharCnt;
	for(int i = 0;i < iCharCnt;i++)
...
void ioCatchMode::ContributeResultSync( const ioHashString &szName, int iContribute )
{
	CatchRecord *pRecord = FindRecordCharName( szName );
	if( !pRecord ) return;
...
void ioCatchMode::CheckNonePlayTime( ioBaseChar *pChar, bool bPlayerChanged )
{
	if( !pChar )
	{
		ioBaseChar *pOwner = GetOwnerChar();
...
void ioCatchMode::OnZeroHP( SP2Packet &rkPacket )
{
...
void ioCatchMode::OnRoundReady( SP2Packet &rkPacket )
{
	ShowEndScoreWnd( false );
	SetBlindMode( false );
...
void ioCatchMode::OnRoundJoin( SP2Packet &rkPacket )
{
...
void ioCatchMode::OnRoundJoinView( SP2Packet &rkPacket )
{
	ioHashString szJoinChar;
	int iModeState;
...
void ioCatchMode::OnRoundJoinObserver( SP2Packet &rkPacket )
{
	ioHashString szJoinChar;
	int iModeState;
...
void ioCatchMode::OnPrisonerMode( SP2Packet &rkPacket )
{	
	...
	if( !pChar )
	{
		LOG.PrintTimeAndLog( 0, "ioCatchMode::PrisonerUser - %s Not Exist User", szPrisonerName.c_str() );
		return;
	}
...
void ioCatchMode::OnEscapePrisonerMode( SP2Packet &rkPacket )
{
	...
	if( !pChar )
	{
		LOG.PrintTimeAndLog( 0, "ioCatchMode::OnEscapePrisonerMode - %s Not Exist User", szName.c_str() );
		return;
	}
...
void ioCatchMode::SetTeamCatchedEvent( ioBaseChar *pChar )
{
	if( !pChar )	return;
...
void ioCatchMode::SetTeamDieEvent( ioBaseChar *pChar )
{
	if( !pChar )	return;
...
void ioCatchMode::EscapePrisonerChar( ioBaseChar *pChar )
{
...
void ioCatchMode::NotifyCharDrop( ioPlayEntity *pEntity, float fDropDamage )
{
...
void ioCatchMode::OnPrisonerDrop( SP2Packet &rkPacket )
{
...
void ioCatchMode::SetResultScoreWnd()
{
...
void ioCatchMode::SetTotalScoreWnd( IntVec &rvClassTypeList,
...
ioGameEntity* ioCatchMode::GetGuideTarget( const D3DXVECTOR3& vArrowPos )
{
	ioBaseChar *pOwner = GetOwnerChar();
...
void ioCatchMode::CheckEndSceneLookAt()
{
	if( !m_LastKillChar.IsEmpty() )
	{
...
void ioCatchMode::ClearModeData()
{
	m_LastDieChar.Clear();
	m_LastKillChar.Clear();
...
bool ioCatchMode::IsNeedRevival( ioBaseChar *pChar )
{
...
ioBaseChar* ioCatchMode::CheckMostNearChar( ioBaseChar *pChar )
{
...
void ioCatchMode::UpdateRevivalTime( const ioHashString &szName, DWORD dwCurRevivalTime )
{
...
int ioCatchMode::GetCurRank( const ioHashString& szCharName )
{
...
int ioCatchMode::GetContributePerChar( const ioHashString& szName )
{
	int iCharCnt = m_vCharRecordList.size();
