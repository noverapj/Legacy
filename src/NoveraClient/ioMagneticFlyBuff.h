...
class ioMagneticFlyBuff : public ioBuff
{
public:
	enum BuffState
	{
...
public:
	ioMagneticFlyBuff();
	ioMagneticFlyBuff( const ioMagneticFlyBuff &rhs );
	virtual ~ioMagneticFlyBuff();
};


inline ioMagneticFlyBuff* ToMagneticFlyBuff( ioBuff *pBuff )
{
	if( !pBuff || pBuff->GetTypeID() != BT_MAGNETIC_FLYING_CONTRAL )
		return NULL;

	return dynamic_cast< ioMagneticFlyBuff* >( pBuff );
}
