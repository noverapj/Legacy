...
class ioRunningAttackSkill7 : public ioAttackSkill
{
public:
	enum RunningState
	{
...
protected:
	...
	float m_fAccelRunSpeed;
	DWORD m_dwAccelRunTime;
	DWORD m_dwAccelRunStartTime;
	bool m_bAccelKeyDown;
	DWORD m_dwAccelRunCount;
	DWORD m_dwCurAccelRunCount;

	//사운드 이름
	ioHashString m_szSoundName;
	DWORD m_dwDelaySoundTime;
	DWORD m_dwCheckSoundTime;
...
protected:
	...
	void PlaySound( ioBaseChar *pChar );
public:
	ioRunningAttackSkill7();
	ioRunningAttackSkill7( const ioRunningAttackSkill7 &rhs );
	virtual ~ioRunningAttackSkill7();
};

inline ioRunningAttackSkill7* ToRunningAttackSkill7( ioSkill *pSkill )
{
	ioAttackSkill *pAttack = ToAttackSkill( pSkill );
	if( !pAttack || pAttack->GetSubSkillType() != AST_RUNNING_ATTACK7 )
		return NULL;

	return dynamic_cast< ioRunningAttackSkill7* >( pAttack );
}
