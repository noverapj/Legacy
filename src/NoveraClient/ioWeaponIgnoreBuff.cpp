...
ioWeaponIgnoreBuff::ioWeaponIgnoreBuff()
{
}

ioWeaponIgnoreBuff::ioWeaponIgnoreBuff( const ioWeaponIgnoreBuff &rhs )
: ioBuff( rhs )
{
	...
	m_vWeaponList = rhs.m_vWeaponList;
}
...
void ioWeaponIgnoreBuff::LoadProperty( ioINILoader &rkLoader )
{
	...
	{
		wsprintf_e( szBuff, "attribute_list%d", i+1 );
		int iWeaponIndex = rkLoader.LoadInt( szBuff, 0 );
		if ( iWeaponIndex == 0 )
			continue;

		m_vWeaponList.push_back( iWeaponIndex );
	}
}

ioBuff* ioWeaponIgnoreBuff::Clone()
{
	return new ioWeaponIgnoreBuff( *this );
}

void ioWeaponIgnoreBuff::StartBuff( ioBaseChar *pOwner )
{
#ifndef SHIPPING 
...
bool ioWeaponIgnoreBuff::ApplyStartBuff( ioBaseChar *pOwner, DWORD dwGlobalTime, DWORD dwGapTime )
{
	m_pOwner = pOwner;
...
void ioWeaponIgnoreBuff::SetExtraDurationByGrowth( float fDuration )
{
...
void ioWeaponIgnoreBuff::SetAction()
{
...
void ioWeaponIgnoreBuff::EndBuff()
{
	if( m_bEnableOutLine )
	{
...
void ioWeaponIgnoreBuff::ProcessBuff( float fTimePerSec )
{
...
void ioWeaponIgnoreBuff::ProcessDelay( float fTimePerSec )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioWeaponIgnoreBuff::CheckOwnerStateCheck()
{
	if( !m_pOwner ) return;
...
bool ioWeaponIgnoreBuff::CheckWeaponIndex( DWORD dwWeaponIndex )
{
	int iWeaponCount = m_vWeaponList.size();
	for( int i = 0; i < iWeaponCount; i++)
	{
		if( m_vWeaponList[i] == dwWeaponIndex )
			return false;
	}
	return true;
}