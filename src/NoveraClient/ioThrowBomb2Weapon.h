...
class ioThrowBomb2Weapon : public ioWeapon
{
protected:
	enum ThrowState
	{
...
public:
	ioThrowBomb2Weapon( ioBaseChar *pOwner, const WeaponAttribute *pAttr );
	virtual ~ioThrowBomb2Weapon();
};

inline ioThrowBomb2Weapon* ToThrowBomb2Weapon( ioWeapon *pWeapon )
{
	if( !pWeapon || pWeapon->GetType() != ioWeapon::WT_THROWBOMB2 )
		return NULL;

	return dynamic_cast< ioThrowBomb2Weapon* >( pWeapon );
}
