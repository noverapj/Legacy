...
class ioPrisonWeapon3 :	public ioWeapon
{
public:
...
protected:
	...
	GameEntityList	m_InnerEntityList;
...
protected:
	...
	IntVec m_vExceptionBuff;

protected:
	DWORD m_dwDelayTime;
	DWORD m_dwCurDelayTime;

	DWORD m_dwMaintainDuration;
	DWORD m_dwCreatePrisonTime;
...
protected:
	bool IsInnerEntity( ioPlayEntity* pEntity );

public:
	ioPrisonWeapon3( ioBaseChar *pOwner, const WeaponAttribute *pAttr );
	virtual ~ioPrisonWeapon3(void);
};

inline ioPrisonWeapon3* ToPrisonWeapon3( ioWeapon *pWeapon )
{
	if( !pWeapon || pWeapon->GetType() != ioWeapon::WT_PRISON_WEAPON3 )
		return NULL;

	return dynamic_cast< ioPrisonWeapon3* >( pWeapon );
}