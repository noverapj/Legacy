...
class ioWhipWeapon : public ioWeapon
{
protected:
...
public:
	ioWhipWeapon( ioBaseChar *pOwner, const WeaponAttribute *pAttr );
	virtual ~ioWhipWeapon();
};

inline ioWhipWeapon* ToWhipWeapon( ioWeapon *pWeapon )
{
	if( !pWeapon || pWeapon->GetType() != ioWeapon::WT_WHIP )
		return NULL;

	return dynamic_cast< ioWhipWeapon* >( pWeapon );
}
