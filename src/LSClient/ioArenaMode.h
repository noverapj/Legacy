...
class ioArenaMode :	public ioPlayMode
{
protected:
	typedef std::vector< ArenaRecord > RecordList;
...
protected:
	...
	ArenaRecord* FindRecordCharName( const ioHashString &rkName );
...
public:
	...
	inline bool GetPrisonerMode()	{ return m_bPrisonerMode; }
...
public:
	ioArenaMode( ioPlayStage *pCreator );
	virtual ~ioArenaMode();
};

inline ioArenaMode* ToArenaMode( ioPlayMode *pMode )
{
	if( !pMode || pMode->GetModeType() != MT_ARENA )
		return NULL;

	return dynamic_cast< ioArenaMode* >( pMode );
}