...
ActionStopType ioThrowBomb3Weapon::m_ActionStopType = AST_NONE;
ActionStopType ioThrowBomb3Weapon::m_DieActionStopType = AST_NONE;

ioThrowBomb3Weapon::ioThrowBomb3Weapon( ioBaseChar *pOwner, const WeaponAttribute *pAttr )
...
ioThrowBomb3Weapon::~ioThrowBomb3Weapon()
{
	if( m_pFireMesh )
	{
...
bool ioThrowBomb3Weapon::InitResource()
{
...
void ioThrowBomb3Weapon::LoadProperty( ioINILoader &rkLoader )
{
	char szBuf[MAX_PATH] = "";
	char szKey[MAX_PATH] = "";
...
void ioThrowBomb3Weapon::SetMoveDir( const D3DXVECTOR3 &vDir )
{
...
void ioThrowBomb3Weapon::SetStartPosition( const D3DXVECTOR3 &vPos, ioPlayStage *pStage )
{
...
void ioThrowBomb3Weapon::SetCurMoveDir( const D3DXVECTOR3 &vDir )
{
	m_vMoveDir = vDir;
	D3DXVec3Cross( &m_vRollAxis, &D3DXVECTOR3( 0.0f, FLOAT1, 0.0f ), &m_vMoveDir );
}

void ioThrowBomb3Weapon::SetMoveSpeed( float fSpeed )
{
	m_fMoveSpeed = fSpeed;
}

bool ioThrowBomb3Weapon::SetFireMesh( const ioHashString &szFileName )
{
	ioBaseChar *pOwner = GetOwner();
...
void ioThrowBomb3Weapon::SetWallCollision( const D3DXVECTOR3 &vFaceNrm )
{
...
void ioThrowBomb3Weapon::Process( float fTimePerSec, ioPlayStage *pStage )
{
...
bool ioThrowBomb3Weapon::TestMapCollision( ioOpcodeShape *pShape, ioPlayStage *pStage, const D3DXMATRIX* pShapeWorldMat )
{
...
void ioThrowBomb3Weapon::OnNoneState( float fTimePerSec, ioPlayStage *pStage )
{
...
void ioThrowBomb3Weapon::OnMoveState( float fTimePerSec, ioPlayStage *pStage )
{
	if( m_dwDuration > 0 && m_dwDuration < FRAMEGETTIME() )
	{
		//SetExplosionState( pStage );
...
void ioThrowBomb3Weapon::OnPingPongState( float fTimePerSec, ioPlayStage *pStage )
{
	if( m_dwDuration > 0 && m_dwDuration < FRAMEGETTIME() )
	{
		//SetExplosionState( pStage );
...
void ioThrowBomb3Weapon::OnExplosionState( float fTimePerSec, ioPlayStage *pStage )
{
...
void ioThrowBomb3Weapon::DestroyResource( ioPlayStage *pStage )
{
	if( m_pFireMesh )
	{
...
ioWeapon::WeaponType ioThrowBomb3Weapon::GetType() const
{
	return WT_THROWBOMB2;
}

ActionStopType ioThrowBomb3Weapon::GetActionStopType() const
{
	return m_ActionStopType;
}

ActionStopType ioThrowBomb3Weapon::GetDieActionStopType() const
{
	return m_DieActionStopType;
}

const ioOrientBox& ioThrowBomb3Weapon::GetLocalAttackBox() const
{
	return m_LocalAttackBox;
}

void ioThrowBomb3Weapon::UpdateWorldAttackBox( float fTimePerSec, bool bSetNull )
{
	if( !IsLive() || bSetNull )
	{
...
/*void ioThrowBomb3Weapon::SetExplosionState( ioPlayStage *pStage )
{
...
/*void ioThrowBomb3Weapon::SetReserveExplosion( ioPlayStage *pStage )
{
	if( m_State == TS_EXPLOSION )
		return;
...
bool ioThrowBomb3Weapon::IsCollision( ioPlayEntity *pEntity, bool bMapCollision )
{
	bool bCollision = false;


	ioBaseChar* pTarget = ToBaseChar( pEntity );
	if( pTarget && GetOwner() && pTarget->GetTeam() == GetOwner()->GetTeam() )
		return false;
...
void ioThrowBomb3Weapon::SetBombDrop()
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioThrowBomb3Weapon::SetJumpThrow()
{
	D3DXVECTOR3 vAxis;
...
void ioThrowBomb3Weapon::RotateMoveDir( float fAngle )
{
	D3DXVECTOR3 vAxis;
	D3DXVec3Cross( &vAxis, &m_vMoveDir, &D3DXVECTOR3( 0.0f, -FLOAT1, 0.0f ) );
...
void ioThrowBomb3Weapon::SetBoomEffect( const ioHashString &rkName )
{
	m_BoomEffect = rkName;
}

bool ioThrowBomb3Weapon::IsExplosion() const
{
	if( m_State == TS_EXPLOSION )
		return true;
...
bool ioThrowBomb3Weapon::IsCanMineExplosion()
{
...
void ioThrowBomb3Weapon::CheckPreCollision( ioPlayEntity *pTarget, ioPlayStage *pStage )
{
	if( m_State != TS_MOVE && m_State != TS_PINGPONG )
		return;
...
DWORD ioThrowBomb3Weapon::GetCollisionEnableTime() const
{
	if( m_State == TS_EXPLOSION )
...
void ioThrowBomb3Weapon::ApplyExtraReserveWeaponByWeaponInfo( ioWeapon* pWeapon )
{
...
void ioThrowBomb3Weapon::SetFloatPower( float fFloatPower )
{
	m_fFloatingPower = fFloatPower;
}

/*void ioThrowBomb3Weapon::CheckReserveExplosion( ioPlayStage *pStage )
{
	if( !m_bReserveExplosion )
	{
...
void ioThrowBomb3Weapon::VisibleCheck( ioPlayStage *pStage )
{
	if( !pStage )
		return;
...
/*void ioThrowBomb3Weapon::CheckTarget()
{
	ioPlayStage *pStage = g_WeaponMgr.GetCreator();
	if( !pStage )
		return;
...
void ioThrowBomb3Weapon::GenerateCallWeapon()
{
	ioPlayStage *pStage = g_WeaponMgr.GetCreator();
	if( !pStage )
		return;
...
void ioThrowBomb3Weapon::GenerateNoTargetCallWeapon()
{
	ioPlayStage *pStage = g_WeaponMgr.GetCreator();
	if( !pStage )
		return;
...
void ioThrowBomb3Weapon::CallAttribute()
{
...
void ioThrowBomb3Weapon::SetWeaponDead()
{
