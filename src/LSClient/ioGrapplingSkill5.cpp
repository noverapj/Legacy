...
ioGrapplingSkill5::ioGrapplingSkill5()
{
	ClearData();
}

ioGrapplingSkill5::ioGrapplingSkill5( const ioGrapplingSkill5 &rhs )
	: ioAttackSkill( rhs ),
...
void ioGrapplingSkill5::ClearData()
{
...
void ioGrapplingSkill5::LoadProperty( ioINILoader &rkLoader )
{
	...
	for( int i = 0; i < m_iTotalAttackCnt; i++ )
	{
		...
		wsprintf_e( szKey, "loop_attack%d", i + 1 );
		LoadAttackAttribute( szKey, tempAttackAttribute, rkLoader );

		m_LoopAttack.insert( AttackAttributeMap::value_type( i, tempAttackAttribute ) );
	}
...
ioSkill* ioGrapplingSkill5::Clone()
{
	return new ioGrapplingSkill5( *this );
}

bool ioGrapplingSkill5::OnSkillStart( ioBaseChar *pChar )
{
	ClearData();
...
void ioGrapplingSkill5::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	...
	case SS_LOOP:
		{
			...
			{
				SetSkillEndAttack( pChar );
				return;
			}
		}
		break;
	case SS_END_ATTACK:
		ProcessSkillEndAttack( pChar );
		break;
	case SS_END:
		break;
	}
...
void ioGrapplingSkill5::OnSkillEnd( ioBaseChar *pChar )
{
...
void ioGrapplingSkill5::AttackSkillFire( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioGrapplingSkill5::SetSkillLoopState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioGrapplingSkill5::SetSkillEndState( ioBaseChar *pChar, bool bSendNet )
{
	if( !pChar ) return;
...
void ioGrapplingSkill5::ApplyAttributeToWeapon( ioWeapon *pWeapon, ioPlayStage *pStage )
{
...
bool ioGrapplingSkill5::SetSkillGrapplingState( ioBaseChar *pChar, const ioHashString &szTargetName )
{
...
D3DXVECTOR3 ioGrapplingSkill5::GetGrapplingTargetPos( ioBaseChar *pChar )
{
...
bool ioGrapplingSkill5::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioGrapplingSkill5::ApplyExtraSkillInfo( ioBaseChar *pChar, ioPlayStage *pStage, SP2Packet &rkPacket )
{
	if( !pChar ) return;
	if( !pStage ) return;
	...
	case SSC_END_ATTACK:
		{
			rkPacket >> m_iTotalPressAttackCnt;
			SetSkillEndAttack( pChar );
		}
		break;
	case SSC_END:
...
void ioGrapplingSkill5::ProcessLoopDir( ioBaseChar *pChar )
{
...
bool ioGrapplingSkill5::IsProtected( int iDefenseBreakType ) const
{
...
bool ioGrapplingSkill5::IsEnableReserve() const
{
	return false;
}

bool ioGrapplingSkill5::CheckUseItemMesh()
{
	return m_bUseItemMesh;
}

void ioGrapplingSkill5::SetSkillEndAttack( ioBaseChar *pChar )
{
	if( !pChar ) return;
	...
	AttackAttribute kAttack = m_LoopAttack[m_iCurAttackCnt - 1];
	...
	if( pChar->IsNeedSendNetwork() )
	{
		...
		kPacket << pChar->GetTargetRot();
		kPacket << m_iTotalPressAttackCnt;
		P2PNetwork::SendToAllPlayingUser( kPacket );
	}
}

void ioGrapplingSkill5::ProcessSkillEndAttack( ioBaseChar* pChar )
{
	if( !pChar ) return;
	...
	if( m_iCurAttackCnt > m_iTotalPressAttackCnt )
	{
		SetSkillEndState( pChar, true );
		return;
	}
...
void ioGrapplingSkill5::CheckCountKey( ioBaseChar *pOwner )
{
	if( !pOwner )
		return;
	...
	{
		if( m_iTotalPressAttackCnt == -1 )
			m_iTotalPressAttackCnt = 1;
		else if( m_iTotalPressAttackCnt < m_iTotalAttackCnt )
			m_iTotalPressAttackCnt++;
		else
			m_iTotalPressAttackCnt = m_iTotalAttackCnt;
		return;
	}
}

void ioGrapplingSkill5::AddLoopEffect( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioGrapplingSkill5::RemoveLoopEffect( ioBaseChar *pChar )
{
	if( !pChar ) return;
