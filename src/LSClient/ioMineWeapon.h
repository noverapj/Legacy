...
class ioMineWeapon : public ioWeapon
{
protected:
...
public:
	ioMineWeapon( ioBaseChar *pOwner, const WeaponAttribute *pAttr );
	virtual ~ioMineWeapon();
};

inline ioMineWeapon* ToMineWeapon( ioWeapon *pWeapon )
{
	if( !pWeapon || pWeapon->GetType() != ioWeapon::WT_MINE )
		return NULL;

	return dynamic_cast< ioMineWeapon* >( pWeapon );
}
