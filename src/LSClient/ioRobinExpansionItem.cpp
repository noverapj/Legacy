...
ioRobinExpansionItem::ioRobinExpansionItem()
{
	m_iCurBullet = 0;
	m_iMaxBullet = 0;
...
ioRobinExpansionItem::ioRobinExpansionItem( const ioRobinExpansionItem &rhs )
: ioWeaponItem( rhs ),
 ...
 //m_vNormalExtendAddAttributeList( rhs.m_vNormalExtendAddAttributeList ),
...
void ioRobinExpansionItem::LoadProperty( ioINILoader &rkLoader )
{
	...
	//////////////////////////////////////////////////////////////////////////
	/*int iExtendNormalAddCnt = rkLoader.LoadInt_e( "normal_extend_add_attack_cnt",1 );
	m_vNormalExtendAddAttributeList.clear();
	m_vNormalExtendAddAttributeList.reserve( iExtendNormalAddCnt );
	for( int i = 0; i < iExtendNormalAddCnt; i++ )
	{
		AttackAttribute kAttribute;
		wsprintf_e( szBuf, "normal_extend_add_attack%d", i+1 );
		LoadAttackAttribute( szBuf, kAttribute, rkLoader );
		m_vNormalExtendAddAttributeList.push_back( kAttribute );
	}*/


	int iExtendNormalAddCnt = rkLoader.LoadInt_e( "zero_extend_attack_cnt",1 );
...
ioItem* ioRobinExpansionItem::Clone()
{
	return new ioRobinExpansionItem( *this );
}

bool ioRobinExpansionItem::SetExtendDefenseState( ioBaseChar *pOwner )
{
...
void ioRobinExpansionItem::CheckExtendDefenseState( ioBaseChar *pOwner )
{
...
void ioRobinExpansionItem::SetNormalAttack( ioBaseChar *pOwner, int iCurCombo )
{
	ClearCancelInfo();
...
void ioRobinExpansionItem::ChangeToAiming( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioRobinExpansionItem::CheckNormalAttack( ioBaseChar *pOwner )
{
	switch( m_FireState )
	{
	case FS_CHARGING:
...
void ioRobinExpansionItem::OnCharging( ioBaseChar *pOwner )
{
	if( !pOwner->IsNeedProcess() )
		return;
...
void ioRobinExpansionItem::OnAiming( ioBaseChar *pOwner )
{
	if( !pOwner->IsNeedProcess() )
		return;
...
void ioRobinExpansionItem::OnAimWait( ioBaseChar *pOwner )
{
	if( !pOwner->IsNeedProcess() )
		return;
...
void ioRobinExpansionItem::OnAimed( ioBaseChar *pOwner )
{
	if( pOwner->IsNeedProcess() )
	{
...
void ioRobinExpansionItem::OnFire( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
	...
	DWORD dwEndTime = m_dwFireStartTime + m_dwFireEndTime + m_dwFireDelayTime;
	...
	/*if( dwCurTime > dwEndTime && pOwner->IsNeedProcess() )
	{
		m_FireState = FS_NONE;
		pOwner->SetState( CS_DELAY );
		return;
	...
	if( m_dwKeyReserveTime && dwCurTime > m_dwKeyReserveTime && pOwner->IsNeedProcess() && pOwner->IsAttackKey() )
	{
		...
		ChangeExtendAttack( pOwner, bAimed, m_nNormalExtendAddAttackIndex );
	}
}


void ioRobinExpansionItem::ChangeExtendAttack( ioBaseChar *pOwner, bool bAimed, int nFireIndex )
{
	if( nFireIndex >=  (int)m_vExtendAttributeList.size() )
	{
		m_FireState = FS_END;
		return;
	}
	...
	//&m_vExtendAttributeList[ nFireIndex ];
	if( m_bZeroAimedFire )
		pAttr = &m_vZeroExtendAddAtrributeList[ nFireIndex ];
	else
		pAttr = &m_vExtendAttributeList[ nFireIndex ];
...
void ioRobinExpansionItem::OnExtendFire( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
	...
	if( m_dwKeyReserveTime && dwCurTime > m_dwKeyReserveTime && pOwner->IsNeedProcess() && 
		pOwner->IsAttackKey() && m_nNormalExtendAddAttackIndex < (int)m_vExtendAttributeList.size() )
	{
		pOwner->InitAttackReserve();
		pOwner->ClearReserveKeyInput();
		
		bool bAimedFire = m_FireState == FS_AIMED_EXTEND_FIRE ? true : false;
		ChangeExtendAttack( pOwner, bAimedFire, m_nNormalExtendAddAttackIndex );
	}
}

void ioRobinExpansionItem::OnJumpFire( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
	DWORD dwAttackOverTime = m_dwFireStartTime + m_dwFireEndTime + m_dwFireDelayTime;
...
void ioRobinExpansionItem::ChangeToNormalAttack( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
	m_fCurChargeRate = 0.0f;
...
void ioRobinExpansionItem::ChangeToAimWaitState( ioBaseChar *pOwner, const ioHashString &szTarget )
{
...
void ioRobinExpansionItem::ChangeToAimedState( ioBaseChar *pOwner )
{
	ioEntityGroup *pGrp = pOwner->GetGroup();
...
void ioRobinExpansionItem::ChangeToNormalFire( ioBaseChar *pOwner )
{
	int iExtendAttackCnt = m_vExtendAttributeList.size();
	if( iExtendAttackCnt <= m_nNormalExtendAddAttackIndex )
		return;
	...
	//스테이트가 영점 조준이면
	if( m_bZeroAimState )
		m_bZeroAimedFire = true;

	//영점 조준으로 쏘고 있던 상황인지 판별
	if( m_bZeroAimedFire )
		pAttr = &m_vZeroExtendAddAtrributeList[m_nNormalExtendAddAttackIndex];
	else
		pAttr = &m_vExtendAttributeList[m_nNormalExtendAddAttackIndex];
...
bool ioRobinExpansionItem::CheckCurrentTargetValid( ioBaseChar *pOwner, bool bAngleAlsoCheck )
{
...
void ioRobinExpansionItem::UpdateAimDir( ioBaseChar *pOwner, bool bJumpAim )
{
	ioEntityGroup *pGrp = pOwner->GetGroup();
	...
	//LOG.PrintConsoleLog( 0, "(1)angle->%.2f", m_fCurAngle );
	...
	/*if( m_bAimJumpAttack || (m_FireState == FS_AIMED_FIRE || m_FireState == FS_AIMED_EXTEND_FIRE) )
	{
	...
	else*/
	{
		...
		if( !bTarget )
		{
			...
			//LOG.PrintConsoleLog( 0, "(2)angle->%.2f", m_fCurAngle );
		}
		else
		{
			...
			//LOG.PrintConsoleLog( 0, "(3)angle->%.2f", m_fCurAngle );
		}
	}
...
void ioRobinExpansionItem::TrackingTarget( ioBaseChar *pOwner, const ioHashString& stUp, const ioHashString& stCenter, const ioHashString& stDown )
{
...
void ioRobinExpansionItem::ChangeToAimedFire( ioBaseChar *pOwner )
{
	int iExtendAttackCnt = m_vExtendAttributeList.size();
	if( iExtendAttackCnt <= m_nNormalExtendAddAttackIndex )
		return;
...
float ioRobinExpansionItem::GetAimedUpDownWeight( const D3DXVECTOR3 &vAimDir, float fMaxAngle )
{
...
void ioRobinExpansionItem::AimedFireToUp( ioEntityGroup *pGrp,
							   float fWeight,
							   float fTimeRate,
							   DWORD dwPreDelay,
							   bool bHold )
...
void ioRobinExpansionItem::AimedFireToDown( ioEntityGroup *pGrp,
								 float fWeight,
								 float fTimeRate,
								 DWORD dwPreDelay,
								 bool bHold )
...
void ioRobinExpansionItem::OnItemGaugeOK( ioBaseChar *pOwner, SP2Packet &rkPacket )
{
	int iFireState;
	bool bZeroAimState;
	bool bAimed;
	...
	case FS_AIMED_FIRE:
		...
		m_nNormalExtendAddAttackIndex = m_iCurCombo;
		ChangeToAimedFire( pOwner );
		break;
	case FS_AIMING:
...
void ioRobinExpansionItem::OnItemGaugeCancel( ioBaseChar *pOwner, SP2Packet &rkPacket )
{
	bool bAimJumpState;
...
void ioRobinExpansionItem::SetExtraInfoToWeapon( ioWeapon *pWeapon, bool bJumpAttack )
{
...
ioWeaponItem::WeaponSubType ioRobinExpansionItem::GetSubType() const
{
	return WST_ROBIN_EXPANSION_ITEM;
}

void ioRobinExpansionItem::OnEquiped( ioBaseChar *pOwner )
{
...
void ioRobinExpansionItem::OnReleased( ioBaseChar *pOwner )
{
...
bool ioRobinExpansionItem::IsCanFire( const ioBaseChar* pOwner, int iFireCnt, bool bNormalAttack ) const
{
	switch( m_BulletType )
	{
	case BCT_COUNT:
		{
			...
			if( GetJumpType() != JT_ROBIN_EXPANSION_JUMP )
				return true;
...
void ioRobinExpansionItem::ModifyFireTimeRate( float fRate )
{
	m_fFireTimeModifyRate *= fRate;
}

void ioRobinExpansionItem::WasteBullet()
{
...
void ioRobinExpansionItem::SetAimedSkillUse( bool bSkillUsed )
{
	m_bSetSkillAimed = bSkillUsed;
}

void ioRobinExpansionItem::SetAimedDirSet( bool bAimedSet )
{
	m_bSetAimedDir = bAimedSet;
}

D3DXVECTOR3 ioRobinExpansionItem::ReCalcMoveDir( ioBaseChar *pOwner, ioWeapon *pWeapon )
{
	/*ioBaseChar *pTarget = pOwner->GetBaseChar( m_AimedTarget );
	if( !pTarget && !m_bAimedBall )
		return m_vAimedDir;
...
bool ioRobinExpansionItem::CheckDownAimState( ioBaseChar *pTarget )
{
...
bool ioRobinExpansionItem::CheckAimState(bool bAimMove)
{
	if( !bAimMove )
	{
		switch( m_FireState )
...
bool ioRobinExpansionItem::SetAimState( ioBaseChar *pChar )
{
	if( m_iMaxBullet > 0 )
	{
...
void ioRobinExpansionItem::SetNoneState( ioBaseChar* pOwner, int eNewState /* = 0 */ )
{
	m_AimedTarget.Clear();
	m_bAimedBall = false;
...
bool ioRobinExpansionItem::SetJumpAttack( ioBaseChar *pOwner, int iAniID, float fTimeRate, AttackType eType )
{
	ClearAirJump();
	...
	if( !pAttr )
	{
		LOG.PrintTimeAndLog( 0, "ioRobinExpansionItem::SetJumpAttack - AttackAttribute Not Exist" );
		return false;
	}
	...
	if( iAniID == -1 )
	{
		LOG.PrintTimeAndLog( 0, "ioRobinExpansionItem::SetJumpAttack - JumpAttack Animation Not Exist" );
		return false;
	}
...
void ioRobinExpansionItem::CheckJumpAttack( ioBaseChar *pOwner )
{
...
bool ioRobinExpansionItem::IsChargeAttackState( ioBaseChar *pOwner )
{
	if( pOwner->GetState() == CS_ATTACK )
	{
...
void ioRobinExpansionItem::CheckAimMoveAniState( ioBaseChar *pOwner )
{
...
void ioRobinExpansionItem::OnEndExtendDefense( ioBaseChar *pOwner )
{
	if( !m_ReloadEffect.IsEmpty() )
	{
...
void ioRobinExpansionItem::OnEndExtendAttack( ioBaseChar *pOwner, bool bReleaseItem )
{
	if( m_ZeroAimedEffectID != -1 )
	{
		pOwner->EndEffect( m_ZeroAimedEffectID, false );
...
bool ioRobinExpansionItem::SetAimJumpState( ioBaseChar *pChar )
{
	if( pChar->GetState() == CS_JUMP )
		return true;
	
	if( GetJumpType() != JT_ROBIN_EXPANSION_JUMP && pChar->IsJumpKeyPress() && pChar->IsCanJumpState() && !pChar->IsBuffLimitJump() )
	{
	...
	if( pChar->IsJumpKeyPress() )
	{
		...
		{
			if( m_pExtendJump && GetJumpType() == JT_ROBIN_EXPANSION_JUMP )
			{
				// AimJump Ready
...
void ioRobinExpansionItem::SetAimJumpReady( ioBaseChar *pChar )
{
...
void ioRobinExpansionItem::SetAimJumpping( ioBaseChar *pChar, bool bFullTime )
{
	m_dwCheckAimCancelTime = 0;
	ioEntityGroup *pGrp = pChar->GetGroup();
	pGrp->ClearAllLoopAni( FLOAT100, true );
	...
	ioAttackableItem *pItem = pChar->GetPriorityItem( SPT_JUMP );
	if( pItem )
	{
		ioExtendJump *pExtendJump = pItem->GetCurExtendJump();
		ioRobinExpansionJump *pRobinExpansionJump = ToRobinExpansionJump( pExtendJump );
		if( pRobinExpansionJump )
		{
			pRobinExpansionJump->ChangeToAimState( pChar, true );
		}
	}
...
void ioRobinExpansionItem::SetAimJumpCancel( ioBaseChar *pChar )
{
	m_dwCheckAimCancelTime = 0;
	m_FireState = FS_NONE;
...
const ioHashString& ioRobinExpansionItem::GetJumppingAnimation() const
{
	bool bAim = false;
	switch( m_FireState )
	{
...
void ioRobinExpansionItem::CheckAimJumpState( ioBaseChar *pChar )
{
	switch( m_FireState )
	{
	case FS_AIMING:
...
bool ioRobinExpansionItem::CheckLanding( ioBaseChar *pChar )
{
	//if( !CheckAimState() )
	{
		pChar->SetTargetMarkerState( ioTargetMarker::MS_NONE );
...
bool ioRobinExpansionItem::CheckZeroAim( ioBaseChar *pOwner )
{
...
void ioRobinExpansionItem::SetAimEffet( ioBaseChar *pOwner )
{
	if( !m_bAimEffect )
	{
		pOwner->AttachEffect( m_AimEffect );
...
void ioRobinExpansionItem::SetZeroAim( ioBaseChar *pOwner, bool bSet )
{
	if( bSet )
	{
...
bool ioRobinExpansionItem::CheckExtendDefenseEnable( ioBaseChar *pChar )
{
	if( m_iCurBullet == m_iMaxBullet)
		return false;
...
void ioRobinExpansionItem::CheckDefenseMoveAniState( ioBaseChar *pOwner )
{
...
float ioRobinExpansionItem::GetAttackSpeedRate() const
{
...
bool ioRobinExpansionItem::IsAimedState()
{
	if( m_FireState == FS_AIMED )
		return true;
...
bool ioRobinExpansionItem::CompareAimedTarget( ioBaseChar *pOwner )
{
...
void ioRobinExpansionItem::CheckChargeRate()
{
	m_fCurChargeRate = 0.0f;
...
void ioRobinExpansionItem::CheckTargetMarker( ioBaseChar *pOwner )
{
	if( m_fCurChargeRate > 0.0f )
	{
...
float ioRobinExpansionItem::GetChargeAimRange()
{
	if( m_fCurChargeRate > 0.0f )
	{
...
float ioRobinExpansionItem::GetChargeAimAngle()
{
	if( m_fCurChargeRate > 0.0f )
	{
...
void ioRobinExpansionItem::SetFireEffect( ioBaseChar *pOwner, const D3DXVECTOR3 &vDir )
{
	if( !pOwner ) return;
...
bool ioRobinExpansionItem::IsEnableAttackDashCheck( ioBaseChar *pOwner, bool bCheck )
{
	switch( m_FireState )
	{
	case FS_AIMING:
...
int ioRobinExpansionItem::GetMaxBullet()
{
	if( m_BulletType == BCT_CHARGE )
...
int ioRobinExpansionItem::GetNeedBullet()
{
	return (int)m_dwNeedCoolTime;
}

int ioRobinExpansionItem::GetCurBullet()
{
	if( m_BulletType == BCT_CHARGE )
	{
		DWORD dwCurTime = FRAMEGETTIME();
...
void ioRobinExpansionItem::SetCurBullet( int iCurBullet )
{
	if( iCurBullet < 0 )
		return;
...
void ioRobinExpansionItem::InitCoolTime()
{
	m_dwCurCoolTime = 0;
	m_dwCheckCoolTime = FRAMEGETTIME();
}

void ioRobinExpansionItem::MaxCoolTime()
{
	m_dwCurCoolTime = m_dwMaxCoolTime;
	m_dwCheckCoolTime = FRAMEGETTIME();
}

void ioRobinExpansionItem::DecreaseCoolTime()
{
...
bool ioRobinExpansionItem::IsChargeBulletType()
{
	if( m_BulletType == BCT_CHARGE )
		return true;
...
ioHashString ioRobinExpansionItem::FindAimTarget( ioBaseChar *pOwner, const ioHashString& stChecker )
{
	ioHashString szTarget;
	m_AimedTarget.Clear();
