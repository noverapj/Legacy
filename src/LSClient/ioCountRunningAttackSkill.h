...
class ioCountRunningAttackSkill : public ioCountSkill
{
public:
	enum RunningState
	{
...
public:
	ioCountRunningAttackSkill();
	ioCountRunningAttackSkill( const ioCountRunningAttackSkill &rhs );
	virtual ~ioCountRunningAttackSkill();
};

inline ioCountRunningAttackSkill* ToCountRunningAttackSkill( ioSkill *pSkill )
{
	ioCountSkill *pCount = ToCountSkill( pSkill );
	if( !pCount || pCount->GetSubSkillType() != CST_RUNNING_ATTACK )
		return NULL;

	return dynamic_cast< ioCountRunningAttackSkill* >( pCount );
}
