...
ioControlFireSkill6::ioControlFireSkill6()
{
...
ioControlFireSkill6::ioControlFireSkill6( const ioControlFireSkill6 &rhs )
	: ioAttackSkill( rhs ),
	...
	m_vSkillUpMotion( rhs.m_vSkillUpMotion ),
	m_vSkillDownMotion( rhs.m_vSkillDownMotion ),
	m_GaugeUseType( rhs.m_GaugeUseType ),
	m_bDefenseEnd( rhs.m_bDefenseEnd ),
	m_bDashEnable( rhs.m_bDashEnable ),
...
void ioControlFireSkill6::LoadProperty( ioINILoader &rkLoader )
{
	...
	if( m_iMaxActiveCount > 0 )
	{
		for( int i=0; i < m_iMaxActiveCount; ++i )
		{
			...
			wsprintf_e( szKey, "weapon_combo%d_skill_up_motion", i+1 );
			rkLoader.LoadString( szKey, "", szBuf, MAX_PATH );
			m_vSkillUpMotion.push_back( ioHashString(szBuf) );

			wsprintf_e( szKey, "weapon_combo%d_skill_down_motion", i+1 );
			rkLoader.LoadString( szKey, "", szBuf, MAX_PATH );
			m_vSkillDownMotion.push_back( ioHashString(szBuf) );
		}
	}

	m_GaugeUseType = (GaugeUseType)rkLoader.LoadInt_e( "gauge_use_type", GUT_ON_KEY_INPUT );
	m_bDefenseEnd = rkLoader.LoadBool_e( "defense_end", false );

	m_bDashEnable = rkLoader.LoadBool_e( "dash_enable", false );
	if( m_bDashEnable )
		LoadAttackAttribute( "dash_info", m_DashInfo, rkLoader );
}

ioSkill* ioControlFireSkill6::Clone()
{
	return new ioControlFireSkill6( *this );
}

bool ioControlFireSkill6::OnSkillStart( ioBaseChar *pChar )
{
	if( !HasSkillMotion() )
		return false;
...
void ioControlFireSkill6::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	...
	case SS_LOOP:
		...
		else if( pChar->IsNeedProcess() && m_bDefenseEnd && pChar->IsDefenseKey() )
		{
			DestroyWeapon( pChar, pStage );
			SetEndState( pChar );
			return;
		}
		else if( pChar->IsNeedProcess() && m_bDashEnable && CheckInputDash( pChar ) )
		{
...
void ioControlFireSkill6::CheckHDir( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioControlFireSkill6::CheckVDir( ioBaseChar *pChar )
{
	...
	{
		LOG.PrintTimeAndLog( 0, "ioControlFireSkill6::CheckVDir - Motion Error" );
		return;
	}
...
void ioControlFireSkill6::CreateEffect(ioBaseChar *pChar, ioPlayStage *pStage )
{
	//Visible Check
	bool bVisible = false;
...
void ioControlFireSkill6::UpdateEffect( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( m_dwSkillMapEffect == -1 )
		return;
...
void ioControlFireSkill6::SetEndState( ioBaseChar* pOwner )
{
	if( !pOwner ) return;
...
void ioControlFireSkill6::OnSkillEnd( ioBaseChar *pChar )
{
	// 기본무기 장착
	if( m_bUseItemMesh )
...
void ioControlFireSkill6::SetLoopState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pChar ) return;
	ioEntityGroup *pGrp = pChar->GetGroup();
	if( !pGrp ) return;
...
void ioControlFireSkill6::SetDashState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if ( !pChar || !pStage )
		return;
...
void ioControlFireSkill6::SetFireState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pChar ) return;
	...
	{
		pChar->ReduceCurSkillGaugeBySkill( GetOwnerItem(), GetNeedGauge(pChar->GetCharName()) );
		m_bReduceGauge = true;
	}
...
void ioControlFireSkill6::AttackSkillFire( ioBaseChar *pChar )
{
	const AttributeList &rkComboList = m_vAttributeList;

	int iMaxCount = rkComboList.size();
	int iComboIndex = m_iCurActiveCount;
	if( !COMPARE( iComboIndex, 0, iMaxCount ) )
		iComboIndex = iMaxCount - 1;

	AttackAttribute kAttack = rkComboList[iComboIndex];
	...
	{
		int iWeaponCount = kAttack.m_vWeaponInfoList.size();
		for( int i = 0; i < iWeaponCount; i++ )
			m_vWeaponIndexList.push_back( m_dwWeaponBaseIndex + i );
	...
	{
		ioHashString szSkillUpMotion;
		int iUpMotionCount = m_vSkillUpMotion.size();
		if( COMPARE( iComboIndex, 0, iUpMotionCount ) )
			szSkillUpMotion = m_vSkillUpMotion[iComboIndex];

		pGrp->SetActionAni( szSkillUpMotion, 0.0f, 0.0f, m_fCurMotionRate, FLOAT1/fTimeRate, 0.0f, dwPreDelay );
		pGrp->SetActionAni( iAniID, 0.0f, 0.0f, FLOAT1 - m_fCurMotionRate, FLOAT1/fTimeRate, 0.0f, dwPreDelay );
	}
	else								// center & down
	{
		ioHashString szSkillDownMotion;
		int iDownMotionCount = m_vSkillDownMotion.size();
		if( COMPARE( iComboIndex, 0, iDownMotionCount ) )
			szSkillDownMotion = m_vSkillDownMotion[iComboIndex];

		pGrp->SetActionAni( szSkillDownMotion, 0.0f, 0.0f, m_fCurMotionRate, FLOAT1/fTimeRate, 0.0f, dwPreDelay );
		pGrp->SetActionAni( iAniID, 0.0f, 0.0f, FLOAT1 - m_fCurMotionRate, FLOAT1/fTimeRate, 0.0f, dwPreDelay );
	}
...
bool ioControlFireSkill6::CheckUseItemMesh()
{
	return m_bUseItemMesh;
}

bool ioControlFireSkill6::IsEnableDashCheck( ioBaseChar *pChar )
{
	return true;
}

void ioControlFireSkill6::CheckKeyInput( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
bool ioControlFireSkill6::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
...
bool ioControlFireSkill6::IsEnableReserve() const
{
	if( m_dwPreDelayStartTime != 0 )
		return false;
...
void ioControlFireSkill6::ApplyExtraSkillInfo( ioBaseChar *pChar, ioPlayStage *pStage, SP2Packet &rkPacket )
{
	int iState;
	rkPacket >> iState;
...
void ioControlFireSkill6::ApplyAttributeToWeapon( ioWeapon *pWeapon, ioPlayStage *pStage )
{
	if( !pWeapon ) return;
...
bool ioControlFireSkill6::IsProtected( int iDefenseBreakType ) const
{	
...
void ioControlFireSkill6::DestroyWeapon( ioBaseChar *pChar, ioPlayStage *pStage )
{
	...
	for ( int i=0 ; i<(int)m_vWeaponIndexList.size() ; ++i)
	{
		...
		if ( pWeapon )
		{
			...
			if ( pBallWeapon && pBallWeapon->IsLive() )
			{
				if( pBallWeapon->IsEndExplosion() )
					pBallWeapon->Explosion( pStage );
				else
					pBallWeapon->SetWeaponDead();
			}
		}
	}
...
bool ioControlFireSkill6::CheckInputDash( ioBaseChar *pChar )
{
