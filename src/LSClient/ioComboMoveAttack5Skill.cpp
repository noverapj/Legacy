...
ioComboMoveAttack5Skill::ioComboMoveAttack5Skill()
{
	m_State = SS_NONE;
...
ioComboMoveAttack5Skill::ioComboMoveAttack5Skill( const ioComboMoveAttack5Skill &rhs )
	: ioNormalSkill( rhs ),
	...
	m_iTotalAttackCnt( rhs.m_iTotalAttackCnt ),
...
void ioComboMoveAttack5Skill::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_iTotalAttackCnt = rkLoader.LoadInt_e( "total_attack_cnt", 0 );
	for( int i = 0; i < m_iTotalAttackCnt; i++ )
	{
		AttackAttribute attackAttribute;
		wsprintf_e( szKey, "charging_combo_attack%d", i + 1 );
		LoadAttackAttribute( szKey, attackAttribute, rkLoader );

		m_AttackAttributeList.push_back( attackAttribute );
	}
...
ioSkill* ioComboMoveAttack5Skill::Clone()
{
	return new ioComboMoveAttack5Skill( *this );
}

bool ioComboMoveAttack5Skill::OnSkillStart( ioBaseChar *pChar )
{	
	if( !HasSkillMotion() )
		return false;
...
void ioComboMoveAttack5Skill::OnSkillEnd( ioBaseChar *pChar )
{
...
void ioComboMoveAttack5Skill::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	...
	case SS_LOOP: 
		{	
			if( COMPARE( dwCurTime, m_dwLoopDurationStart, m_dwLoopDurationEnd ) && COMPARE( m_iCurCombo, 0, m_iTotalAttackCnt ) )
			{	
				// 뱡향 변화 체크
...
void ioComboMoveAttack5Skill::SetEndJumpState( ioBaseChar *pChar ){
...
bool ioComboMoveAttack5Skill::StartPreDelayAnimation( ioBaseChar *pChar )
{
...
void ioComboMoveAttack5Skill::SetPreActionState( ioBaseChar *pChar )
{
...
void ioComboMoveAttack5Skill::SetLoopState( ioBaseChar *pChar, bool bSend )
{
	if( !pChar ) return;
...
void ioComboMoveAttack5Skill::CheckLoopState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pChar )
		return;	
...
void ioComboMoveAttack5Skill::SetActionState( ioBaseChar *pChar, ioPlayStage *pStage, bool bSend )
{	
	if( !pChar ) return;
	ioEntityGroup *pGrp = pChar->GetGroup();
	...
	if( !COMPARE( m_iCurCombo, 0, m_iTotalAttackCnt ) )
		return;

	int iAniID = pGrp->GetAnimationIdx( m_AttackAttributeList[m_iCurCombo].m_AttackAnimation );
	float fAniRate = m_AttackAttributeList[m_iCurCombo].m_fAttackAniRate;
	if( fAniRate <= 0.0f )
		fAniRate = FLOAT1;

	DWORD dwPreDelay = m_AttackAttributeList[m_iCurCombo].m_dwPreDelay * fAniRate;
	...
	//Refresh
	if( !m_AttackAttributeList[m_iCurCombo].m_vWeaponInfoList.empty() )
	{
		/*pChar->RefreshFireTimeListWithBase( m_dwWeaponBaseIndex,
			iAniID,
			m_AttackAttributeList[m_iCurCombo].m_vWeaponInfoList,
			FTT_SKILL_ATTACK,
			fAniRate,
			dwPreDelay 
			);*/
		pChar->RefreshFireTimeList( iAniID,	m_AttackAttributeList[m_iCurCombo].m_vWeaponInfoList, FTT_SKILL_ATTACK,	fAniRate, dwPreDelay );
	}
...
void ioComboMoveAttack5Skill::ApplyExtraSkillInfo( ioBaseChar *pChar, ioPlayStage *pStage, SP2Packet &rkPacket )
{
	if( !pChar ) return;
	...
	case SSC_ACTION:
		{
			D3DXVECTOR3 vPos;
			D3DXQUATERNION qtRot;
			rkPacket >> vPos >> qtRot >> m_iCurCombo;
			pChar->SetWorldPosition( vPos );
			pChar->SetTargetRotToRotate( qtRot, true, false );
...
bool ioComboMoveAttack5Skill::IsProtected( int iDefenseBreakType ) const
{	
...
bool ioComboMoveAttack5Skill::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{	
	if( m_State == SS_END )
		return true;
...
bool ioComboMoveAttack5Skill::IsAttackEndState() const
{	
	if( m_State == SS_END_JUMP )
		return true;
...
bool ioComboMoveAttack5Skill::IsNoDropState() const
{
...
bool ioComboMoveAttack5Skill::CheckUseItemMesh()
{
	return m_bUseItemMesh;
}

void ioComboMoveAttack5Skill::ApplyAttributeToWeapon( ioWeapon *pWeapon, ioPlayStage *pStage )
{
...
void ioComboMoveAttack5Skill::CheckCharMoveDir( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioComboMoveAttack5Skill::SetJumpActionState( ioBaseChar *pChar )
{
	if( !pChar )
		return;
	ioEntityGroup *pGrp = pChar->GetGroup();
...
bool ioComboMoveAttack5Skill::CheckLanding( ioBaseChar *pChar )
{
	if( !pChar )
		return true;
...
void ioComboMoveAttack5Skill::CheckKeyInput( ioBaseChar *pChar )
{
...
void ioComboMoveAttack5Skill::ProcessRotate( ioBaseChar *pChar )
{
...
void ioComboMoveAttack5Skill::ProcessJumpMove( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pChar )
		return;
	if( !pStage )
...
void ioComboMoveAttack5Skill::SetEndState( ioBaseChar *pChar )
{
	if( !pChar )
		return;
