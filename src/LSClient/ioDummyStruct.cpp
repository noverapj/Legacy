...
: ioDummyChar( pGrp, pMode )
{
	...
	m_szGardenerSpecialTargetName.Clear();
}
...
void ioDummyStruct::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_bCheckGardenerSpecial = rkLoader.LoadBool_e( "set_check_gardener_special", false );
	m_fGardenerSpecialRange = rkLoader.LoadFloat_e( "check_gardener_special_dist", 0.0f );
	m_fGardenerSpecialEndJumpPower = rkLoader.LoadFloat_e( "gardener_special_end_jump_power", 0.0f );
	m_vGardenerSpecialOffset.x = rkLoader.LoadFloat_e( "gardener_special_offset_x", 0.0f );
	m_vGardenerSpecialOffset.y = rkLoader.LoadFloat_e( "gardener_special_offset_y", 0.0f );
	m_vGardenerSpecialOffset.z = rkLoader.LoadFloat_e( "gardener_special_offset_z", 0.0f );

	LoadOwnerBuffList( rkLoader );
	LoadRemoveBuffList( rkLoader );

	m_bPowerChargeShieldDummy = rkLoader.LoadBool_e( "power_charge_shield_dummy", false );
...
ApplyWeaponExp ioDummyStruct::ApplyWeapon( ioWeapon *pWeapon )
{
	...
	if ( m_bPowerChargeShieldDummy )
	{
		ioBaseChar *pOwner = GetOwner();
		if ( !pOwner || !pOwner->IsNeedProcess() )
			return AWE_NO_EXPLOSION;

		ioPowerChargeShieldItem *pShieldItem = ToPowerChargeShieldItem( pOwner->GetWeapon() );
		if ( !pShieldItem )
			return AWE_NO_EXPLOSION;

		if ( pWeapon->GetDefenseBreakType() != DBT_NONE )
			pShieldItem->DestoryShieldDummy( pOwner );
		else
			pShieldItem->DecreaseBulletByShieldDummy( fDamage );
	}
...
void ioDummyStruct::ApplyDummyCharSync( ioPlayStage *pStage, SP2Packet &rkPacket )
{
	...
	switch( iType )
	{
	case SYNC_GARDENER_TARGET:
		{
			rkPacket >> m_szGardenerSpecialTargetName;
			if( !m_szGardenerSpecialTargetName.IsEmpty() )
			{
				ioBaseChar *pTarget = pStage->GetBaseChar( m_szGardenerSpecialTargetName );
				if( pTarget )
					SetGardenerSpecial( pTarget );
			}
		}
		return;
	default:
...
//더미 죽는 상태 설정
void ioDummyStruct::SetDieState( bool bEffect, bool bWoundedDie /*= false */ )
{
	ProcessEnd();
	...
	if( !m_szGardenerSpecialTargetName.IsEmpty() )
	{
		...
		ioBaseChar *pTarget = pStage->GetBaseChar( m_szGardenerSpecialTargetName );
		if( pTarget && pTarget->GetState() == CS_GARDENER_DELAY_SPECIAL )
			pTarget->SetSKillEndJumpState( m_fGardenerSpecialEndJumpPower, false, false, false, true );
	}
}
...
void ioDummyStruct::CheckGardenerSpecial()
{
	if ( !m_bCheckGardenerSpecial )
		return;
	...
	if( m_szGardenerSpecialTargetName.IsEmpty() )
	{
		float fRangeSq = m_fGardenerSpecialRange * m_fGardenerSpecialRange;
		...
		for( BaseCharList::const_iterator iter = rkTargetList.begin(); iter != rkTargetList.end(); ++iter )
		{
			...
			if( !pChar )
				continue;
			if( !pChar->IsEquipEnableObject() )
				continue;
			if( pOwner->GetTeam() != pChar->GetTeam() )
				continue;

			D3DXVECTOR3 vDiff = pChar->GetWorldPosition() - GetWorldPosition();
			float fTargetDist = D3DXVec3LengthSq( &vDiff );
			if( fTargetDist < fRangeSq && pChar->IsJumpKey() )
			{
				SetGardenerSpecial( pChar );
				return;
			}
		}
	}
...
void ioDummyStruct::SetGardenerSpecial( ioBaseChar* pChar )
{
	m_szGardenerSpecialTargetName = pChar->GetCharName();

	D3DXVECTOR3 vPos = GetWorldPosition() + m_vGardenerSpecialOffset;
	pChar->SetWorldPosition( vPos );
	pChar->SetTargetRotToTargetPos( vPos + D3DXVECTOR3( 0.0f, 0.0f, -FLOAT1 ), true, true );
	pChar->SetState( CS_GARDENER_DELAY_SPECIAL );
	...
	if( pChar->IsNeedSendNetwork() )
	{
		...
		kPacket << m_iCreateIndex;
		kPacket << SYNC_GARDENER_TARGET;
		kPacket << m_szGardenerSpecialTargetName;
		P2PNetwork::SendToAllPlayingUser( kPacket );
	}
}

void ioDummyStruct::Update( float fTimePerSec, DWORD dwPreTime, DWORD dwCurTime )
{
...
void ioDummyStruct::CreateTrigerAttachWeapon()
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
...
void ioDummyStruct::DestoryTrigerAttachWeapon()
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
...
void ioDummyStruct::CreateTrigerAreaWeapon()
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
...
void ioDummyStruct::DestoryTrigerAreaWeapon()
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
...
void ioDummyStruct::CheckHealHp()
{
