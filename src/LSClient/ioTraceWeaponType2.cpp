...
: ioWeapon( pOwner, pAttr )
{
	...
	m_bCallAreaWeaponExceptChar	= false;
}
...
void ioTraceWeaponType2::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_bNoCallAreaWeaponOnContactChar = rkLoader.LoadBool_e( "no_call_area_weapon_contact_char", false );
...
bool ioTraceWeaponType2::IsCollision( ioPlayEntity *pEntity, bool bMapCollision )
{
	...
	if( bCollision && ToBaseChar( pEntity ) )
		m_bCallAreaWeaponExceptChar	= true;
...
void ioTraceWeaponType2::CallReserveAreaWeapon()
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
	...
	if( m_bNoCallAreaWeaponOnContactChar && m_bCallAreaWeaponExceptChar )
		return;
