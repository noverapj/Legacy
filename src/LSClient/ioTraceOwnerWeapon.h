...
class ioTraceOwnerWeapon : public ioWeapon
{
public:
	enum TraceState
	{
...
public:
	ioTraceOwnerWeapon( ioBaseChar *pOwner, const WeaponAttribute *pAttr );
	virtual ~ioTraceOwnerWeapon();
};

inline ioTraceOwnerWeapon* ToTraceOwnerWeapon( ioWeapon *pWeapon )
{
	if( !pWeapon || pWeapon->GetType() != ioWeapon::WT_TRACE_OWNER )
		return NULL;

	return dynamic_cast< ioTraceOwnerWeapon* >( pWeapon );
}
