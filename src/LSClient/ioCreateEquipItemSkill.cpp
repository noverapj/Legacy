...
ioCreateEquipItemSkill::ioCreateEquipItemSkill()
{
...
ioCreateEquipItemSkill::ioCreateEquipItemSkill( const ioCreateEquipItemSkill &rhs )
	: ioNormalSkill( rhs ),
...
void ioCreateEquipItemSkill::LoadProperty( ioINILoader &rkLoader )
{
...
ioSkill* ioCreateEquipItemSkill::Clone()
{
	return new ioCreateEquipItemSkill( *this );
}

bool ioCreateEquipItemSkill::IsProtected( int iDefenseBreakType ) const
{
...
bool ioCreateEquipItemSkill::IsCanUseSkill( ioBaseChar *pChar, bool bHelpMsg )
{
...
bool ioCreateEquipItemSkill::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
	if( m_CreateState != RS_END )
		return false;
...
bool ioCreateEquipItemSkill::OnSkillStart( ioBaseChar *pChar )
{
	if( !HasSkillMotion() )
		return false;
...
void ioCreateEquipItemSkill::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
...
void ioCreateEquipItemSkill::OnSkillEnd( ioBaseChar *pOwner )
{
	// 기본무기 장착
...
void ioCreateEquipItemSkill::StartPreDelayEffect( ioBaseChar *pChar )
{
	ioSkill::StartPreDelayEffect( pChar );
}

void ioCreateEquipItemSkill::EndPreDelayEffect( ioBaseChar *pChar )
{
	ioSkill::EndPreDelayEffect( pChar );
}

void ioCreateEquipItemSkill::SetActionState( ioBaseChar *pChar )
{
...
void ioCreateEquipItemSkill::SetLoopState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	...
	D3DXVECTOR3	vItemPos1, vItemPos2, vItemPos3;
	std::vector< D3DXVECTOR3 > vItemPos;
	for( int i = 0; i < iSize; i++ )
	{
		vItemPos.push_back( pChar->GetWorldPosition() + pChar->GetWorldOrientation() * m_vDummyOffsetList[i] );
	}
	...
	if( P2PNetwork::IsNetworkPlaying() && pStage->IsNetworkMode() )
	{
		if( pChar->IsNeedSendNetwork() )
		{
			...
			SP2Packet kPacket( CTPK_CREATE_FIELD_ITEM_BY_USE_SKILL );
			for( int i = 0; i < iSize; i++ )
			{
				kPacket << vItemPos[i];
			}
		...
		for( int i = 0; i < iSize; i++ )
		{
			...
			if( dwCreateItemCode != 0 )
			{
				...
				else
				{
					...
					if( pField )
					{
						pField->SetWorldPositionByServerSide( vItemPos[i] );
...
void ioCreateEquipItemSkill::SetEndState( ioBaseChar *pChar )
{
	ioEntityGroup *pGrp = pChar->GetGroup();
...
bool ioCreateEquipItemSkill::CheckUseItemMesh()
{
