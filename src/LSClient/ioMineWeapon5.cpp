...
ActionStopType ioMineWeapon5::m_ActionStopType = AST_NONE;
ActionStopType ioMineWeapon5::m_DieActionStopType = AST_NONE;

ioMineWeapon5::ioMineWeapon5( ioBaseChar *pOwner, const WeaponAttribute *pAttr )
...
ioMineWeapon5::~ioMineWeapon5()
{
	if( m_pMine )
	{
		LOG.PrintTimeAndLog( 0, "ioMineWeapon5::~ioMineWeapon5 - UnReleased Resource" );
	}
}

void ioMineWeapon5::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_dwDeadTime = FRAMEGETTIME() + m_dwMaxLifeTime;

}

void ioMineWeapon5::SetTeam( ioPlayStage *pStage, TeamType eType )
{
	if( !pStage ) return;
...
void ioMineWeapon5::SetStartPosition( const D3DXVECTOR3 &vPos, ioPlayStage *pStage )
{
	if( !pStage )
	{
		SetPosition( vPos );
...
void ioMineWeapon5::Process( float fTimePerSec, ioPlayStage *pStage )
{
	switch( m_State )
	{
	case MS_CHARGE:
	...
	if( FRAMEGETTIME() > m_dwDeadTime )
		SetWaitExplosionMine( pStage, false );
}

void ioMineWeapon5::OnCharge( ioPlayStage *pStage )
{
...
void ioMineWeapon5::OnSensing( ioPlayStage *pStage, float fTimePerSec )
{
	VisibleCheck( pStage );
	...
	//		if( pChar->IsEntityCollisionSkipState() ) continue;

	//		if( pChar->IsChatModeState( false ) )
	//			continue;

	//		if ( pChar->HasBuff( BT_WEAK_AIRBOMB ) )  continue;
	//		if( pChar->HasBuff(BT_CHANGE_TEAM) )	continue;
	...
	//// Mine Check
	if( pStage->CheckSensingMine5( this, m_fMineSensingRange ) )
	{
		SetWaitExplosionMine( pStage, true );
	}
	else
...
// 이부분 보이는거 체크
void ioMineWeapon5::VisibleCheck( ioPlayStage *pStage )
{
	if( !m_pMine )	return;
...
bool ioMineWeapon5::DropZoneCheck( ioPlayStage *pStage )
{
	D3DXVECTOR3 vCurPos = GetPosition();
	if( vCurPos.y <= 0.0f )
		return true;
...
void ioMineWeapon5::UpdateWorldCollisionBox()
{
...
void ioMineWeapon5::OnWaitExplosion( ioPlayStage *pStage )
{		
...
void ioMineWeapon5::SetWaitExplosionMine( ioPlayStage *pStage, bool bSendNetwork )
{
...
void ioMineWeapon5::SetNormalDeadMine( ioPlayStage *pStage, bool bSendNetwork )
{
	if( m_State == MS_WAIT_EXPLOSION ||
		m_State == MS_EXPLOSION ||
		m_State == MS_NORMAL_DEAD )
...
void ioMineWeapon5::ExplosionMine( ioPlayStage *pStage, bool bWait )
{
...
void ioMineWeapon5::NormalDeadMine( ioPlayStage *pStage, bool bWait )
{
	if( !IsCanExplosion() )	return;
...
void ioMineWeapon5::TranslateByConveyer( const D3DXVECTOR3 &vMoveAmt )
{
...
bool ioMineWeapon5::IsCanMineExplosion()
{
...
bool ioMineWeapon5::IsCanExplosion() const
{
...
void ioMineWeapon5::OnExplosion()
{
	if( m_bWait )
	{
		m_bWait = false;
...
void ioMineWeapon5::OnNormalDead()
{
	if( m_bWait )
	{
		m_bWait = false;
...
void ioMineWeapon5::DestroyResource( ioPlayStage *pStage )
{
	if( m_pMine )
	{
...
const ioOrientBox& ioMineWeapon5::GetLocalAttackBox() const
{
	return m_LocalAttackBox;
}

ioWeapon::WeaponType ioMineWeapon5::GetType() const
{
	return WT_MINE5;
}

ActionStopType ioMineWeapon5::GetActionStopType() const
{
	return m_ActionStopType;
}

ActionStopType ioMineWeapon5::GetDieActionStopType() const
{
	return m_DieActionStopType;
}

bool ioMineWeapon5::IsCanVisible( ioWeapon *pWeapon ) const
{
...
bool ioMineWeapon5::IsCollision( ioPlayEntity *pEntity, bool bMapCollision )
{
	if( m_State != MS_EXPLOSION )
		return false;
...
DWORD ioMineWeapon5::GetSensingStartGapTime()
{
...
DWORD ioMineWeapon5::GetCollisionEnableTime() const
{
	if( m_State == MS_EXPLOSION )
...
const BuffInfoList& ioMineWeapon5::GetTargetAirBuffList() const
{
...
const BuffInfoList& ioMineWeapon5::GetTargetBuffList() const
{
	if( !m_bUseTeamTargetBuff )
...
const BuffInfoList& ioMineWeapon5::GetTargetDownBuffList() const
{
	if( !m_bUseTeamTargetBuff )
...
void ioMineWeapon5::CreateMineEffect( float fTimePerSec, ioPlayStage *pStage )
{
	if( !pStage )
		return;
...
void ioMineWeapon5::ProcessPrisionEffect( float fTimePerSec, ioPlayStage *pStage )
{
	ioMapEffect* pEffect = pStage->FindMapEffect( m_MineEffectID );
	if( !pEffect )
		return;
