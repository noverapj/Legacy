...
ActionStopType ioPrisonWeapon2::m_ActionStopType = AST_NONE;
ActionStopType ioPrisonWeapon2::m_DieActionStopType = AST_NONE;

ioPrisonWeapon2::ioPrisonWeapon2( ioBaseChar *pOwner, const WeaponAttribute *pAttr )
...
ioPrisonWeapon2::~ioPrisonWeapon2()
{
	if( m_pFireMesh )
	{
...
void ioPrisonWeapon2::ClearData()
{
...
void ioPrisonWeapon2::LoadProperty( ioINILoader &rkLoader )
{
...
void ioPrisonWeapon2::LoadUpDownProperty( ioINILoader &rkLoader )
{
	char szBuf[MAX_PATH] = "";
...
bool ioPrisonWeapon2::InitResource()
{
...
bool ioPrisonWeapon2::SetFireMesh( const ioHashString &szFileName )
{
...
void ioPrisonWeapon2::SetStartPosition( const D3DXVECTOR3 &vPos, ioPlayStage *pStage )
{
...
void ioPrisonWeapon2::SetMoveDir( const D3DXVECTOR3 &vDir )
{
	m_vMoveDir = vDir;
...
void ioPrisonWeapon2::SetMoveRange( float fMoveRange )
{
	m_fMoveRange = fMoveRange;
}

void ioPrisonWeapon2::SetPrisionPos( const D3DXVECTOR3 &vPos )
{
	m_vPrisionPos = vPos;
}

D3DXVECTOR3 ioPrisonWeapon2::GetMoveDir()
{
	return m_vMoveDir;
}

D3DXVECTOR3 ioPrisonWeapon2::GetAttackDir( const ioPlayEntity *pEntity ) const
{
	if( pEntity )
	{
...
ioWeapon::WeaponType ioPrisonWeapon2::GetType() const
{
	return WT_PRISON_WEAPON2;
}

ActionStopType ioPrisonWeapon2::GetActionStopType() const
{
	return m_ActionStopType;
}

ActionStopType ioPrisonWeapon2::GetDieActionStopType() const
{
	return m_DieActionStopType;
}

void ioPrisonWeapon2::CreateDealyEffect( float fTimePerSec, ioPlayStage *pStage )
{
	if( !pStage )
		return;
...
void ioPrisonWeapon2::CreatePrisonEffect( float fTimePerSec, ioPlayStage *pStage )
{
	if( !pStage )
		return;
	...
	ioMapEffect* pMapEffect = NULL;
	if( GetOwner()->GetTeam() == TEAM_BLUE )
		pMapEffect = pStage->CreateMapEffect( m_szBluePrisonEffect, m_vPrisionPos, D3DXVECTOR3( FLOAT1, FLOAT1, FLOAT1 ) );
	else
		pMapEffect = pStage->CreateMapEffect( m_szRedPrisonEffect, m_vPrisionPos, D3DXVECTOR3( FLOAT1, FLOAT1, FLOAT1 ) );

	if( pMapEffect )
		m_PrisonEffectID = pMapEffect->GetUniqueID();
}

void ioPrisonWeapon2::CreateEndEffect( float fTimePerSec, ioPlayStage *pStage )
{	
	if( !pStage )
		return;

	pStage->CreateMapEffect( m_szEndEffect, m_vPrisionPos, D3DXVECTOR3( FLOAT1, FLOAT1, FLOAT1 ) );
}

void ioPrisonWeapon2::Process( float fTimePerSec, ioPlayStage *pStage )
{
...
void ioPrisonWeapon2::ProcessMove( float fTimePerSec, ioPlayStage *pStage )
{
	if( fTimePerSec == 0.0f )
		fTimePerSec = 0.001f;
...
void ioPrisonWeapon2::ProcessUpDown( float fTimePerSec, ioPlayStage *pStage )
{		
...
void ioPrisonWeapon2::ProcessPrisionEffect( float fTimePerSec, ioPlayStage *pStage )
{
...
void ioPrisonWeapon2::CheckTestCollisionBox()
{
	ioBaseChar *pOwner = GetOwner();
	if( !pOwner )
		return;
...
bool ioPrisonWeapon2::IsCollision( ioPlayEntity *pEntity, bool bMapCollision )
{
...
void ioPrisonWeapon2::SetWeaponDead()
{
	DestroyAttachAreaWeapon();
	m_bLive = false;	
}

void ioPrisonWeapon2::DestroyResource( ioPlayStage *pStage )
{
	if( pStage )
	{
