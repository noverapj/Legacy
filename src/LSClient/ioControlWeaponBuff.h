...
class ioControlWeaponBuff : public ioBuff
{
protected:
...
public:
	ioControlWeaponBuff();
	ioControlWeaponBuff( const ioControlWeaponBuff &rhs );
	virtual ~ioControlWeaponBuff();
};


inline ioControlWeaponBuff* ToControlWeaponBuff( ioBuff *pBuff )
{
	if( !pBuff || pBuff->GetTypeID() != BT_CONTROL_WEAPON )
		return NULL;

	return dynamic_cast< ioControlWeaponBuff* >( pBuff );
}
