...
ioCountRunningAttackSkill::ioCountRunningAttackSkill()
{
	ClearData();
...
ioCountRunningAttackSkill::ioCountRunningAttackSkill( const ioCountRunningAttackSkill &rhs )
: ioCountSkill( rhs ),
...
void ioCountRunningAttackSkill::ClearData()
{
...
void ioCountRunningAttackSkill::LoadProperty( ioINILoader &rkLoader )
{
...
ioSkill* ioCountRunningAttackSkill::Clone()
{
	return new ioCountRunningAttackSkill( *this );
}

bool ioCountRunningAttackSkill::IsProtected( int iDefenseBreakType ) const
{
...
bool ioCountRunningAttackSkill::IsEndState() const
{
	if( m_RunningState == RST_END )
		return true;
...
bool ioCountRunningAttackSkill::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
...
bool ioCountRunningAttackSkill::OnSkillStart( ioBaseChar *pChar )
{
	if( !HasSkillMotion() )
		return false;
...
void ioCountRunningAttackSkill::SetReadyState( ioBaseChar *pChar )
{
	if( !pChar ) return;
	...
	if( !m_bReduceGauge && !m_bReduceOnParent )
	{
...
void ioCountRunningAttackSkill::SetRunState( ioBaseChar *pChar )
{
	if( !m_bReduceGauge && !m_bReduceOnParent && m_SkillUseType != SUT_NORMAL )
	{
		ReduceNeedCount( m_iNeedCount );
		m_bReduceGauge = true;
	}
...
void ioCountRunningAttackSkill::AttackSkillFire( ioBaseChar *pChar )
{
	ioEntityGroup *pGrp = pChar->GetGroup();
...
void ioCountRunningAttackSkill::ProcessSkillExplicitFire( ioBaseChar *pChar )
{
...
void ioCountRunningAttackSkill::AttackSkillExplicitFire( ioBaseChar *pChar, int iWeaponIndex, D3DXVECTOR3 vPos ) 
{
...
void ioCountRunningAttackSkill::SetLastThrowState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioCountRunningAttackSkill::SetRunEndState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioCountRunningAttackSkill::SetLastThrowEndState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioCountRunningAttackSkill::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioCountRunningAttackSkill::OnSkillEnd( ioBaseChar *pChar )
{
...
void ioCountRunningAttackSkill::ApplyAttributeToWeapon( ioWeapon *pWeapon, ioPlayStage *pStage )
{
...
void ioCountRunningAttackSkill::CheckKeyInput( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioCountRunningAttackSkill::ProcessRotate( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioCountRunningAttackSkill::ApplyExtraSkillInfo( ioBaseChar *pChar, ioPlayStage *pStage, SP2Packet &rkPacket )
{
	int iType;
	rkPacket >> iType;
...
int ioCountRunningAttackSkill::CheckRunningActionType( ioBaseChar *pOwner )
{
...
void ioCountRunningAttackSkill::ProcessExtraAttack( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioCountRunningAttackSkill::SetInfoByApplyWeapon( ioBaseChar *pOwner, ioBaseChar *pTarget, DWORD dwWeaponBaseIndex, bool bBlock )
{
	if( !pOwner )
		return;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//추가 yong jun
void ioCountRunningAttackSkill::CheckAttackKey( ioBaseChar *pChar )
{
...
int ioCountRunningAttackSkill::ReduceNeedCount( int iNeedCount )
{
...
bool ioCountRunningAttackSkill::IsCanUseSkill( ioBaseChar *pChar, bool bHelpMsg )
{
