...
ioRunningAttackSkill3::ioRunningAttackSkill3()
{
	ClearData();
...
ioRunningAttackSkill3::ioRunningAttackSkill3( const ioRunningAttackSkill3 &rhs )
: ioAttackSkill( rhs ),
...
void ioRunningAttackSkill3::ClearData()
{
...
void ioRunningAttackSkill3::LoadProperty( ioINILoader &rkLoader )
{
...
ioSkill* ioRunningAttackSkill3::Clone()
{
	return new ioRunningAttackSkill3( *this );
}

bool ioRunningAttackSkill3::IsProtected( int iDefenseBreakType ) const
{
...
bool ioRunningAttackSkill3::IsEndState() const
{
...
bool ioRunningAttackSkill3::IsAttackEndState() const
{	
...
bool ioRunningAttackSkill3::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
	if( m_dwPreDelayStartTime > 0 )
		return false;
...
bool ioRunningAttackSkill3::OnSkillStart( ioBaseChar *pChar )
{
	if( !HasSkillMotion() )
		return false;
...
void ioRunningAttackSkill3::SetRunState( ioBaseChar *pChar )
{
...
void ioRunningAttackSkill3::AttackSkillFire( ioBaseChar *pChar )
{
	ioEntityGroup *pGrp = pChar->GetGroup();
...
void ioRunningAttackSkill3::SetForceEndState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioRunningAttackSkill3::SetStunEndState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioRunningAttackSkill3::SetEndState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioRunningAttackSkill3::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	DWORD dwCurTime = FRAMEGETTIME();
	...
	case RST_RUN:
		{		
			////////////////////////////////////////////////////////////////////////////////////////////////////////////
			//타이탄 장신구 동기화 튀는거 예방
			if( m_ThisSkillName == SKN_TITAN && !m_bTitanWeaponChecked && m_dwFireStartTime < dwCurTime )
			{
				m_bTitanWeaponChecked = true;

				ioWeapon* pWeapon = g_WeaponMgr.FindWeapon( pChar, m_dwWeaponBaseIndex );
				ioAdhesiveMissileWeapon4* pWeapon4 = ToAdhesiveMissileWeapon4( pWeapon );
				if( pWeapon4 )
					pWeapon4->SetUseColCheck( true );
			}
			////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void ioRunningAttackSkill3::OnSkillEnd( ioBaseChar *pChar )
{
	// 기본무기 장착
	if( m_bUseItemMesh )
	{
...
void ioRunningAttackSkill3::ApplyAttributeToWeapon( ioWeapon *pWeapon, ioPlayStage *pStage )
{
...
bool ioRunningAttackSkill3::CheckSkillEndKey( ioBaseChar *pOwner )
{
...
void ioRunningAttackSkill3::CheckSkillEnd( ioBaseChar *pOwner, RunEndType endType )
{
...
void ioRunningAttackSkill3::CheckKeyInput( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioRunningAttackSkill3::ProcessRotate( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioRunningAttackSkill3::ProcessCharRotate( ioBaseChar *pChar )
{
...
void ioRunningAttackSkill3::ApplyExtraSkillInfo( ioBaseChar *pChar, ioPlayStage *pStage, SP2Packet &rkPacket )
{
	int iType;
	rkPacket >> iType;
...
void ioRunningAttackSkill3::SetCreateWeaponDependency( ioBaseChar *pOwner, ioPlayStage *pStage )
{
	if( !pOwner ) return;
	if( !pStage ) return;
...
SkillEndJumpType ioRunningAttackSkill3::GetSkillEndNextJumpMustEnable()
{
	return m_JumpEndType;
}

bool ioRunningAttackSkill3::CheckControlWeapon( ioBaseChar *pChar, DWORD dwCurTime )
{
...
bool ioRunningAttackSkill3::IsNoDropState() const
{
