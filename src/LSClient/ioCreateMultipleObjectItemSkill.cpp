...
ioCreateMultipleObjectItemSkill::ioCreateMultipleObjectItemSkill(void)
{
...
ioCreateMultipleObjectItemSkill::ioCreateMultipleObjectItemSkill( const ioCreateMultipleObjectItemSkill &rhs )
	: ioNormalSkill( rhs ),
	...
	m_vDummyOffsetList( rhs.m_vDummyOffsetList )
{
	m_CreateState = RS_NONE;
...
void ioCreateMultipleObjectItemSkill::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_vDummyOffsetList.clear();
	m_vDummyOffsetList.reserve( iCnt );
	...
	for( int i=0; i < iCnt; ++i )
	{
		...
		D3DXVECTOR3	vOffset;
		wsprintf_e( szKey, "dummy%d_offset_x", i + 1 );
		vOffset.x = rkLoader.LoadFloat( szKey, 0.0f );
		wsprintf_e( szKey, "dummy%d_offset_y", i + 1 );
		vOffset.y = rkLoader.LoadFloat( szKey, 0.0f );
		wsprintf_e( szKey, "dummy%d_offset_z", i + 1 );
		vOffset.z = rkLoader.LoadFloat( szKey, 0.0f );

		m_vDummyOffsetList.push_back( vOffset );
	}

	m_bImmediatelyEquip = rkLoader.LoadBool_e( "immediately_equip", false );
}

ioSkill* ioCreateMultipleObjectItemSkill::Clone()
{
	return new ioCreateMultipleObjectItemSkill( *this );
}

bool ioCreateMultipleObjectItemSkill::IsProtected( int iDefenseBreakType ) const
{
...
bool ioCreateMultipleObjectItemSkill::IsCanUseSkill( ioBaseChar *pChar, bool bHelpMsg )
{
...
bool ioCreateMultipleObjectItemSkill::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
	if( m_CreateState != RS_END )
		return false;
...
bool ioCreateMultipleObjectItemSkill::OnSkillStart( ioBaseChar *pChar )
{
	if( !HasSkillMotion() )
		return false;
...
void ioCreateMultipleObjectItemSkill::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
...
void ioCreateMultipleObjectItemSkill::OnSkillEnd( ioBaseChar *pOwner )
{
	// 기본무기 장착
...
void ioCreateMultipleObjectItemSkill::StartPreDelayEffect( ioBaseChar *pChar )
{
	ioSkill::StartPreDelayEffect( pChar );
}

void ioCreateMultipleObjectItemSkill::EndPreDelayEffect( ioBaseChar *pChar )
{
	ioSkill::EndPreDelayEffect( pChar );
}

void ioCreateMultipleObjectItemSkill::SetActionState( ioBaseChar *pChar )
{
...
void ioCreateMultipleObjectItemSkill::SetLoopState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	...
	{
		vCreatePos = pChar->GetWorldPosition() + pChar->GetWorldOrientation() * m_vDummyOffsetList[i];
		dwCreateItemCode = m_vObjectItemList[i];
...
void ioCreateMultipleObjectItemSkill::SetEndState( ioBaseChar *pChar )
{
	ioEntityGroup *pGrp = pChar->GetGroup();
...
bool ioCreateMultipleObjectItemSkill::CheckUseItemMesh()
{
