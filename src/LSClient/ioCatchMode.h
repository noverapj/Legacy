...
class ioCatchMode : public ioPlayMode
{
protected:
...
public:
	ioCatchMode( ioPlayStage *pCreator );
	virtual ~ioCatchMode();
};

inline ioCatchMode* ToCatchMode( ioPlayMode *pMode )
{
	if( !pMode || pMode->GetModeType() != MT_CATCH )
		return NULL;

	return dynamic_cast< ioCatchMode* >( pMode );
}
