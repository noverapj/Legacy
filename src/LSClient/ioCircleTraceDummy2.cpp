...
ioCircleTraceDummy2::ioCircleTraceDummy2( ioEntityGroup *pGrp, ioPlayMode *pMode ):
...
ioCircleTraceDummy2::~ioCircleTraceDummy2(void)
{
}

void ioCircleTraceDummy2::LoadProperty( ioINILoader &rkLoader )
{
	char szBuf[MAX_PATH];
	char szKey[MAX_PATH];
	...
	int nDummyAttckCnt = rkLoader.LoadInt_e( "attack_cnt",  0 );
	for( int i = 0; i < nDummyAttckCnt; i++ )
	{
		WeaponInfo DummyAttack;

		wsprintf_e( szKey, "attack_type%d", i+1 );
		DummyAttack.m_iWeaponIdx = rkLoader.LoadInt( szKey, NO_WEAPON_ATTRIBUTE );

		wsprintf_e( szKey, "attack_type%d_resist", i+1 );
		DummyAttack.m_iResistanceIdx = rkLoader.LoadInt( szKey, NO_WEAPON_ATTRIBUTE );

		wsprintf_e( szKey, "attack_type%d_wounded", i+1 );
		rkLoader.LoadString( szKey, "", szBuf, MAX_PATH );
		DummyAttack.m_WoundedAnimation = szBuf;

		wsprintf_e( szKey, "attack_type%d_wounded_time", i+1 );
		DummyAttack.m_dwWoundedDuration = rkLoader.LoadInt( szKey, 0 );

		wsprintf_e( szKey, "attack_type%d_wounded_loop_ani", i+1 );
		DummyAttack.m_bWoundedLoopAni = rkLoader.LoadBool( szKey, false );

		m_DummyAttackList.push_back( DummyAttack );
	}
...
void ioCircleTraceDummy2::AddCirclePosition( D3DXVECTOR3 vPos )
{
...
void ioCircleTraceDummy2::ProcessState( float fTimerPerSec, DWORD dwPreTime )
{
...
void ioCircleTraceDummy2::ProcessStart( ioBaseChar *pOwner, ioPlayStage *pStage )
{
	if ( !pOwner || !pStage )
		return;
...
void ioCircleTraceDummy2::SetState( CircleTraceDummyState eState )
{
...
void ioCircleTraceDummy2::ProcessMoveState( ioBaseChar *pOwner, ioPlayStage *pStage, float fTimerPerSec )
{
...
void ioCircleTraceDummy2::SetNextTargetCircle( bool bSend )
{
	if ( m_CirclePositionList.empty() )
		return;
...
void ioCircleTraceDummy2::ProcessSensingAttack( ioBaseChar *pOwner, ioPlayStage *pStage, float fTimerPerSec )
{
	if ( !pOwner || !pStage )
		return;
...
void ioCircleTraceDummy2::SearchTarget( ioBaseChar *pOwner, ioPlayStage *pStage, ioHashStringVec& vTargetList )
{
	if ( !pOwner || !pStage )
		return;
...
bool ioCircleTraceDummy2::CheckTargetState( ioBaseChar *pOwner, ioBaseChar *pTarget )
{
	if ( !pOwner || !pTarget )
		return true;
...
bool ioCircleTraceDummy2::CheckSkipBuff( ioBaseChar *pChar )
{
	if ( !pChar )
		return false;
...
void ioCircleTraceDummy2::CheckTargetWeaponFire( ioBaseChar *pOwner, ioPlayStage *pStage, const ioHashStringVec& vTargetList )
{
	if ( !pOwner )
		return;
...
void ioCircleTraceDummy2::CreateWeapon( ioBaseChar *pOwner, ioPlayStage *pStage, ioHashString szTargetName )
{
	if ( !pOwner || !pStage || szTargetName.IsEmpty() )
		return;

	if( m_nWeaponIndex >= (int)m_DummyAttackList.size() )
		m_nWeaponIndex = 0;

	if ( m_DummyAttackList[m_nWeaponIndex].m_iWeaponIdx == NO_WEAPON_ATTRIBUTE )
		return;
	...
	kFireTime.iAttributeIdx = m_DummyAttackList[m_nWeaponIndex].m_iWeaponIdx;
	kFireTime.iResistanceIdx = m_DummyAttackList[m_nWeaponIndex].m_iResistanceIdx;
	kFireTime.szWoundedAni  = m_DummyAttackList[m_nWeaponIndex].m_WoundedAnimation;
	kFireTime.dwWoundedAniDuration = m_DummyAttackList[m_nWeaponIndex].m_dwWoundedDuration;
	kFireTime.bLoopWoundedAni = m_DummyAttackList[m_nWeaponIndex].m_bWoundedLoopAni;
	kFireTime.dwWeaponIdx = dwWeaponBaseIndex++;
	kFireTime.eFireTimeType = FTT_NORMAL_ATTACK;
	D3DXVECTOR3 vCurDir = pTarget->GetMidPositionByRate() - GetWorldPosition();
...
void ioCircleTraceDummy2::ApplyDummyCharSync( ioPlayStage *pStage, SP2Packet &rkPacket )
{
	if( !pStage ) 
		return;
...
void ioCircleTraceDummy2::SetDieState( bool bEffect, bool bWoundedDie /*= false */ )
{
...
void ioCircleTraceDummy2::FillDummyCharMidSync( ioBaseChar* pOwner, SP2Packet &rkPacket )
{
	rkPacket << m_nWeaponIndex;
}

void ioCircleTraceDummy2::ApplyDummyCharMidSync( ioBaseChar* pOwner, SP2Packet &rkPacket )
{
	rkPacket >> m_nWeaponIndex;
}