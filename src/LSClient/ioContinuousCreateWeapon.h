...
class ioContinuousCreateWeapon : public ioWeapon
{
public:
	enum DoubleIceState
	{
...
public:
	ioContinuousCreateWeapon( ioBaseChar *pOwner, const WeaponAttribute *pAttr );
	virtual ~ioContinuousCreateWeapon();
};

inline ioContinuousCreateWeapon* ToContinuousCreateWeapon( ioWeapon *pWeapon )
{
	if( !pWeapon || pWeapon->GetType() != ioWeapon::WT_CONTINIOUS_CREATE )
		return NULL;

	return dynamic_cast< ioContinuousCreateWeapon* >( pWeapon );
}
