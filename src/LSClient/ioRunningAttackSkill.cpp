...
ioRunningAttackSkill::ioRunningAttackSkill()
{
	ClearData();
...
ioRunningAttackSkill::ioRunningAttackSkill( const ioRunningAttackSkill &rhs )
: ioAttackSkill( rhs ),
...
void ioRunningAttackSkill::ClearData()
{
...
void ioRunningAttackSkill::LoadProperty( ioINILoader &rkLoader )
{
...
ioSkill* ioRunningAttackSkill::Clone()
{
	return new ioRunningAttackSkill( *this );
}

bool ioRunningAttackSkill::IsProtected( int iDefenseBreakType ) const
{
...
bool ioRunningAttackSkill::IsEndState() const
{
	if( m_RunningState == RST_END )
		return true;
...
bool ioRunningAttackSkill::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
...
bool ioRunningAttackSkill::OnSkillStart( ioBaseChar *pChar )
{
	if( !HasSkillMotion() )
		return false;
...
void ioRunningAttackSkill::SetReadyState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioRunningAttackSkill::SetRunState( ioBaseChar *pChar )
{
...
void ioRunningAttackSkill::AttackSkillFire( ioBaseChar *pChar )
{
	ioEntityGroup *pGrp = pChar->GetGroup();
...
void ioRunningAttackSkill::ProcessSkillExplicitFire( ioBaseChar *pChar )
{
...
void ioRunningAttackSkill::AttackSkillExplicitFire( ioBaseChar *pChar, int iWeaponIndex, D3DXVECTOR3 vPos ) 
{
...
void ioRunningAttackSkill::SetLastThrowState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioRunningAttackSkill::SetRunEndState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioRunningAttackSkill::SetLastThrowEndState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioRunningAttackSkill::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioRunningAttackSkill::OnSkillEnd( ioBaseChar *pChar )
{
	// 기본무기 장착
	if( m_bUseItemMesh )
	{
...
void ioRunningAttackSkill::ApplyAttributeToWeapon( ioWeapon *pWeapon, ioPlayStage *pStage )
{
...
void ioRunningAttackSkill::CheckKeyInput( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioRunningAttackSkill::ProcessRotate( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioRunningAttackSkill::ApplyExtraSkillInfo( ioBaseChar *pChar, ioPlayStage *pStage, SP2Packet &rkPacket )
{
	int iType;
	rkPacket >> iType;
...
int ioRunningAttackSkill::CheckRunningActionType( ioBaseChar *pOwner )
{
...
void ioRunningAttackSkill::ProcessExtraAttack( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioRunningAttackSkill::SetInfoByApplyWeapon( ioBaseChar *pOwner, ioBaseChar *pTarget, DWORD dwWeaponBaseIndex, bool bBlock )
{
	if( !pOwner )
		return;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//추가 yong jun
void ioRunningAttackSkill::CheckAttackKey( ioBaseChar *pChar )
{
