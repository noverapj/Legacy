...
ioMachineCannon::ioMachineCannon( ioEntityGroup *pGrp, ioPlayMode *pMode )
...
ioMachineCannon::~ioMachineCannon()
{
}

void ioMachineCannon::LoadProperty( ioINILoader &rkLoader )
{
...
MachineStructType ioMachineCannon::GetType() const
{
	return MCT_CANNON;
}

void ioMachineCannon::InitMachineStructInfo( int iStructNum, int iArrayNum, DWORD dwCreateGapTime, DWORD dwSeed,
...
bool ioMachineCannon::IsTakeEnableState()
{
...
void ioMachineCannon::SetTakeState( const ioHashString &szTakeCharName )
{
...
void ioMachineCannon::ReleaseState()
{
...
void ioMachineCannon::SetStructDie( ioBaseChar *pChar, bool bEffect )
{
	m_CannonState = MCS_NONE;
...
void ioMachineCannon::StructDie( bool bEffect )
{
	m_CannonState = MCS_NONE;
...
void ioMachineCannon::ProcessState( float fTimePerSec )
{
...
void ioMachineCannon::ProcessNone( float fTimePerSec )
{
}

void ioMachineCannon::ProcessDelay( float fTimePerSec )
{
	ioPlayStage *pPlayStage = g_MachineStructMgr.GetPlayStage();
	if( !pPlayStage ) return;
...
void ioMachineCannon::ChangeToFireState()
{
	ioPlayStage *pPlayStage = g_MachineStructMgr.GetPlayStage();
	if( !pPlayStage ) return;
	...
	m_dwChangeStateTime = FRAMEGETTIME();
...
void ioMachineCannon::ProcessFire( float fTimePerSec )
{
	ioPlayStage *pPlayStage = g_MachineStructMgr.GetPlayStage();
	if( !pPlayStage ) return;
	...
	if( m_dwFireTime > 0 && m_dwFireTime < dwCurTime )
	{
		...
		D3DXVec3Cross( &vSideDir, &ioMath::UNIT_Y, &vCurDir );
		D3DXVec3Normalize( &vSideDir, &vSideDir );

		D3DXVECTOR3 vCurPos = GetMidPositionByRate();
		vCurPos = vCurPos + (vCurDir * m_fWeaponOffSet);
		vCurPos = vCurPos + (vSideDir * m_fWeaponSideOffSet);
		vCurPos.y += m_fWeaponHeightOffSet;
...
void ioMachineCannon::ProcessRotate( ioBaseChar *pOwner )
{
...
void ioMachineCannon::CheckKeyInput( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
	if( !pOwner->IsNeedProcess() )
		return;
...
void ioMachineCannon::ApplyMachineStructExtraInfo( SP2Packet &rkPacket )
{
	if( m_CannonState == MCS_NONE )
		return;
...
void ioMachineCannon::ApplyMachineState( SP2Packet &rkPacket )
{
	ioPlayStage *pPlayStage = g_MachineStructMgr.GetPlayStage();
	if( !pPlayStage ) return;
...
bool ioMachineCannon::IsCollisionAvailableWeapon( ioWeapon *pWeapon )
{
...
bool ioMachineCannon::IsEntityCollisionSkipState( ioGameEntity* pColEntity ) const
{
	return false;
}

ApplyWeaponExp ioMachineCannon::ApplyWeapon( ioWeapon *pWeapon )
{
