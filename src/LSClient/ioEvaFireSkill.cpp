...
#include "ioEvaFireSkill.h"
#include "ioGrowthUpInfo.h"
#include "WeaponDefine.h"
#include "ItemDefine.h"

ioEvaFireSkill::ioEvaFireSkill()
{
	ClearData();
}

ioEvaFireSkill::ioEvaFireSkill( const ioEvaFireSkill &rhs )
	: ioAttackSkill( rhs ),
	...
	m_vEvaAimInfoList( rhs.m_vEvaAimInfoList ),
	...
	m_bAllFireState(rhs.m_bAllFireState),
	m_iAimMaxCnt(rhs.m_iAimMaxCnt),
	m_bCheckEnableSkill(rhs.m_bCheckEnableSkill)
{
	ClearData();
}
...
void ioEvaFireSkill::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_iAimMaxCnt = rkLoader.LoadInt_e( "aim_info_count", 0 );
	m_vEvaAimInfoList.clear();
	m_vEvaAimInfoList.reserve( m_iAimMaxCnt );
	for( int i=0; i<m_iAimMaxCnt; ++i )
	{
		EvaAimInfo kInfo;
		kInfo.Init();
		...
		m_vEvaAimInfoList.push_back(kInfo);
	}
	...
	m_bAllFireState = rkLoader.LoadBool_e( "all_fire_state", false );

	// 스킬 활성화/비활성화 체크
	m_bCheckEnableSkill = rkLoader.LoadBool_e( "check_enable_skill", false );
}

void ioEvaFireSkill::LoadWeaponInfo( const char *szWeaponInfo, WeaponInfoList &rkWeaponInfoList, ioINILoader &rkLoader )
{
...
ioSkill* ioEvaFireSkill::Clone()
{
	return new ioEvaFireSkill( *this );
}


void ioEvaFireSkill::CalcCoolTimeValue( ioBaseChar *pChar )
{
	if( pChar == NULL )
	{
...
bool ioEvaFireSkill::OnSkillStart( ioBaseChar *pChar )
{
	if( !HasSkillMotion() )
	{
		return false;
...
void ioEvaFireSkill::OnProcessState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	...
	case SS_WAIT:
		{			
			...
			if( pChar->IsNeedProcess()  )
			{
				...
				if( pChar->IsAttackKey() && !m_bAllFireState )
				{
	...
	case SS_FIRE:
		{
			...
			if( pTarget )
			{
				ioMapEffect *pMapEffect = pStage->FindMapEffect( m_vEvaAimInfoList[m_iAimCombo].m_dwZeroAimMarkID );
				if( pMapEffect )
					pMapEffect->SetWorldPosition( pTarget->GetMidPositionByRate() );
			}
			else
				pStage->DestroyMapEffect( m_vEvaAimInfoList[m_iAimCombo].m_dwZeroAimMarkID );
...
void ioEvaFireSkill::CheckVDir( ioBaseChar *pChar )
{
	...
	{
		LOG.PrintTimeAndLog( 0, "ioEvaFireSkill::CheckVDir - Motion Error" );
		return;
	}
...
void ioEvaFireSkill::OnSkillEnd( ioBaseChar *pChar )
{
	// 기본무기 장착
	if( m_bUseItemMesh )
	...
	// 모든 스킬이 끝나면 서브 웨폰도 삭제한다.
	ioItem *pItem = pChar->GetEquipedItem(ES_WEAPON);
	ioEvaItem *pEvaItem = ToEvaItem( pItem );
	if( pEvaItem )
	{
		pEvaItem->EndWeaponEffec( pChar );
	}
}

bool ioEvaFireSkill::IsSkillMotionEnd( DWORD dwActionStop, ioBaseChar* pOwner ) const
{
...
void ioEvaFireSkill::UpdateReduceGauge( ioBaseChar *pOwner )
{
...
bool ioEvaFireSkill::IsEnableReserve() const
{
	if( m_dwPreDelayStartTime != 0 )
		return false;
...
void ioEvaFireSkill::ApplyExtraSkillInfo( ioBaseChar *pChar, ioPlayStage *pStage, SP2Packet &rkPacket )
{
	int iState;
	rkPacket >> iState;
...
void ioEvaFireSkill::ApplyAttributeToWeapon( ioWeapon *pWeapon, ioPlayStage *pStage )
{
	if( !pWeapon ) return;
...
void ioEvaFireSkill::SetLoopState( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pChar ) return;
	ioEntityGroup *pGrp = pChar->GetGroup();
	if( !pGrp ) return;
...
void ioEvaFireSkill::SetFireState( ioBaseChar *pChar )
{
	if( !pChar ) return;
...
void ioEvaFireSkill::AttackSkillFire( ioBaseChar *pChar )
{
	ioEntityGroup *pGrp = pChar->GetGroup();
	...
	vWeaponInfoList = m_vEvaAimInfoList[m_iAimCombo].m_AimWeaponInfoList;
...
bool ioEvaFireSkill::CheckUseItemMesh()
{
	return m_bUseItemMesh;
}

void ioEvaFireSkill::CheckAimTime()
{
	DWORD dwCurTime = FRAMEGETTIME();
	
	if( m_dwAimCheckTime > 0 &&
		m_dwAimCheckTime + m_vEvaAimInfoList[m_iAimCombo].m_dwAimTime < dwCurTime )
	{
		m_dwAimCheckTime = 0;	
	}
}

void ioEvaFireSkill::CheckKeyInput( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
bool ioEvaFireSkill::IsEnableTargetState( ioBaseChar *pTarget )
{
	if( !pTarget )
		return false;
...
void ioEvaFireSkill::CheckTarget( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pStage )	return;
	if( !pChar )	return;
	...
	m_vEvaTargetInfoList.clear();
	int iSize = rkTargetList.size();
	m_vEvaTargetInfoList.reserve(iSize);
	...
	for( iter = rkTargetList.begin(); iter != rkTargetList.end(); ++iter )
	{
		...
		if( fDiffRangeSq < fTargetRangeSq )
		{
			...
			m_vEvaTargetInfoList.push_back(kInfo);
		}
	}

	if( m_szTargetName.IsEmpty() )
	{
		int iCnt = m_vEvaTargetInfoList.size();
		for( int i=0; i<iCnt; ++i )
		{
			float fDistSq = m_vEvaTargetInfoList[i].m_fLenth;
			if( fDistSq < fTargetRangeSq )	// Enemy
			{
				fTargetRangeSq = fDistSq;
				m_szTargetName = m_vEvaTargetInfoList[i].m_TargetName;
			}
		}
	...
	else if( !m_vEvaTargetInfoList.empty() )
	{
		int iCurTarget = -1;
		...
		for( int i=0; i<iCnt; ++i )
		{
			if( m_szTargetName == m_vEvaTargetInfoList[i].m_TargetName )
				iCurTarget = i;
		}
		...
		if( iCurTarget != -1 )
		{
			...
			m_szTargetName = m_vEvaTargetInfoList[iCurTarget].m_TargetName;
...
void ioEvaFireSkill::UpdateAimMark( ioBaseChar *pChar, ioPlayStage *pStage )
{
	if( !pChar ) return;
	...
	if( m_dwAimCheckTime > 0 )
	{
		...
		if( m_vEvaAimInfoList[m_iAimCombo].m_dwAimMarkID == -1 )
		{			
			ioMapEffect *pMapEffect = pStage->CreateMapEffect( m_vEvaAimInfoList[m_iAimCombo].m_szAimMark, vPos, vScale );
			if( pMapEffect )
				m_vEvaAimInfoList[m_iAimCombo].m_dwAimMarkID = pMapEffect->GetUniqueID(); 
		}
		else
		if( m_vEvaAimInfoList[m_iAimCombo].m_dwAimMarkID != -1 )
		{
			ioMapEffect *pMapEffect = pStage->FindMapEffect( m_vEvaAimInfoList[m_iAimCombo].m_dwAimMarkID );
			if( pMapEffect )
				pMapEffect->SetWorldPosition( vPos );
		}

		if( m_vEvaAimInfoList[m_iAimCombo].m_dwZeroAimMarkID != -1 )
		{
			pStage->DestroyMapEffect( m_vEvaAimInfoList[m_iAimCombo].m_dwZeroAimMarkID );
			m_vEvaAimInfoList[m_iAimCombo].m_dwZeroAimMarkID = -1;
		}
	}
}

void ioEvaFireSkill::DestroyAimMark( ioBaseChar *pChar )
{
	if( !pChar )	return;
	...
	pChar->DestroyMapEffect( m_vEvaAimInfoList[m_iAimCombo].m_dwAimMarkID );
	m_vEvaAimInfoList[m_iAimCombo].m_dwAimMarkID = -1;

	pChar->DestroyMapEffect( m_vEvaAimInfoList[m_iAimCombo].m_dwZeroAimMarkID );
	m_vEvaAimInfoList[m_iAimCombo].m_dwZeroAimMarkID = -1;
}

bool ioEvaFireSkill::IsProtected( int iDefenseBreakType ) const
{
...
bool ioEvaFireSkill::IsCheckEnableSkill( int iSlot, ioBaseChar *pChar )
{
	if( !pChar )				{	return true;	}
	if( !m_bCheckEnableSkill )	{	return true;	}
	if( iSlot != ES_WEAPON )	{	return true;	}

	// 서브웨폰이 5개일 때 사용 가능 체크 	
	ioItem *pItem = pChar->GetEquipedItem(ES_WEAPON);
	ioEvaItem *pEvaItem = ToEvaItem( pItem );
	if( pEvaItem && pEvaItem->IsSubWeaponAttackState() )
	{
		return true;
	}
