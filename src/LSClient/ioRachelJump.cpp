...
ioRachelJump::ioRachelJump( const ioRachelJump &rhs )
: ioExtendJump( rhs ),
...
ioExtendJump* ioRachelJump::Clone()
{
	return new ioRachelJump( *this );
}
...
JumpType ioRachelJump::GetType() const
{
	return JT_RACHEL_JUMP;
}

void ioRachelJump::LoadProperty( ioINILoader &rkLoader )
{
...
void ioRachelJump::LoadComboList( ioINILoader &rkLoader )
{
	char szBuf[MAX_PATH];
...
void ioRachelJump::SetJumpping( ioBaseChar *pOwner, bool bFullTime )
{
	m_ChargeComboState = CCS_NONE;
...
void ioRachelJump::ProcessJumpping( ioBaseChar *pOwner, float fHeightGap, float fNeedGauge )
{
	if( !pOwner )
		return;
...
// combo
void ioRachelJump::CheckComboJumpAttack( ioBaseChar *pOwner )
{
...
void ioRachelJump::SetComboJumpAttack( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
// double
void ioRachelJump::SetDoubleJump( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
bool ioRachelJump::IsCanDoubleJump( ioBaseChar *pOwner, float fHeightGap )
{
...
void ioRachelJump::SetDoubleJumpAni( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
// jump dash
bool ioRachelJump::IsCanJumpDash( ioBaseChar *pOwner, float fHeightGap )
{
	if( !pOwner ) return false;
...
int ioRachelJump::GetJumpDashAni( ioBaseChar *pOwner )
{
	ioEntityGroup *pGrp = pOwner->GetGroup();
	D3DXVECTOR3 vSightDir = pOwner->GetMoveDir();
...
bool ioRachelJump::IsJumpDashState()
{
...
bool ioRachelJump::IsJumpAttackReserve()
{
	return m_bJumpAttackReserve;
}

void ioRachelJump::CheckKeyReserve( ioBaseChar *pOwner )
{
...
void ioRachelJump::ProcessJumpAttack( ioBaseChar *pOwner, float fHeightGap )
{
	switch( m_ChargeComboState )
	{
	case CCS_NORMAL:
...
void ioRachelJump::ProcessComboJumpAttack( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioRachelJump::ProcessJumpDash( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioRachelJump::ApplyJump( ioBaseChar *pOwner, SP2Packet &rkPacket )
{
	if( !pOwner ) return;
...
void ioRachelJump::SetLandingState( ioBaseChar *pOwner )
{
	m_bUseExtendJump = false;
	m_ChargeComboState = CCS_NONE;
...
bool ioRachelJump::IsNoProcessGravity()
{
	switch( m_ChargeComboState )
	{
	case CCS_GATHER:
...
float ioRachelJump::GetExtraJumpSpeedRate( const D3DXVECTOR3 &vDir, const D3DXQUATERNION &qtRot ) const
{
...
bool ioRachelJump::IsCanJumpingSkill( ioBaseChar *pOwner )
{
	switch( m_ChargeComboState )
	{
	case CCS_DASH:
...
DWORD ioRachelJump::GetChargingTime( ioBaseChar *pChar )
{
...
void ioRachelJump::SetChargingState( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioRachelJump::SetGatherJump( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
void ioRachelJump::ChangeToGatherJumpAttack( ioBaseChar *pOwner )
{
	ioEntityGroup *pGrp = pOwner->GetGroup();
	if( !pGrp ) return;
...
void ioRachelJump::ProcessCharging( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
	if( !pOwner->IsNeedProcess() )
		return;
...
void ioRachelJump::ProcessGather( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
	if( !pOwner->IsNeedProcess() )
		return;
...
void ioRachelJump::ProcessGatherAttack( ioBaseChar *pOwner )
{
	ProcessAirJump( pOwner );
}

void ioRachelJump::ProcessGatherMaxAttack( ioBaseChar *pOwner )
{
	ProcessAirJump( pOwner );
}

void ioRachelJump::SetGatherMax( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
	...
	int iAniID = pGrp->GetAnimationIdx( m_ExtendMaxAttack.m_AttackAnimation );
	float fTimeRate = m_ExtendMaxAttack.m_fAttackAniRate;
	DWORD dwPreDelay = m_ExtendMaxAttack.m_dwPreDelay;
	...
	pGrp->SetActionAni( iAniID, FLOAT100, FLOAT100, FLOAT1, FLOAT1/fTimeRate, 0.0f, dwPreDelay, true );
	pOwner->RefreshFireTimeList( iAniID, m_ExtendMaxAttack.m_vWeaponInfoList, FTT_JUMP_ATTACK, fTimeRate, dwPreDelay );
	pOwner->IncreaseWeaponIndexBase();

	pOwner->CheckCharColSkipTime( iAniID, fTimeRate, dwPreDelay );
	pOwner->SetReservedSliding( m_ExtendMaxAttack.m_vForceInfoList, iAniID, fTimeRate, dwPreDelay );
...
int ioRachelJump::GetCurChargeComboState()
{
	return m_ChargeComboState;
}

void ioRachelJump::SetDoubleJumpAttack( ioBaseChar *pOwner )
{
	if( !pOwner )
		return;
...
void ioRachelJump::ProcessAirJump( ioBaseChar *pOwner )
{
	if( !pOwner ) return;
...
bool ioRachelJump::IsCanDoubleJumpAttack()
{
	return m_bUsedDoubleJump;
}

void ioRachelJump::SetNewDoubleJumpAttack( ioBaseChar *pOwner, float fGravityAmt )
{
	if( !pOwner ) return;
