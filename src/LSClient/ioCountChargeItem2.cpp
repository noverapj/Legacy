...
ioCountChargeItem2::ioCountChargeItem2()
{
...
ioCountChargeItem2::ioCountChargeItem2( const ioCountChargeItem2 &rhs )
: ioWeaponItem( rhs ),
...
void ioCountChargeItem2::LoadProperty( ioINILoader &rkLoader )
{
...
ioItem* ioCountChargeItem2::Clone()
{
	return new ioCountChargeItem2( *this );
}

ioWeaponItem::WeaponSubType ioCountChargeItem2::GetSubType() const
{
	return WST_COUNT_CHARGE2;
}

void ioCountChargeItem2::ClearState()
{
	m_ChargeState = CS_NONE;
...
void ioCountChargeItem2::SetNormalAttack( ioBaseChar *pOwner, int iCurCombo )
{
	if( !pOwner ) return;
...
void ioCountChargeItem2::CheckNormalAttack( ioBaseChar *pOwner )
{
...
void ioCountChargeItem2::OnCharging( ioBaseChar *pOwner )
{
...
void ioCountChargeItem2::OnGathering( ioBaseChar *pOwner )
{
	if( !pOwner->IsNeedProcess() )	return;
...
void ioCountChargeItem2::OnChargeFire( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
	DWORD dwLooseTime = 0;
	if( pOwner )
...
void ioCountChargeItem2::OnFireEnd( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioCountChargeItem2::OnRetreatState( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioCountChargeItem2::ChangeToGathering( ioBaseChar *pOwner )
{
	ioEntityGroup *pGrp = pOwner->GetGroup();
	pGrp->ClearAllActionAni( FLOAT100, true );
	pGrp->ClearAllLoopAni( FLOAT100, true );
...
void ioCountChargeItem2::ChangeToNormalAttack( ioBaseChar *pOwner )
{
	pOwner->SetAttackMoveEnable( false );
	pOwner->SetChargingState( false );
	m_bSetChargingMoveAni = false;
...
void ioCountChargeItem2::ChangeToAttackFire( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioCountChargeItem2::ChangeToRetreatState( ioBaseChar *pOwner, const ioHashString &szAni, float fAniRate, D3DXVECTOR3 vDir )
{
	if( !pOwner ) return;
	ioEntityGroup *pGrp = pOwner->GetGroup();
	if( !pGrp ) return;
...
void ioCountChargeItem2::OnItemGaugeOK( ioBaseChar *pOwner, SP2Packet &rkPacket )
{
	int iState;
	rkPacket >> iState;
...
void ioCountChargeItem2::SetExtraInfoToWeapon( ioWeapon *pWeapon, bool bJumpAttack )
{
...
bool ioCountChargeItem2::IsChargeAttackState( ioBaseChar *pOwner )
{
...
bool ioCountChargeItem2::IsEnableAttackDashCheck( ioBaseChar *pOwner, bool bCheck )
{
	if( pOwner->GetState() == CS_ATTACK )
	{
		switch( m_ChargeState )
...
void ioCountChargeItem2::CheckChargingMoveState( ioBaseChar *pOwner )
{
...
void ioCountChargeItem2::OnEndExtendAttack( ioBaseChar *pOwner, bool bReleaseItem )
{
	pOwner->SetAttackMoveEnable( false );
	pOwner->SetChargingState( false );
	m_bSetChargingMoveAni = false;
...
bool ioCountChargeItem2::IsChargeAutoTarget( ioBaseChar *pOwner )
{
	if( !pOwner )
		return false;
...
void ioCountChargeItem2::GetAutoTargetValue( ioBaseChar *pOwner,
...
float ioCountChargeItem2::GetChargeDamageRateForSkill()
{
...
float ioCountChargeItem2::GetChargeForceRateForSkill()
{
...
float ioCountChargeItem2::GetChargeBlowRateForSkill()
{
...
float ioCountChargeItem2::GetChargeRangeRateForSkill()
{
...
float ioCountChargeItem2::GetChargeSpeedRateForSkill()
{
...
float ioCountChargeItem2::GetChargeFloatRateForSkill()
{
...
float ioCountChargeItem2::GetChargeScaleRateForSkill()
{
...
float ioCountChargeItem2::GetChargeExplosionRateForSkill()
{
...
void ioCountChargeItem2::ClearCurChargeRateForSkill()
{
	m_fCurChargeRate = 0.0f;
	m_fCurChargeRateForSkill = 0.0f;
}

void ioCountChargeItem2::SetFireMotion( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioCountChargeItem2::SetFireEndMotion( ioBaseChar *pOwner )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioCountChargeItem2::SetAttackByAttribute( ioBaseChar *pOwner, const AttackAttribute &rkAttr, bool bEnd, float fExtraAniRate, float fExtraForceRate )
{
	if( !pOwner ) return;
...
bool ioCountChargeItem2::CheckRetreat( ioBaseChar *pOwner, bool bFront )
{
...
void ioCountChargeItem2::CheckReserve( ioBaseChar *pOwner )
{
