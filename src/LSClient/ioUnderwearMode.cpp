...
ioUnderwearMode::ioUnderwearMode( ioPlayStage *pCreator )
: ioPlayMode( pCreator )
{
	m_iBlueTeamCnt = 0;
	m_iRedTeamCnt = 0;
	...
	m_ModeType = MT_UNDERWEAR;

	m_vCharRecordList.reserve( 100 );
}

ioUnderwearMode::~ioUnderwearMode()
{
	m_bZeroHP = false;
	ClearMode();
}

void ioUnderwearMode::InitMode( int iMapIndex )
{
...
void ioUnderwearMode::LoadModeResource( ioINILoader &rkLoader )
{	
...
void ioUnderwearMode::ProcessMode()
{
...
void ioUnderwearMode::ProcessGUI()
{
...
void ioUnderwearMode::UpdateMiniMapSpotPoint()
{
...
void ioUnderwearMode::UpdateTimeScoreWnd()
{
...
void ioUnderwearMode::UpdateRoundStateWnd()
{
...
void ioUnderwearMode::ProcessReadyState()
{
...
void ioUnderwearMode::ProcessPlayState()
{
...
void ioUnderwearMode::CheckRoundEnd()
{
...
int ioUnderwearMode::GetTeamPlayerCount( TeamType eTeam )
{
	int iUserCnt = 0;
...
int ioUnderwearMode::GetTeamPurePlayerCount( TeamType eTeam )
{
	int iUserCnt = 0;
...
float ioUnderwearMode::GetBlueCatchRate()
{
...
float ioUnderwearMode::GetRedCatchRate()
{
...
void ioUnderwearMode::ProcessResultState()
{
...
void ioUnderwearMode::RenderPreGUI()
{
	if( m_ModeState == MS_PLAY )
	{
...
void ioUnderwearMode::RenderAfterGUI()
{
	/*
	if( m_ModeState == MS_PLAY )
	{
...
void ioUnderwearMode::AddNewRecord( ioBaseChar *pChar )
{
	if( !pChar )
		return;
...
void ioUnderwearMode::RemoveRecord( const ioHashString &rkName )
{
	int iCharCnt = m_vCharRecordList.size();
	for( int i=0 ; i<iCharCnt ; i++ )
	{
...
void ioUnderwearMode::RemoveRecord( int nIndex )
{
	int iCharCnt = m_vCharRecordList.size();
	if( iCharCnt > nIndex )
	{
...
ioBaseChar* ioUnderwearMode::GetRecordChar( int i )
{
...
ioBaseChar* ioUnderwearMode::GetRecordChar( const ioHashString &rkName )
{
...
int ioUnderwearMode::GetRecordCharCnt() const
{
	return m_vCharRecordList.size();
}

ModeRecord* ioUnderwearMode::FindModeRecord( int iIdx )
{
...
ModeRecord* ioUnderwearMode::FindModeRecordCharName( const ioHashString &rkName )
{
	return (ModeRecord*)FindRecordCharName( rkName );
}

CatchRecord* ioUnderwearMode::FindRecordCharName( const ioHashString &rkName )
{
	int iCharCnt = m_vCharRecordList.size();
	for( int i=0 ; i<iCharCnt ; i++ )
	{
...
void ioUnderwearMode::SetRoundEndInfo( WinTeamType eTeam, bool bRoundSetEnd )
{
	CheckLastKiller();
	ioPlayMode::SetRoundEndInfo( eTeam, bRoundSetEnd );
}

bool ioUnderwearMode::GetRoundInfo( int iIndex, RoundInfo &rkInfo )
{
...
bool ioUnderwearMode::GetResultInfo( int iIndex, ResultInfo &rkInfo )
{
...
bool ioUnderwearMode::GetFinalInfo( int iIndex, FinalInfo &rkInfo )
{
...
void ioUnderwearMode::ApplyModeInfo( SP2Packet &rkPacket )
{
...
void ioUnderwearMode::ApplyCharModeInfo( const ioHashString &rkName, SP2Packet &rkPacket, bool bDieCheck )
{
	bool bRecord;
	rkPacket >> bRecord;
...
bool ioUnderwearMode::ProcessTCPPacket( SP2Packet &rkPacket )
{
	ErrorReport::SetPosition( 1030, 9 );
...
bool ioUnderwearMode::ProcessUDPPacket( SP2Packet &rkPacket )
{
...
void ioUnderwearMode::NotifyEntityDie( ioPlayEntity *pEntity, ioPlayEntity *pKiller, ioBaseChar *pBestAttacker,
...
void ioUnderwearMode::NotifyCharDropDie( ioBaseChar *pDieChar, ioBaseChar *pKillChar, ioBaseChar *pBestAttacker,
									 float fLastRate, float fBestRate )
{
	OnNotifyCharDie( pDieChar, pKillChar, pBestAttacker, fLastRate, fBestRate );
...
void ioUnderwearMode::OnNotifyCharDie( ioBaseChar *pDieChar, ioBaseChar *pKillChar, ioBaseChar *pBestAttacker,
								  float fLastRate, float fBestRate )
{
	if( !pDieChar )
	{
		LOG.PrintTimeAndLog( 0, "ioUnderwearMode::OnNotifyCharDie - Entity is Not Char" );
		return;
	}
	...
	if( !pDieRecord )
	{
		LOG.PrintTimeAndLog( 0, "ioUnderwearMode::OnNotifyCharDie - %s Not Exist Char",
	...
	if( pKillChar )
	{
		...
		if( !pKillRecord )
		{
			LOG.PrintTimeAndLog( 0, "ioUnderwearMode::OnNotifyCharDie - %s Not Exist Kill Char",
...
void ioUnderwearMode::UpdateDropDieRecord( ioBaseChar *pDieChar, ioBaseChar *pKillChar, ioBaseChar *pBestAttacker )
{
	bool bDropKill = true;
...
void ioUnderwearMode::ReStartMode()
{
...
D3DXVECTOR3 ioUnderwearMode::GetRevivalPos( ioBaseChar *pChar )
{
...
void ioUnderwearMode::ContributeLOG()
{
	char szLog[MAX_PATH];
...
void ioUnderwearMode::ContributeConvertTeam( TeamType eTeam )
{
...
void ioUnderwearMode::ContributeConvert()
{
	ContributeConvertTeam( TEAM_NONE );        //TEAM_NONE는 전체 유저에 대한 기여도 평가	
}

int ioUnderwearMode::ContributeMyRank( int &rRankValue1, int &rRankValue2, int &rRankValue3, int &rRankValue4 )
{
...
void ioUnderwearMode::ContributeRoundPercent( DWORD dwCurTime, DWORD dwTotalTime )
{
	//기여도의 100분율
...
void ioUnderwearMode::ContributeRoundAllPercent( DWORD dwCurTime, DWORD dwTotalTime )
{
	//기여도의 100분율
...
void ioUnderwearMode::DamageContribute( const ioHashString szAttacker, const ioHashString szTarget, float fDamage, DWORD dwItemCode, int iWeaponType )
{
...
void ioUnderwearMode::KillContribute( const ioHashString szKiller, const ioHashString szKilled, DWORD dwItemCode )
{
...
void ioUnderwearMode::AttackCntContribute( const ioHashString szAttacker, const ioHashString szTarget, DWORD dwItemCode, int iAttackValue/* = 100 */ )
{
...
void ioUnderwearMode::PrisonerContribute( const ioHashString szAttacker, const ioHashString szPrisoner, bool bFirstPrisoner )
{
...
void ioUnderwearMode::PrisonerEscapeContribute( const ioHashString szAttacker, const ioHashString szPrisonerEscape )
{
...
void ioUnderwearMode::PrisonerDropContribute( const ioHashString szAttacker, const ioHashString szPrisonerDrop )
{
...
void ioUnderwearMode::CheckLastKiller()
{
...
void ioUnderwearMode::FillLastPlayRecordInfo( SP2Packet &rkPacket )
{
	int iCharCnt = m_vCharRecordList.size();
	rkPacket << iCharCnt;
	for(int i = 0;i < iCharCnt;i++)
...
void ioUnderwearMode::FillPlayRecordInfo( SP2Packet &rkPacket )
{	
	int iCharCnt = m_vCharRecordList.size();
	rkPacket << iCharCnt;
	for(int i = 0;i < iCharCnt;i++)
...
void ioUnderwearMode::ApplyPlayRecordInfo( SP2Packet &rkPacket )
{
	int iCharCnt;
	rkPacket >> iCharCnt;
	for(int i = 0;i < iCharCnt;i++)
...
void ioUnderwearMode::ContributeResultSync( const ioHashString &szName, int iContribute )
{
	CatchRecord *pRecord = FindRecordCharName( szName );
	if( !pRecord ) return;
...
void ioUnderwearMode::CheckNonePlayTime( ioBaseChar *pChar, bool bPlayerChanged )
{
	if( !pChar )
	{
		ioBaseChar *pOwner = GetOwnerChar();
...
void ioUnderwearMode::OnZeroHP( SP2Packet &rkPacket )
{
...
void ioUnderwearMode::OnRoundReady( SP2Packet &rkPacket )
{
	ShowEndScoreWnd( false );
	SetBlindMode( false );
...
void ioUnderwearMode::OnRoundStart( SP2Packet &rkPacket )
{
	ioPlayMode::OnRoundStart( rkPacket );
}

void ioUnderwearMode::OnRoundJoin( SP2Packet &rkPacket )
{
...
void ioUnderwearMode::OnRoundJoinView( SP2Packet &rkPacket )
{
	ioHashString szJoinChar;
	int iModeState;
...
void ioUnderwearMode::OnRoundJoinObserver( SP2Packet &rkPacket )
{
	ioHashString szJoinChar;
	int iModeState;
...
void ioUnderwearMode::OnPrisonerMode( SP2Packet &rkPacket )
{	
	...
	if( !pChar )
	{
		LOG.PrintTimeAndLog( 0, "ioUnderwearMode::PrisonerUser - %s Not Exist User", szPrisonerName.c_str() );
		return;
	}
...
void ioUnderwearMode::OnEscapePrisonerMode( SP2Packet &rkPacket )
{
	...
	if( !pChar )
	{
		LOG.PrintTimeAndLog( 0, "ioUnderwearMode::OnEscapePrisonerMode - %s Not Exist User", szName.c_str() );
		return;
	}
...
void ioUnderwearMode::SetTeamCatchedEvent( ioBaseChar *pChar )
{
	if( !pChar )	return;
...
void ioUnderwearMode::SetTeamDieEvent( ioBaseChar *pChar )
{
	if( !pChar )	return;
...
void ioUnderwearMode::EscapePrisonerChar( ioBaseChar *pChar )
{
...
void ioUnderwearMode::NotifyCharDrop( ioPlayEntity *pEntity, float fDropDamage )
{
...
void ioUnderwearMode::OnPrisonerDrop( SP2Packet &rkPacket )
{
...
void ioUnderwearMode::SetResultScoreWnd()
{
...
void ioUnderwearMode::SetTotalScoreWnd( IntVec &rvClassTypeList,
...
ioGameEntity* ioUnderwearMode::GetGuideTarget( const D3DXVECTOR3& vArrowPos )
{
	ioBaseChar *pOwner = GetOwnerChar();
...
void ioUnderwearMode::CheckEndSceneLookAt()
{
	if( !m_LastKillChar.IsEmpty() )
	{
...
void ioUnderwearMode::ClearModeData()
{
	m_LastDieChar.Clear();
	m_LastKillChar.Clear();
...
bool ioUnderwearMode::IsNeedRevival( ioBaseChar *pChar )
{
...
ioBaseChar* ioUnderwearMode::CheckMostNearChar( ioBaseChar *pChar )
{
...
void ioUnderwearMode::UpdateRevivalTime( const ioHashString &szName, DWORD dwCurRevivalTime )
{
...
int ioUnderwearMode::GetCurRank( const ioHashString& szCharName )
{
...
int ioUnderwearMode::GetContributePerChar( const ioHashString& szName )
{
	int iCharCnt = m_vCharRecordList.size();
