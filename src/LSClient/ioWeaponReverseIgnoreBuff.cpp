...
ioWeaponReverseIgnoreBuff::ioWeaponReverseIgnoreBuff()
{
}

ioWeaponReverseIgnoreBuff::ioWeaponReverseIgnoreBuff( const ioWeaponReverseIgnoreBuff &rhs )
: ioBuff( rhs )
{
	...
	m_vNoHitWeaponList = rhs.m_vNoHitWeaponList;
}
...
void ioWeaponReverseIgnoreBuff::LoadProperty( ioINILoader &rkLoader )
{
	...
	int iCnt = rkLoader.LoadInt_e( "nohit_attribute_cnt", 0 );
	for ( int i = 0; i < iCnt; ++i )
	{
		wsprintf_e( szBuff, "nohit_attribute_list%d", i+1 );
		int iWeaponIndex = rkLoader.LoadInt( szBuff, 0 );
		if ( iWeaponIndex == 0 )
			continue;

		m_vNoHitWeaponList.push_back( iWeaponIndex );
	}
}

ioBuff* ioWeaponReverseIgnoreBuff::Clone()
{
	return new ioWeaponReverseIgnoreBuff( *this );
}

void ioWeaponReverseIgnoreBuff::StartBuff( ioBaseChar *pOwner )
{
#ifndef SHIPPING 
...
bool ioWeaponReverseIgnoreBuff::ApplyStartBuff( ioBaseChar *pOwner, DWORD dwGlobalTime, DWORD dwGapTime )
{
	m_pOwner = pOwner;
...
void ioWeaponReverseIgnoreBuff::SetExtraDurationByGrowth( float fDuration )
{
...
void ioWeaponReverseIgnoreBuff::SetAction()
{
...
void ioWeaponReverseIgnoreBuff::EndBuff()
{
	if( m_bEnableOutLine )
	{
...
void ioWeaponReverseIgnoreBuff::ProcessBuff( float fTimePerSec )
{
...
void ioWeaponReverseIgnoreBuff::ProcessDelay( float fTimePerSec )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
void ioWeaponReverseIgnoreBuff::CheckOwnerStateCheck()
{
	if( !m_pOwner ) return;
...
bool ioWeaponReverseIgnoreBuff::CheckWeaponIndex( DWORD dwWeaponIndex )
{
	int iWeaponCount = m_vNoHitWeaponList.size();
	for( int i = 0; i < iWeaponCount; i++)
	{
		if( m_vNoHitWeaponList[i] == dwWeaponIndex )
			return false;
	}
	return true;
}