...
class ioLuciferAirGrapWeapon : public ioWeapon
{
protected:
...
public:
	ioLuciferAirGrapWeapon( ioBaseChar *pOwner, const WeaponAttribute *pAttr );
	virtual ~ioLuciferAirGrapWeapon();
};

inline ioLuciferAirGrapWeapon* ToLuciferAirGrapWeapon( ioWeapon *pWeapon )
{
	if( !pWeapon || pWeapon->GetType() != ioWeapon::WT_LUCIFER_AIR_GRAP )
		return NULL;

	return dynamic_cast< ioLuciferAirGrapWeapon* >( pWeapon );
}
