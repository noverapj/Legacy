...
ioFarmingMode::ioFarmingMode( ioPlayStage *pCreator )
: ioPlayMode( pCreator )
{
	m_iBlueTeamCnt = 0;
	m_iRedTeamCnt = 0;
	...
	m_ModeType = MT_FARMING;

	m_vCharRecordList.reserve( 100 );
}

ioFarmingMode::~ioFarmingMode()
{
	m_bZeroHP = false;
	ClearMode();
}

void ioFarmingMode::InitMode( int iMapIndex )
{
...
#ifndef SHIPPING
	LOG.PrintConsoleLog( 0, "ioFarmingMode::InitMode" );
#endif
}

void ioFarmingMode::LoadModeResource( ioINILoader &rkLoader )
{	
...
void ioFarmingMode::ProcessMode()
{
...
void ioFarmingMode::UpdateMiniMapSpotPoint()
{
...
void ioFarmingMode::UpdateTimeScoreWnd()
{
...
void ioFarmingMode::UpdateRoundStateWnd()
{
...
void ioFarmingMode::ProcessReadyState()
{
...
void ioFarmingMode::ProcessPlayState()
{
...
void ioFarmingMode::CheckRoundEnd()
{
...
int ioFarmingMode::GetTeamPlayerCount( TeamType eTeam )
{
	int iUserCnt = 0;
...
int ioFarmingMode::GetTeamPurePlayerCount( TeamType eTeam )
{
	int iUserCnt = 0;
...
float ioFarmingMode::GetBlueCatchRate()
{
...
float ioFarmingMode::GetRedCatchRate()
{
...
void ioFarmingMode::ProcessResultState()
{
...
void ioFarmingMode::RenderPreGUI()
{
	if( m_ModeState == MS_PLAY )
	{
...
void ioFarmingMode::RenderAfterGUI()
{
	/*
	if( m_ModeState == MS_PLAY )
	{
...
void ioFarmingMode::AddNewRecord( ioBaseChar *pChar )
{
	if( !pChar )
		return;
...
void ioFarmingMode::RemoveRecord( const ioHashString &rkName )
{
	int iCharCnt = m_vCharRecordList.size();
	for( int i=0 ; i<iCharCnt ; i++ )
	{
...
void ioFarmingMode::RemoveRecord( int nIndex )
{
	int iCharCnt = m_vCharRecordList.size();
	if( iCharCnt > nIndex )
	{
...
ioBaseChar* ioFarmingMode::GetRecordChar( int i )
{
...
ioBaseChar* ioFarmingMode::GetRecordChar( const ioHashString &rkName )
{
...
int ioFarmingMode::GetRecordCharCnt() const
{
	return m_vCharRecordList.size();
}

ModeRecord* ioFarmingMode::FindModeRecord( int iIdx )
{
...
ModeRecord* ioFarmingMode::FindModeRecordCharName( const ioHashString &rkName )
{
	return (ModeRecord*)FindRecordCharName( rkName );
}

CatchRecord* ioFarmingMode::FindRecordCharName( const ioHashString &rkName )
{
	int iCharCnt = m_vCharRecordList.size();
	for( int i=0 ; i<iCharCnt ; i++ )
	{
...
void ioFarmingMode::SetRoundEndInfo( WinTeamType eTeam, bool bRoundSetEnd )
{
	CheckLastKiller();
	ioPlayMode::SetRoundEndInfo( eTeam, bRoundSetEnd );
}

bool ioFarmingMode::GetRoundInfo( int iIndex, RoundInfo &rkInfo )
{
...
bool ioFarmingMode::GetResultInfo( int iIndex, ResultInfo &rkInfo )
{
...
bool ioFarmingMode::GetFinalInfo( int iIndex, FinalInfo &rkInfo )
{
...
void ioFarmingMode::ApplyModeInfo( SP2Packet &rkPacket )
{
...
void ioFarmingMode::ApplyCharModeInfo( const ioHashString &rkName, SP2Packet &rkPacket, bool bDieCheck )
{
	bool bRecord;
	rkPacket >> bRecord;
...
bool ioFarmingMode::ProcessTCPPacket( SP2Packet &rkPacket )
{
	ErrorReport::SetPosition( 1030, 9 );
...
bool ioFarmingMode::ProcessUDPPacket( SP2Packet &rkPacket )
{
...
void ioFarmingMode::NotifyEntityDie( ioPlayEntity *pEntity, ioPlayEntity *pKiller, ioBaseChar *pBestAttacker,
...
void ioFarmingMode::NotifyCharDropDie( ioBaseChar *pDieChar, ioBaseChar *pKillChar, ioBaseChar *pBestAttacker,
									 float fLastRate, float fBestRate )
{
	OnNotifyCharDie( pDieChar, pKillChar, pBestAttacker, fLastRate, fBestRate );
...
void ioFarmingMode::OnNotifyCharDie( ioBaseChar *pDieChar, ioBaseChar *pKillChar, ioBaseChar *pBestAttacker,
								  float fLastRate, float fBestRate )
{
	if( !pDieChar )
	{
		LOG.PrintTimeAndLog( 0, "ioFarmingMode::OnNotifyCharDie - Entity is Not Char" );
		return;
	}
	...
	if( !pDieRecord )
	{
		LOG.PrintTimeAndLog( 0, "ioFarmingMode::OnNotifyCharDie - %s Not Exist Char",
	...
	if( pKillChar )
	{
		...
		if( !pKillRecord )
		{
			LOG.PrintTimeAndLog( 0, "ioFarmingMode::OnNotifyCharDie - %s Not Exist Kill Char",
...
void ioFarmingMode::UpdateDropDieRecord( ioBaseChar *pDieChar, ioBaseChar *pKillChar, ioBaseChar *pBestAttacker )
{
	bool bDropKill = true;
...
void ioFarmingMode::ReStartMode()
{
...
D3DXVECTOR3 ioFarmingMode::GetRevivalPos( ioBaseChar *pChar )
{
...
void ioFarmingMode::ContributeLOG()
{
	char szLog[MAX_PATH];
...
void ioFarmingMode::ContributeConvertTeam( TeamType eTeam )
{
...
void ioFarmingMode::ContributeConvert()
{
	ContributeConvertTeam( TEAM_NONE );        //TEAM_NONE는 전체 유저에 대한 기여도 평가	
}

int ioFarmingMode::ContributeMyRank( int &rRankValue1, int &rRankValue2, int &rRankValue3, int &rRankValue4 )
{
...
void ioFarmingMode::ContributeRoundPercent( DWORD dwCurTime, DWORD dwTotalTime )
{
	//기여도의 100분율
...
void ioFarmingMode::ContributeRoundAllPercent( DWORD dwCurTime, DWORD dwTotalTime )
{
	//기여도의 100분율
...
void ioFarmingMode::DamageContribute( const ioHashString szAttacker, const ioHashString szTarget, float fDamage, DWORD dwItemCode, int iWeaponType )
{
...
void ioFarmingMode::KillContribute( const ioHashString szKiller, const ioHashString szKilled, DWORD dwItemCode )
{
...
void ioFarmingMode::AttackCntContribute( const ioHashString szAttacker, const ioHashString szTarget, DWORD dwItemCode, int iAttackValue/* = 100 */ )
{
...
void ioFarmingMode::PrisonerContribute( const ioHashString szAttacker, const ioHashString szPrisoner, bool bFirstPrisoner )
{
...
void ioFarmingMode::PrisonerEscapeContribute( const ioHashString szAttacker, const ioHashString szPrisonerEscape )
{
...
void ioFarmingMode::PrisonerDropContribute( const ioHashString szAttacker, const ioHashString szPrisonerDrop )
{
...
void ioFarmingMode::CheckLastKiller()
{
...
void ioFarmingMode::FillLastPlayRecordInfo( SP2Packet &rkPacket )
{
	int iCharCnt = m_vCharRecordList.size();
	rkPacket << iCharCnt;
	for(int i = 0;i < iCharCnt;i++)
...
void ioFarmingMode::FillPlayRecordInfo( SP2Packet &rkPacket )
{	
	int iCharCnt = m_vCharRecordList.size();
	rkPacket << iCharCnt;
	for(int i = 0;i < iCharCnt;i++)
...
void ioFarmingMode::ApplyPlayRecordInfo( SP2Packet &rkPacket )
{
	int iCharCnt;
	rkPacket >> iCharCnt;
	for(int i = 0;i < iCharCnt;i++)
...
void ioFarmingMode::ContributeResultSync( const ioHashString &szName, int iContribute )
{
	CatchRecord *pRecord = FindRecordCharName( szName );
	if( !pRecord ) return;
...
void ioFarmingMode::CheckNonePlayTime( ioBaseChar *pChar, bool bPlayerChanged )
{
	if( !pChar )
	{
		ioBaseChar *pOwner = GetOwnerChar();
...
void ioFarmingMode::OnZeroHP( SP2Packet &rkPacket )
{
...
void ioFarmingMode::OnRoundReady( SP2Packet &rkPacket )
{
	ShowEndScoreWnd( false );
	SetBlindMode( false );
...
void ioFarmingMode::OnRoundStart( SP2Packet &rkPacket )
{
	ioPlayMode::OnRoundStart( rkPacket );
}

void ioFarmingMode::OnRoundJoin( SP2Packet &rkPacket )
{
...
void ioFarmingMode::OnRoundJoinView( SP2Packet &rkPacket )
{
	ioHashString szJoinChar;
	int iModeState;
...
void ioFarmingMode::OnRoundJoinObserver( SP2Packet &rkPacket )
{
	ioHashString szJoinChar;
	int iModeState;
...
void ioFarmingMode::OnPrisonerMode( SP2Packet &rkPacket )
{	
	...
	if( !pChar )
	{
		LOG.PrintTimeAndLog( 0, "ioFarmingMode::PrisonerUser - %s Not Exist User", szPrisonerName.c_str() );
		return;
	}
...
void ioFarmingMode::OnEscapePrisonerMode( SP2Packet &rkPacket )
{
	...
	if( !pChar )
	{
		LOG.PrintTimeAndLog( 0, "ioFarmingMode::OnEscapePrisonerMode - %s Not Exist User", szName.c_str() );
		return;
	}
...
void ioFarmingMode::SetTeamCatchedEvent( ioBaseChar *pChar )
{
	if( !pChar )	return;
...
void ioFarmingMode::SetTeamDieEvent( ioBaseChar *pChar )
{
	if( !pChar )	return;
...
void ioFarmingMode::EscapePrisonerChar( ioBaseChar *pChar )
{
...
void ioFarmingMode::NotifyCharDrop( ioPlayEntity *pEntity, float fDropDamage )
{
...
void ioFarmingMode::OnPrisonerDrop( SP2Packet &rkPacket )
{
...
void ioFarmingMode::SetResultScoreWnd()
{
...
void ioFarmingMode::SetTotalScoreWnd( IntVec &rvClassTypeList,
...
ioGameEntity* ioFarmingMode::GetGuideTarget( const D3DXVECTOR3& vArrowPos )
{
	ioBaseChar *pOwner = GetOwnerChar();
...
void ioFarmingMode::CheckEndSceneLookAt()
{
	if( !m_LastKillChar.IsEmpty() )
	{
...
void ioFarmingMode::ClearModeData()
{
	m_LastDieChar.Clear();
	m_LastKillChar.Clear();
...
bool ioFarmingMode::IsNeedRevival( ioBaseChar *pChar )
{
...
ioBaseChar* ioFarmingMode::CheckMostNearChar( ioBaseChar *pChar )
{
...
void ioFarmingMode::UpdateRevivalTime( const ioHashString &szName, DWORD dwCurRevivalTime )
{
...
int ioFarmingMode::GetCurRank( const ioHashString& szCharName )
{
...
int ioFarmingMode::GetContributePerChar( const ioHashString& szName )
{
	int iCharCnt = m_vCharRecordList.size();
