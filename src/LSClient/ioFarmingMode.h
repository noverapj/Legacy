...
class ioFarmingMode : public ioPlayMode
{
protected:
...
public:
	ioFarmingMode( ioPlayStage *pCreator );
	virtual ~ioFarmingMode();
};

inline ioFarmingMode* ToFarmingMode( ioPlayMode *pMode )
{
	if( !pMode || pMode->GetModeType() != MT_FARMING )
		return NULL;

	return dynamic_cast< ioFarmingMode* >( pMode );
}
