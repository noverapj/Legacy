...
ioStateProtectionBuff::ioStateProtectionBuff()
{
}

ioStateProtectionBuff::ioStateProtectionBuff( const ioStateProtectionBuff &rhs )
: ioBuff( rhs ),
...
m_bNoEmoticon( rhs.m_bNoEmoticon ),
...
void ioStateProtectionBuff::LoadProperty( ioINILoader &rkLoader )
{
	...
	m_bNoEmoticon = rkLoader.LoadBool_e( "set_no_emoticon", false );

	m_bNoCancel = rkLoader.LoadBool_e( "no_cancel", false );
}

ioBuff* ioStateProtectionBuff::Clone()
{
	return new ioStateProtectionBuff( *this );
}

void ioStateProtectionBuff::StartBuff( ioBaseChar *pOwner )
{
#ifndef SHIPPING 
...
bool ioStateProtectionBuff::ApplyStartBuff( ioBaseChar *pOwner, DWORD dwGlobalTime, DWORD dwGapTime )
{
	m_pOwner = pOwner;
...
void ioStateProtectionBuff::SetExtraDurationByGrowth( float fDuration )
{
...
void ioStateProtectionBuff::EndBuff()
{
	if( m_bEnableOutLine )
	{
...
void ioStateProtectionBuff::ProcessBuff( float fTimePerSec )
{
...
void ioStateProtectionBuff::SetAction()
{
...
void ioStateProtectionBuff::ProcessDelay( float fTimePerSec )
{
	DWORD dwCurTime = FRAMEGETTIME();
...
float ioStateProtectionBuff::GetProtectionRate()
{
	return m_fDamageRate;
}

bool ioStateProtectionBuff::CheckOwnerStateCheck()
{
...
void ioStateProtectionBuff::CheckActiveCount()
{
	m_iCurActiveCount--;
	m_iCurActiveCount = max( m_iCurActiveCount, 0 );
}

bool ioStateProtectionBuff::IsNoCancel()
{
	return m_bNoCancel;
}