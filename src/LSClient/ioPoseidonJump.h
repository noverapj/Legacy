...
class ioPoseidonJump :	public ioExtendJump
{
public:
	enum ComboState
	{
...
protected:
	...
	ioZoneEffectWeapon*	m_AttachWeapon;
	...
	int	m_iMaxJumpChargeCnt;
	int	m_iCurJumpChargeCnt;
...
protected:
	...
	void	ChangeToChargeReadyState( ioBaseChar* pOwner );
	void	OnChargeReadyState( ioBaseChar* pOwner );
	...
	void	OnChargeActionEndState( ioBaseChar* pOwner );

	void	ResetExtraGauge( ioBaseChar* pOwner );

	void	TeleportDummy( ioBaseChar* pOwner );

public:
	ioPoseidonJump(void);
	ioPoseidonJump( const ioPoseidonJump& rhs );
	virtual ~ioPoseidonJump(void);
};

inline	ioPoseidonJump*	ToPoseidonJump( ioExtendJump* pJump )
{
	if( !pJump || pJump->GetType() != JT_POSEIDON_JUMP )
		return	NULL;

	return	dynamic_cast< ioPoseidonJump* >( pJump );
}