...
bool CSetupAndStart::SetupAndStartGame( bool bReadRegClientPath )
{
	...
	if(m_szAutoFileName.IsEmpty()||
		...
		m_szOptions.IsEmpty()||
#ifdef SRC_KR
		m_szIPKeyValue.IsEmpty()||
#endif
		m_szGameServerID.IsEmpty() ||
		m_szFullZipVer.IsEmpty() ||
		m_szParameterVersion.IsEmpty() ||
...
#ifndef STEAM_ATTACH
	StringCbPrintf(CmdLine, sizeof(CmdLine), " %s?%s?%s?%s?%s?%s?",
		(LPCTSTR)m_szExpandValue,(LPCTSTR)m_szKeyValue,(LPCTSTR)m_szOptions,
		(LPCTSTR)m_szGameServerID,(LPCTSTR)m_szFullZipVer,(LPCTSTR)m_szParameterVersion);
#else
	StringCbPrintf(CmdLine, sizeof(CmdLine), " %s%s,%s?%s?%s?%s?%s?%s?",
		(LPCTSTR)m_szExpandValue,(LPCTSTR)m_szOGPKey,(LPCTSTR)m_szSteamID,
		(LPCTSTR)m_szKeyValue,(LPCTSTR)m_szOptions,(LPCTSTR)m_szGameServerID,(LPCTSTR)m_szFullZipVer,(LPCTSTR)m_szParameterVersion);
#endif

#ifdef SRC_KR
	StringCbPrintf(CmdLine, sizeof(CmdLine), " %s?%s?%s?%s?%s?%s?%s?",
		(LPCTSTR)m_szExpandValue,(LPCTSTR)m_szKeyValue,(LPCTSTR)m_szOptions,
		(LPCTSTR)m_szIPKeyValue,(LPCTSTR)m_szGameServerID,(LPCTSTR)m_szFullZipVer,(LPCTSTR)m_szParameterVersion);
#endif

	// 대만 태국(가레나) 추가 인자값 처리
	if( (ioLocalManager::GetLocalType() == ioLocalManager::LCT_THAILAND) && !m_szAutoDownloadExt.IsEmpty() )
	{
...
bool CSetupAndStart::ParseCmd( LPCTSTR szCmdLine )
{
	if( szCmdLine == NULL )
		return false; 
	// lostsagatwbeta://e57edf3255496d42ff89199763131c61f2f5336b87727393e4,60170731525292/
	// lostsagatwbeta://d72cb7f8213a8def60dd4bff64fc3661f2f5336b87727393e4,60172933537452/  대만 cmd값
	//       lostsaga://8cdfb3edf0d7e935b6c8392c7c7c4e10a9c8d46343ca00,60169095704503/
...
#ifdef SRC_KR
		case OPTION_TYPE_IP_KEY_VALUE: m_szIPKeyValue = pos; break;
#endif
		case OPTION_TYPE_GAMESERVER_ID: m_szGameServerID = pos; break;

		case OPTION_TYPE_FULLZIP_VER: m_szFullZipVer = pos; break;
		case OPTION_TYPE_PARAMETER_VERSION: m_szParameterVersion = pos; break;
