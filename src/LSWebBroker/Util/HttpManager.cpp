...
bool HttpManager::SendGetData( IN const char *szURL, IN const char *szGetData, IN bool bHttps /*= true */ )
{
	if( szURL == NULL )
	{
		return false;
	...
	hRequest = HttpOpenRequest( hConnect, NULL, szSendData, NULL, NULL, NULL, iSecureFlags , NULL );
	if( hRequest == NULL )
	{
		InternetCloseHandle( hInternet );
...
#if WEB_RUN
	CString szGetData = kUrlcomponent.lpszUrlPath;// + szPostData;
	szGetData += "?c=set&v=";
	szGetData += szPostData;
	hRequest = HttpOpenRequest( hConnect, NULL, szGetData, NULL, NULL, NULL, iSecureFlags , NULL );
#else if SRC_KOR
	CString szGetData = kUrlcomponent.lpszUrlPath;// + szPostData;
	szGetData += "?";
	szGetData += szPostData;
...
#endif
#ifdef SRC_TW
	szGetData = kUrlcomponent.lpszUrlPath;// + szPostData;
	szGetData += "?";
	szGetData += szPostData;
	hRequest = HttpOpenRequest( hConnect, NULL, szGetData, NULL, NULL, NULL, iSecureFlags , NULL );
#endif
