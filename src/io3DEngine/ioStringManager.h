...
public:
	...
	void GetStringINI( IN const char *szKey, IN const char *szSecondKey, OUT char *szText, IN int iTextSize );
	const char* GetStringByID(const char* key);
	void GetStringByID(const char* key, char* out, int len);
...
#define GET_STRING( key ) g_StringMgr.GetStringByID(strlwr(key))
